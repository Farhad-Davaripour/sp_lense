...
import pytest
from soft_drift_constrained_fake_recording import build_full_recording
...
def test_full_native_full_vocabulary_actual_finalizer_and_reader(tmp_path, monkeypatch, accepted):
    ...
    for module in (checker, checker.inherited, checker.legacy, checker.engine):
...
def test_fixed_hash_containers_and_no_native_arrays(tmp_path):
    ...
    keys = {"A_row_sha256": ["0" * 64] * 12, "D_row_sha256": ["1" * 64] * 6}
...
def test_certificate_requires_new_full_pipeline_attestations(tmp_path, monkeypatch):
    monkeypatch.setattr(recording, "recording_policy", lambda root: {})
    monkeypatch.setattr(recording, "certificate_source_paths", lambda: frozenset())
    monkeypatch.setattr(recording, "IMMUTABLE_HELPERS", {})
    certificate = {
