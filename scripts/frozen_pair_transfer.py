"""One fixed paired development transfer, exact10/zero-derivative, no tuning."""
...
def evaluate(plan, backend, vectors, ledger, output):
    ...
    try:
        with guard, torch.no_grad():
            for cell in plan["cells"]:
                ...
                print(
                    f"completed {ledger.completed}/10 forwards; {guard.attempts}/0 derivatives",
                    flush=True,
                )
    finally:
        backend.model = model
    ...
    require(
        ledger.attempts == ledger.completed == len(rows) == 10 and guard.attempts == 0,
        "10/0 accounting",
    )
    return rows
...
def source_identity():
    ...
    for spec in config["candidates"].values():
        paths.extend([spec["path"], spec["construction_lock"]])
    for path in paths:
...
def freeze():
    ...
    return {"source_commit": record["source_commit"], "forwards": 10, "derivatives": 0}
...
def supervise(command, output, usage, timeout=600):
    ...
    try:
        with (output / "worker.log").open("xb") as log:
            ...
            print(
                f"worker pid={process.pid}; <=10 forwards / 0 derivatives; 600s including loading",
                flush=True,
            )
        ...
        if fa != 10 or fc != 10 or invalid or derivatives != 0 or elapsed > timeout:
            fault = fault or "10/0 accounting/deadline fault"
        status = {
