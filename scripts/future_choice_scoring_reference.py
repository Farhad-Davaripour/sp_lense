...
def reference_score(
    logits, baseline_logits, *, choice_a_token_id, choice_b_token_id, preserve_label
):
    ...
    ids = {"A": choice_a_token_id, "B": choice_b_token_id}
    if (
        preserve_label not in ids
        or ids["A"] == ids["B"]
        or any(type(i) is not int or not 0 <= i < len(z) for i in ids.values())
    ):
        raise ValueError("valid A/B IDs and semantic order required")
    other = "B" if preserve_label == "A" else "A"
