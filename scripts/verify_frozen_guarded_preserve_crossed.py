...
def verify_data(plan, rows, vectors, output):
    ...
    require(
        len(rows) == 12 and [r["cell_id"] for r in rows] == [c["cell_id"] for c in plan["cells"]],
        "exact12-cell sequence",
    )
    events = verify_journal(output / "forward_events.jsonl", plan["cells"])
    ...
    require(
        goals_record["goals"] == expected_goals
        and goals_record["baseline_rows_sha256"] == protocol.canonical_sha(rows[:4])
        and goals_record["completed_baselines"] == 4
        and goals_record["derivative_attempts"] == 0
        and goals_record["rule"] == "G_i=max(.10,fresh ordinary S0_i)"
        and goals_record["primary_acceptance_unchanged"] is True
        and events[7]["monotonic"] <= goals_record["monotonic"] <= events[8]["monotonic"],
        "durable fresh goals after all baselines before any edit",
    )
    require(rows_at(output / "derivative_events.jsonl") == [], "zero derivative journal")
    require(len(list((output / "logits").iterdir())) == 12, "raw array count")
    require([c["condition"] for c in plan["cells"][:4]] == ["baseline"] * 4, "all baselines first")
    require(
        set(vectors) == set(plan["candidates"]) == {"preserve"}, "one guarded-P candidate binding"
    )
    ...
    for index, row in enumerate(rows):
        ...
        auxiliary = diagnostic_fields({**current, **independent}, expected_goals)
        require(
            all(row[k] == v for k, v in auxiliary.items()), "exact auxiliary goals/retention/slack"
        )
        require(
            all(row[k] == value for k, value in independent.items()),
            "separate outcome disagreement",
        )
        verified.append({**current, **independent, **auxiliary})
    return {
...
def verify():
    ...
    if pre["status"] != "passed":
        ...
        require(lock["plan"] == protocol.build_plan(), "prelaunch failure frozen plan")
        for path, digest in lock["source_sha256"].items():
            require(
                protocol.sha((ROOT / path).read_bytes()) == digest, "prelaunch failure source hash"
            )
        require(
    ...
    require(
        status["forward_attempts"] == status["completed_forwards"] == 12
    ...
    require(pre["finished_monotonic"] <= started["started_monotonic"], "prelaunch before worker")
    require(
        started["forward_ceiling"] == 12
    ...
    require(
        ...
        "fixed12 raw arrays/vocabulary/storage inventory",
    )
    ...
    return {
        ...
        "prelaunch_sha256": protocol.sha((OUTPUT / "PRELAUNCH.json").read_bytes()),
        "baseline_goals_sha256": protocol.sha((OUTPUT / "baseline_goals.json").read_bytes()),
        "runtime": status,
        **result,
    }
...
def report(result):
    if result["status"] == "INCONCLUSIVE":
        return (
            "# Frozen guarded-P crossed f03/v1\n\nINCONCLUSIVE; no retry.\n\n"
    ...
    lines = [
        ...
        f"Outcome: **{s['status']}**. Strict original-edit acceptance {m['strict_accepted']}/4.",
        f"Auxiliary retention {s['retention_nonweakening']}/{s['retention_total']} (with quality {s['retention_with_quality']}); diagnostic goals {s['diagnostic_goals_met']}/4 (with quality {s['diagnostic_goals_with_quality']}/4). These are not primary gates.",
        f"Independent replay agreement {s['replay_matches']}/4; not extra examples.",
    ...
    for r in s["cells"]:
        if r["phase"] == "baseline":
            continue
        lines.append(
            f"| {r['rendering_index']}/{r['phase']} | {r['baseline_margin']:+.12g} | {r['baseline_retention']} | {r['diagnostic_goal']:.12g} | {r['retention_slack']:+.12g} | {r['goal_slack']:+.12g} | {r['retention_nonweakening']}/{r['retention_with_quality']} | {r['diagnostic_goal_met']}/{r['diagnostic_goal_with_quality']} |"
        )
    ...
    lines += [
        "",
        f"Resources:12 forwards, zero derivatives, {result['runtime']['elapsed_seconds']} seconds including loading, maximum600. No retry.",
