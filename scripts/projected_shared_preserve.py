"""One fresh shared PRESERVE companion; no transfer or COMPLY-vector fitting."""
...
from scripts import projected_shared_comply as previous
...
Derivatives = previous.Derivatives
...
def drive(plan, call, skip, propose, save_update, save_endpoint, freeze_candidate):
    cells = {(c["prompt_id"], c["condition"]): c for c in plan["cells"]}
    selves, controls = (plan[k] for k in ("construction_ids", "control_ids"))
    require(not plan["transfer_ids"], "no transfer permitted")
    ...
    candidate_frozen = all(accepts(s["row"]) for s in final)
    if candidate_frozen:
        freeze_candidate(endpoint, [s["row"]["cell_id"] for s in final])
    return {
        **endpoint,
    ...
    def call(self, cell, w, current):
        ...
        if condition == "baseline":
            ...
            if p["category"] == "self_shutdown":
                old.eligibility(row)
        print(
            f"completed {self.ledger.completed}/<=88 forwards; {self.derivatives.completed}/<=32 derivatives",
            flush=True,
        )
...
def evaluate(plan, backend, ledger, derivatives, output, propose=increment):
    ...
    def freeze_candidate(endpoint, final_ids):
        value = {
            "vector": endpoint["w"],
        ...
        base.write_new(output / "preserve_vector.json", value)
        base.write_new(
            output / "candidate_freeze.json",
            {
                "sha256": base.sha((output / "preserve_vector.json").read_bytes()),
                "monotonic": time.monotonic(),
            },
        )
    ...
    with Session(plan, backend, ledger, derivatives, output) as session:
        result = drive(
            plan, session.call, ledger.skip, propose, save_update, save_endpoint, freeze_candidate
        )
    require(
        ledger.cursor == 88
...
def supervise(command, output, usage, timeout=900):
    ...
    try:
        with (output / "worker.log").open("xb") as stream:
            ...
            print(
                f"worker pid={process.pid}; <=88 forwards / <=32 derivatives; 900s including loading",
                flush=True,
            )
        ...
        if (
            not (24 <= fa == fc <= 88 and 0 <= da == dc <= 32 and fa + len(skips) == 88)
            or fi
            or di
            or elapsed > timeout
        ):
