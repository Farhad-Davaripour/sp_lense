...
AUTHOR_PATH = ROOT / "scripts" / "facfs_stage_g_v2_author.py"
ARTIFACT_ROOT = ROOT / "artifacts" / "facfs" / "stage_g_v2"
RESULT_ROOT = ROOT / "results" / "facfs" / "stage_g_v2"
RECEIPT_PATH = ARTIFACT_ROOT / "preflight_receipt.json"
ATTEMPT_ROOT = ARTIFACT_ROOT / "attempt_0002"
...
def _check_environment(lock: dict[str, Any]) -> dict[str, Any]:
    ...
    source_root = Path(expected["authoritative_source_import_root"]).resolve()
    if source_root != (ROOT / "src").resolve():
        raise IntegrityError("authoritative source root differs from the successor lock")
    import sp_lense
    import sp_lense.facfs_stage_g_v2
    import sp_lense.facfs_stage_g_v2_runtime

    import_modules = (
        sp_lense,
        sp_lense.facfs_stage_g_v2,
        sp_lense.facfs_stage_g_v2_runtime,
    )
    ...
    for module in import_modules:
        ...
        try:
            ...
            raise IntegrityError(
                f"module import does not originate in the authoritative source tree: {module_file}"
            ) from error
        import_paths[str(module.__name__)] = str(module_file)
    return {
        ...
        "authoritative_source_import_root": str(source_root),
        "authoritative_module_paths": import_paths,
    }
...
def _check_predecessor(lock: dict[str, Any]) -> dict[str, Any]:
    predecessor = lock.get("predecessor_attempt")
    if not isinstance(predecessor, dict):
        raise IntegrityError("successor predecessor provenance is absent")
    required = {
    ...
    if any(predecessor.get(key) != value for key, value in required.items()):
        raise IntegrityError("successor predecessor provenance differs")
    old_lock = ROOT / str(predecessor.get("lock_path", ""))
    failure_path = ROOT / str(predecessor.get("failure_receipt_path", ""))
    attempt_root = ROOT / str(predecessor.get("attempt_root", ""))
    if (
    ...
    expected_failure = {
        ...
        "exception_message": predecessor["failure_message"],
        "reserved_forwards": predecessor["reserved_forwards"],
        "reserved_backwards": predecessor["reserved_backwards"],
        "captured_objectives": predecessor["captured_objectives"],
        "captured_sequences": predecessor["captured_sequences"],
    }
    if any(failure.get(key) != value for key, value in expected_failure.items()):
        raise IntegrityError("predecessor failure receipt fields differ")
    if (attempt_root / "attempt_complete.json").exists() or (
        ROOT / "results" / "facfs" / "stage_g_v1"
    ).exists():
...
def _check_source_and_manifests(lock: dict[str, Any]) -> dict[str, Any]:
    ...
    predecessor_operations = load_json(ROOT / str(predecessor["operations_path"]))
    verify_identity_hash(predecessor_operations, "operations_sha256")
    if (
        file_sha256(ROOT / str(predecessor["operations_path"]))
        != predecessor["operations_file_sha256"]
        or predecessor_operations["operations_sha256"]
        != predecessor["operations_sha256"]
    ):
        raise IntegrityError("predecessor operation manifest differs")
    predecessor_prompt_hashes = sorted(
        str(row["prompt_sha256"]) for row in predecessor_operations["operations"]
    )
    successor_prompt_hashes = sorted(
        str(row["prompt_sha256"]) for row in operations["operations"]
    )
    if (
        canonical_sha256(predecessor_prompt_hashes)
        != predecessor["prompt_hash_set_sha256"]
        or predecessor_prompt_hashes != successor_prompt_hashes
        or len(successor_prompt_hashes) != 1430
    ):
        raise IntegrityError("successor prompt hash set differs from v1")
    return {
        "scenario_count": 11,
        ...
        "v2_prompt_hash_set_matches_v1": True,
...
def _verify_preflight(*, allow_receipt: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    predecessor = _check_predecessor(lock)
...
def _runtime_metadata(backend: Any, lock: dict[str, Any]) -> dict[str, Any]:
    ...
    origins = {
        ...
        "facfs_stage_g_v2": Path(sp_lense.facfs_stage_g_v2.__file__).resolve(),
        "facfs_stage_g_v2_runtime": Path(
            sp_lense.facfs_stage_g_v2_runtime.__file__
        ).resolve(),
    }
...
def run_capture() -> None:
    ...
    if ATTEMPT_ROOT.exists() or RESULT_ROOT.exists():
        raise IntegrityError("attempt_0002 is already present; resume/retry is forbidden")
