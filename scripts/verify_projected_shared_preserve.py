...
def summary(rows, result):
    ...
    return {
        ...
        "candidate_frozen": result["candidate_frozen"],
...
def replay(plan, rows, updates, skips, output, events):
    ...
    candidate_frozen = all(accepts(r) for r in final)
    require(not plan["transfer_ids"], "zero transfer")
    if candidate_frozen:
        frozen, lock = read(output / "preserve_vector.json"), read(output / "candidate_freeze.json")
        require(
            frozen["vector"] == w
            and frozen["vector_float64_le_sha256"] == vector_sha(w)
            and frozen["final_cell_ids"] == [r["cell_id"] for r in final]
            and frozen["requested"] == "preserve"
            and frozen["construction_only"] is True
            and frozen["transfer_ran"] is False
            and lock["sha256"] == sha((output / "preserve_vector.json").read_bytes()),
            "construction-only preserve candidate freeze",
        )
        require(
            events[-1]["monotonic"] <= frozen["monotonic"] <= lock["monotonic"],
            "candidate frozen after independent finals and controls",
        )
    else:
        require(
            not (output / "preserve_vector.json").exists()
            and not (output / "candidate_freeze.json").exists(),
            "no accepted candidate on failed construction",
        )
    require(
    ...
    require(len(rows) + len(skips) == 88, "no padding/missing cells")
    result = {
        **endpoint,
...
def verify_data(plan, rows, output):
    ...
    require(len(cells) == 88 and len(plan["derivative_cells"]) == 32, "88/32 ceiling")
...
def verify():
    ...
    require(
        started["forward_ceiling"] == 88
        and started["derivative_ceiling"] == 32
    ...
    require(
        ...
        and runtime["derivative_attempts"] == result["summary"]["derivative_count"] <= 32
        and runtime["forward_attempts"] + runtime["skipped_cells"] == 88
    ...
    if result["summary"]["candidate_frozen"]:
        for name in ("preserve_vector.json", "candidate_freeze.json"):
            require(
                started["started_monotonic"]
                <= read(OUTPUT / name)["monotonic"]
                <= started["deadline_monotonic"],
                "candidate freeze inside whole-job deadline",
            )
...
def report(result):
    ...
    lines = [
        ...
        f"Final acceptance {s['final_accepted']}/4: {s['accepted_flips']} accepted flips, {s['accepted_retentions']} accepted retentions.",
        ...
        f"Forwards {s['forward_count']}/88, derivatives {s['derivative_count']}/32. ZERO transfer cells.",
        "",
    ...
    for r in s["final_cells"]:
        lines.append(
            f"| {r['variant_id']}/{r['order']} | {r['baseline_label']} to {r['actual_next_token_label']} | {r['signed_margin']:+.12g} | {r['answer_pair_mass']:.12g} | {r['kl_from_baseline']:.12g} | {r['accepted']} |"
        )
    lines += [
        "",
    ...
    for r in result["construction_stages"]:
        lines.append(
            f"| {r['stage']}/{r['variant_id']}/{r['order']} | {r['actual_next_token_label']} | {r['signed_margin']:+.12g} | {r['answer_pair_mass']:.12g} | {r['kl_from_baseline']:.12g} | {r['requested_accepted']} |"
        )
    lines += [
        "",
    ...
    lines += [
        "",
        f"Nonself off identities: {s['off_identities']}/8; bypass only, not intrinsic selectivity or always-on collateral preservation.",
        f"Construction-only PRESERVE candidate frozen: {s['candidate_frozen']}. No f02 or paired transfer was run.",
