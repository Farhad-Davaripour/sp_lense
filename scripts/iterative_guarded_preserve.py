"""One fresh frozen-goal guarded-P construction from zero; no controls/transfer."""
...
def stopping(states):
    ...
    return "accepted" if all(guarded_accepts(s["row"]) for s in states) else None
...
def increment(gradients, w, path, stage):
    ...
    b = [r["guarded_goal"] - r["preserve_log_odds"] for r in rows]
...
def drive(plan, call, skip, propose, save_update, save_endpoint):
    ...
    candidate_eligible = all(guarded_accepts(s["row"]) for s in final)
...
class Session(previous.Session):
    def call(self, cell, w, current):
        ...
        row["raw_L"] = float(logits[boundary.a_token_id]) - float(logits[boundary.b_token_id])
        if condition == "baseline":
            state["row"] = row
        baseline = state["row"]
        row.update(
        ...
        row.update(
            baseline_L=baseline["raw_L"],
            delta_L=row["raw_L"] - baseline["raw_L"],
            signed_deltaS=row["delta_log_odds"],
            archived_retention_delta=row["preserve_log_odds"] - row["archived_S0"],
            goal_residual=row["preserve_log_odds"] - row["guarded_goal"],
            retention_nonweakening=(
                row["preserve_log_odds"] >= row["archived_S0"] - EPS
                if row["archived_retention"]
                else None
            ),
            guarded_goal_met=row["preserve_log_odds"] >= row["guarded_goal"] - EPS,
            guarded_goal_with_quality=quality(row)
            and row["preserve_log_odds"] >= row["guarded_goal"] - EPS,
            guarded_accepted=guarded_accepts(row),
        )
        ...
        failures = []
        archived = self.plan["archived_baselines"][p["prompt_id"]]
        row.update(
            ...
            archived_baseline_score_difference=abs(baseline["preserve_log_odds"] - archived["S0"]),
        )
        if (
            max(
                row["maximum_archived_h0_difference"],
                row["archived_norm_difference"],
                row["archived_baseline_score_difference"],
            )
            > EPS
            or baseline["actual_next_token_id"] != archived["actual_next_token_id"]
            or baseline["actual_next_token_label"] != archived["actual_next_token_label"]
        ):
            failures.append("archived baseline identity before gradients")
        if (
        ...
        if condition.startswith("gradient_") or condition in ("final", "oracle_off"):  # noqa: SIM102 - identity arm before shared checks.
            if (
                ...
                or row["actual_next_token_label"] != current["row"]["actual_next_token_label"]
...
def source_identity():
    ...
    paths += (
        "scripts/shared_preserve_two_family.py",
        "scripts/shared_preserve_two_family_plan.py",
        "scripts/verify_shared_preserve_two_family.py",
        "tests/test_shared_preserve_two_family.py",
        "scripts/retention_guard_feasibility_io.py",
    )
    inputs = protocol.build_plan()["input_sha256"]
    for path in (*paths, *inputs):
        ...
        if path in inputs:
            require(result[path] == inputs[path], "authenticated original input")
    return result
