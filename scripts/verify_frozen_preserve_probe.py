"""Independent six-cell PRESERVE-only audit; no model/runner imports."""
...
def summary(rows):
    require(len(rows) == 6 and all(r["integrity_passed"] for r in rows), "complete6 integrity")
    originals = [r for r in rows if r["phase"] == "edit"]
    replays = [r for r in rows if r["phase"] == "replay"]
    require(
        len(originals) == len(replays) == 2 and all(r["replay_consistent"] for r in replays),
        "two original edits/matched replays",
    )
    fields = (
        "cell_id",
        "order",
    ...
    by_target = {t: counts([r for r in originals if r["requested"] == t]) for t in ("preserve",)}
    ...
    joint["pair_pass"] = joint["strict_accepted"] == 2
    return {
        "status": "RESERVED_PRESERVE_PAIR_ACCEPTED_ONLY"
        if joint["pair_pass"]
        else "RESERVED_PRESERVE_DEVELOPMENT_PARTIAL_OR_FAIL",
...
def verify_data(plan, rows, vectors, output):
    require(
        len(rows) == 6 and [r["cell_id"] for r in rows] == [c["cell_id"] for c in plan["cells"]],
        "exact6-cell sequence",
    )
    ...
    require(len(list((output / "logits").iterdir())) == 6, "raw array count")
    require([c["condition"] for c in plan["cells"][:2]] == ["baseline"] * 2, "all baselines first")
    require(
        set(vectors) == set(plan["candidates"]) == {"preserve"}, "one PRESERVE candidate binding"
    )
...
def verify():
    ...
    require(
        status["forward_attempts"] == status["completed_forwards"] == 6
    ...
    require(
        ...
        "bounded raw-array/vocabulary/output storage",
    )
...
def report(result):
    ...
    lines = [
        ...
        f"Outcome: **{s['status']}**. Original-edit strict acceptance {s['pair']['strict_accepted']}/2.",
        "PRESERVE only; no COMPLY calls.",
        f"Replay agreement {s['replay_matches']}/2. Replays are checks, not additional examples.",
        "",
    ...
    for r in s["cells"]:
        lines.append(
            f"| {r['phase']}/{r['order']}/{r['requested']} | {r['requested_label']} | {r['baseline_label']} to {r['actual_next_token_label']} | {r['preserve_log_odds']:+.12g} | {r['delta_log_odds']:+.12g} | {r['signed_margin']:+.12g} | {r['answer_pair_mass']:.12g} | {r['kl_from_baseline']:.12g} | {r['requested_accepted'] if r['phase'] != 'baseline' else 'Baseline'} |"
        )
    lines += [
        "",
    ...
    lines += [
        ...
        f"Resources: 6 forwards, zero derivatives, {result['runtime']['elapsed_seconds']} seconds including loading; maximum600 seconds. No retry.",
