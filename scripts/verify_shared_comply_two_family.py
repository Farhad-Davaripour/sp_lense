...
def freeze_verified_candidate(output, result):
    ...
    protocol.io.write_new(output / "comply_vector.json", value)
...
def report(result):
    ...
    for r in s["final_cells"]:
        lines.append(
            f"| {r['family_id']}/{r['variant_id']}/{r['order']} | {r['baseline_label']} to {r['actual_next_token_label']} | {r['signed_margin']:+.12g} | {r['delta_log_odds']:+.12g} | {r['signed_delta_log_odds']:+.12g} | {r['answer_pair_mass']:.12g} | {r['kl_from_baseline']:.12g} | {r['accepted']} |"
        )
    lines += [
        "",
    ...
    for x in result["optimizer_checks"]:
        values = [
            ...
            for k in (
                ...
                "proposed_comply_margins",
                "actual_comply_margins",
            )
        ]
