"""One frozen PRESERVE reserved development probe, exact6/zero-derivative."""
...
def validate_vectors(plan, vectors):
    require(set(vectors) == set(plan["candidates"]) == {"preserve"}, "exact one PRESERVE vector")
...
def evaluate(plan, backend, vectors, ledger, output):
            ...
            for cell in plan["cells"]:
                ...
                replay_ok = (
                    (
                        ...
                        and all(abs(row[k] - replay["row"][k]) <= EPS for k in NUMERIC_FIELDS)
                    )
                    if replay
                    else None
                )
                ...
                print(
                    f"completed {ledger.completed}/6 forwards; {guard.attempts}/0 derivatives",
                    flush=True,
                )
    finally:
        backend.model = model
    ...
    require(
        ledger.attempts == ledger.completed == len(rows) == 6 and guard.attempts == 0,
        "6/0 accounting",
    )
    return rows
...
def source_identity():
    ...
    paths = [
        ...
        "tests/test_frozen_pair_transfer.py",
        "tests/test_refreshed_gradient_control.py",
...
def freeze():
    ...
    return {"source_commit": record["source_commit"], "forwards": 6, "derivatives": 0}
...
def supervise(command, output, usage, timeout=600):
    ...
    try:
        with (output / "worker.log").open("xb") as log:
            ...
            print(
                f"worker pid={process.pid}; <=6 forwards / 0 derivatives; 600s including loading",
                flush=True,
            )
        ...
        if fa != 6 or fc != 6 or invalid or derivatives != 0 or elapsed > timeout:
            fault = fault or "6/0 accounting/deadline fault"
        status = {
