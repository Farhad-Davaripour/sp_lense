...
quality, accepts, stop_for = previous.quality, previous.accepts, previous.stop_for
compare_summary = previous.compare_summary
EPS = 1e-6


def summary(rows, result):
    return {**previous.summary(rows, result), "attempted_updates": result["attempted_updates"]}
...
def verify_update(update, gradients, w, path):
    ...
    expected = {
        ...
        "proposed_comply_margins": [x + y for x, y in zip(c, predicted_s, strict=True)],
        "actual_comply_margins": [x + y for x, y in zip(c, predicted_r, strict=True)],
        "predicted_comply_margins": [x + y for x, y in zip(c, predicted_r, strict=True)],
    ...
    return {
        ...
        "proposed_comply_margins": expected["proposed_comply_margins"],
        "actual_comply_margins": expected["actual_comply_margins"],
...
def replay(plan, rows, updates, skips, output, events):
    ...
    require(len(rows) + len(skips) == 92, "no padding/missing cells")
    result = {
        **endpoint,
...
def verify_data(plan, rows, output):
    ...
    require(len(cells) == 92 and len(plan["derivative_cells"]) == 32, "92/32 ceiling")
...
def verify():
    ...
    require(
        ...
        and runtime["forward_attempts"] + runtime["skipped_cells"] == 92
...
def report(result):
    text = (
        ...
        .replace("Shared nonlinear comply", "Projected shared comply")
        .replace("One shared nonlinear comply arrow", "One fresh projected shared comply arrow")
    )
    ...
    lines = [
        ...
        "## Projected updates",
        "",
        f"Attempted rounds: {s['attempted_updates']}/8; applied/scored: {s['updates']}.",
        "Actual path limit .40, instantaneous/net limit .20, per-step limit .05.",
        "",
        "| Stage | ||d|| | ||s|| proposed | ||r|| actual | Path | Net | Projection factor |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    for x in result["optimizer_checks"]:
        vals = [
            ...
            for k in ("signed_projection_loss", "proposed_comply_margins", "actual_comply_margins")
        ]
        lines.append(f"| {x['stage']} | " + " | ".join(vals) + " |")
    lines += [
        "",
