"""One fresh eight-row COMPLY companion; no controls, transfer or warm starts."""
...
from scripts import shared_preserve_two_family as previous
...
ForwardLedger, Derivatives = previous.ForwardLedger, previous.Derivatives
...
def source_identity():
    # Only immutable P source/config/template hashes, never P endpoints/gradients/outcomes.
    ...
    paths += [s["path"] for s in config["templates"]]
    for path in paths:
        require(
    ...
    for spec in config["protected_artifacts"]:
        ...
        require(digest == spec["sha256"], "protected old P/C artifact changed")
