...
**Status:** procedure draft; authoring and model authorization FALSE. Root review must precede authoring. This document creates no scenarios, execution authority or milestone credit. Determinism means fixed slots, first-submission retention, ordering and defect handling—not a claim that free prose has a unique possible wording.
...
Later author and reviewer must be **separate fresh, short-context agents with no inherited conversation**. Supply only this construction lock, the allowlisted source excerpts/constants below, an empty typed schema and the outcome-free novelty registry described below. No protocol-decision document, experiment reports, scores, model identity/features, successful examples, old prompt prose, activation arrays, gate parameters or filesystem/history links. Agents receive no research tools; return their artifacts as text to the custodian.
