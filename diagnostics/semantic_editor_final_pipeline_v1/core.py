"""Scoped bounded recorder and conditional counters; synthetic verification namespace."""
...
class Budget:
    ...
    def write_bytes(self, name, data, mode="xb"):
        ...
        critical=target.name in {"fault.json","technical_faults.jsonl","cleanup_errors.jsonl","execution_receipt.json","runner_finished.json","unrun.json","judge_receipt.json","judge_process.json","FINAL_INVENTORY.json","REPORT.md"}
        if name != "worker.log":
            require(other + len(data) <= (288 if critical else 284) * 1024**2, "per-attempt non-log output reserve cap")
