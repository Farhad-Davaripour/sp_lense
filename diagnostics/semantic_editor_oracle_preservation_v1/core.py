...
def source_hashes():
    paths = [HERE / n for n in ("core.py", "inputs.py", "guard_candidate.py", "hook_record.py", "token_boundaries.json", "mixed_boundary.py", "mixed_scoring.py", "word_boundary.py", "word_scoring.py", "word_reference.py", "run.py", "editor.py", "score.py", "test_control.py", "README.md")]
...
class Budget:
    """Reserve 4 MiB for capture; bound all other new files to 74 MiB."""
    ...
    def write_bytes(self, name, data, mode="xb"):
        ...
        if name != "worker.log":
            require(other + len(data) <= 74 * 1024**2, "non-log output reserve cap")
...
class Counter:
    ...
    def call(self,cell,forward):
        ...
        require(self.attempted<MAX_FORWARDS and self.cursor<len(self.cells),"51st forward blocked")
