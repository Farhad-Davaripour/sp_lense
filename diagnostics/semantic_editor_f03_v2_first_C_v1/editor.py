...
class EndpointBehaviorError(EligibilityError):
    """Finite endpoint failure stops this new assay after durable request cleanup."""
    pass
...
def evaluate(plan, backend, ledger, derivatives, output, guard):
    ...
    def on_request(p, request_spec, entry):
        ...
        try:
            ...
            completed=True
            stop_after_failed_endpoint(accepted(current["row"],sign,wanted) and accepted(replay["row"],sign,wanted))
        finally:
