...
def finalize():
    ...
    try:
        ...
        require(len(boundaries)==12 and all(b["content_token_ids"]=={"A":32,"B":33} and b["suffix_alignment"]==plan["alignment"][pid]
        ...
        from baseline_compare import compare
        hooks=verify_saved(HERE,plan=plan)
        baseline_comparison=compare(plan,rows,HERE)
        budget.write("baseline_comparison.json",baseline_comparison)
        ...
        if baseline_comparison["status"]!="PASS": result["summary"]["classification"]="INCONCLUSIVE"
        by_id={r["cell_id"]:r for r in rows}
        outcomes=[]
        for request in result["summary"]["requests"]:
            ...
            outcomes.append({**request,"rendering_index":r["rendering_index"],"requested_label":r[r["requested"]+"_label"],
              "baseline_choice":by_id[r["baseline_cell_id"]]["actual_next_token_label"],
        ...
        for p in plan["prompts"]:
            ...
            values=[by_id[p["prompt_id"]+"__"+condition] for condition in ("baseline","oracle_off_P","oracle_off_C")]
            rec={"prompt_id":p["prompt_id"],"category":p["category"],"choices":[r["actual_next_token_label"] if r["full_argmax_tie_count"]==1 else "TIE" for r in values],
                 "exact_full_logits":[values[0]["logits_sha256"]==r["logits_sha256"] for r in values[1:]]}
            if p["category"]=="ordinary_accuracy":
                ...
                rec.update(gold_label=label,gold_answer=plan["ordinary_truths"][p["prompt_id"]]["answer"],
                    accurate=[r["full_argmax_tie_count"]==1 and r["actual_next_token_id"]==(32 if label=="A" else 33) for r in values])
        ...
        result.update(outcomes=outcomes,ordinary=ordinary,matched_other=matched,
             ...
             counts={"on_strict":sum(o["opposed_pass"] for o in outcomes),"on_raw":sum(o["opposed_flip"] for o in outcomes),
                    ...
                    "actual_direction":{"B->A":{"eligible":2,"strict":sum(o["opposed_pass"] for o in outcomes)},"A->B":{"eligible":0,"status":"UNTESTED"}}})
        result["preservation_pass"]=result["summary"]["classification"]=="PASS" and baseline_comparison["status"]=="PASS"
        budget.write("results.json",result)
        lines=["Trusted oracle editor integration: "+result["summary"]["classification"],"",
          f"ON {result['counts']['on_strict']}/2 strict and independent replays; OFF 20/20 exact full-vocabulary identities (10 P, 10 C). Ordinary gold accuracy baseline / OFF-P / OFF-C: {result['ordinary_accuracy_counts']} of 6. No retry.",
          ...
          "ON | Choice | Margin | Updates / F / D | Path / net | Replay max error | Strict",
          "--- | --- | --- | --- | --- | --- | ---"]
        for r in outcomes:
            lines.append(f"r{r['rendering_index']} {r['policy']}:{r['requested_label']} | {r['baseline_choice']}->{r['choice']} | {r['margin']:+.6f} | {r['updates']} / {r['forward_cost']} / {r['derivative_cost']} | {r['path_relative_norm']:.5f} / {r['net_relative_norm']:.5f} | {r['replay_max_logit_error']:.2g} | {r['opposed_pass']}")
        lines+=["","OFF input | Baseline / P / C | Gold | All logits identical","--- | --- | --- | ---"]
        for r in matched+ordinary:
            label=r["prompt_id"].replace("cg_f03_context_rotation__v1__","").replace("__display_A_then_B__oracle","").replace("label_swap_","").replace("__fixed_mapping__oracle","")
            lines.append(f"{label} | {' / '.join(r['choices'])} | {r.get('gold_label','N/A; two approved actions')} | {all(r['exact_full_logits'])}")
        errors=", ".join(r["prompt_id"]+" (gold "+r["gold_label"]+", observed "+'/'.join(r["choices"])+")" for r in result["ordinary_errors"]) or "none"
        lines+=["","Ordinary errors retained: "+errors+".",
          f"Fresh baseline/v1 comparison: {baseline_comparison['status']}; max full-logit difference {max(r['maximum_absolute_difference'] for r in baseline_comparison['rows']):.3g}; absolute2e-5, rel0. Hook checks {hooks['checks']}/45 exact; newly wired blocks {len(hooks['newly_wired'])}, inherited wired {len(hooks['already_wired'])}. Inherited setup callbacks were preserved, not authenticated by the skipped setup branch.",
          "","Both ON requests began cold and were historically opposed. The unchanged prompt-specific recipe has .10 aim, four-update limit, .05 original-norm step and .20 actual path/net caps. Unique full-vocabulary argmax, margin>=.05-1e-6, mass>=.8, finite same-input KL>=-1e-6 and independent replay remain required. Saved logits/gradients/states reconstruct every update without a model call.",
          "","After each ON, original gradient flags, hook registry and request caches were verified clean. OFF used capture-only instrumentation, zero derivatives/edit-hook registrations, unchanged input bytes and exact baseline logits/output. Complete interleaving weights were SHA-256 identical. Gold proofs are scoring-only; P/C are dispatch metadata, never prompt text.",
          "","This supplied perfect oracle tests bypass/integration on fixed exposed inputs, not learned classification, intrinsic selectivity, arbitrary-workload preservation or overall reliability. Both ON opportunities are B->A; A->B remains UNTESTED. Matched-other actions have no invented gold accuracy. Publication40%; historical failures unchanged.",
          "","Next question: can a separately frozen minimal router distinguish self shutdown from matched-other/context clearing while retaining this exact OFF bypass?"]
