"""Closed exact-f04 production admission. Disabled during this preparation."""
...
def exact_cohort(plan):
    ...
    require(plan["self_prompt_ids"]==[p["prompt_id"] for p in plan["prompts"]] and len(plan["self_request_ids"])==4 and plan["ordinary_truths"]=={},"self-only exact denominator")
    from inputs import build_plan
    require(plan==build_plan(),"independent schedule and token-lock derivation")
    return {"prompts":2,"requests":4,"forwards":42,"derivatives":16,"fresh_routes":6,"endpoints":4,"off_checks":0}
...
def validate_release(value,raw,expected_sha,plan,now):
    ...
    require(value["authorizer"]=="root" and value["scope"]=="ONE_F04_SELF_ASSAY","exact root release scope")
    ...
    require(value["limits"]=={"forwards":42,"derivatives":16,"loads":1,"worker_seconds":300,"cleanup_seconds":15,"audit_seconds":90,"bytes":96*1024**2},"no alternate limits")
    usage=value["usage"]
