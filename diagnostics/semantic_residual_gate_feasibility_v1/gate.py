...
def fit_train(plan,rows):
    ...
    require([r["example_id"] for r in rows]==plan["train_ids"] and len(rows)==6,"explicit six training IDs/order only")
    manifest={r["example_id"]:r for r in plan["examples"]}
    for row in rows:
        m=manifest[row["example_id"]]
        require(row["family_id"]==m["family_id"]=="cg_f01_archive_closeout" and row["assay_split"]==m["assay_split"]=="train","f03/test rejected even if original split is discovery")
    ...
    model.fit([r["values"] for r in rows],[r["label"] for r in rows],split_labels=["discovery"]*6)
