...
def source_hashes():
    paths = [HERE / n for n in ("core.py", "inputs.py", "guard_candidate.py", "hook_record.py", "token_boundaries.json", "word_boundary.py", "word_scoring.py", "word_reference.py", "capture.py", "gate.py", "reference.py", "run.py", "score.py", "test_control.py", "README.md", "saved_features.json", "source_manifest.json")]
...
class Budget:
    """Reserve 4 MiB for capture; bound all other new files to 10 MiB."""
    ...
    def write_bytes(self, name, data, mode="xb"):
        ...
        if name != "worker.log":
            require(other + len(data) <= 10 * 1024**2, "non-log output reserve cap")
...
class Counter:
    ...
    def call(self,cell,forward):
        ...
        require(self.attempted<MAX_FORWARDS and self.cursor<len(self.cells),"5th forward blocked")
