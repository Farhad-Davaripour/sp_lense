"""Focused exact-feature, train isolation, deterministic math and four-capture tests."""
import copy
import json
import tempfile
import time
...
class GateTests(unittest.TestCase):
    ...
    def test_exact_source_features_transforms_boundaries_and_dedup(self):
        p=self.plan;self.assertEqual((len(p["examples"]),len(p["train_ids"]),len(p["test_ids"])),(12,6,6))
        self.assertEqual([x["category"] for x in p["examples"]],(["self_shutdown"]*2+["other_shutdown"]*2+["control"]*2)*2)
        self.assertEqual(len({x["prompt_sha256"] for x in p["examples"]}),12)
        legacy=core.git("show",inputs.LEGACY_COMMIT+":"+inputs.LEGACY_PATH);self.assertEqual(core.sha(legacy),inputs.LEGACY_SHA)
        original=[x for x in json.loads(legacy)["plan"]["prompts"] if x["category"] in ("other_shutdown","control")]
        ...
        data,_=inputs.archive(inputs.F01,["rows/01.json","freeze.json","runtime.json"])
        row=json.loads(data["rows/01.json"]);prompt=json.loads(data["freeze.json"])["plan"]["prompts"][0];boundary=json.loads(data["runtime.json"])["boundaries"][0]
        self.assertEqual(inputs.select_feature(row,prompt,boundary),row["h0"])
        for key,value in (("condition","retention"),("gradient",[0.]*1024),("target_sign",1)):
            bad=copy.deepcopy(row);bad[key]=value
            with self.assertRaises(ValueError):inputs.select_feature(bad,prompt,boundary)
    ...
    def test_explicit_family_fit_isolation_exact_reference_tie_degeneracy_reload(self):
        all_rows=self.synthetic();train=all_rows[:6]
        ...
        for row in changed[6:]:row["values"]=[999.]*1024;row["feature_sha256"]=core.sha(inputs.feature_bytes(row["values"]))
        self.assertEqual(core.json_bytes(params),core.json_bytes(gate.parameter_record(gate.fit_train(self.plan,changed[:6]))))
        with self.assertRaises(ValueError):gate.fit_train(self.plan,all_rows[6:]) # Every original split was discovery.
        wrong=copy.deepcopy(train);wrong[0]["family_id"]="cg_f03_context_rotation"
        ...
        with tempfile.TemporaryDirectory() as d:
            result=score.fit_and_score(self.plan,train,all_rows[6:],core.Budget(d))
            self.assertTrue(result["independent_arithmetic"]["all12_scores_exact"])
            self.assertEqual(core.read(Path(d)/"pretest_freeze.json")["test_scores_computed"],0)
    def test_four_capture_real_hook_recorder_and_final_receipts(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                with self.assertRaisesRegex(ValueError,"5th"):counter.call(p["cells"][-1],lambda:self.fail("fifth forward"))
            finally:guard.restore()
            self.assertEqual(counter.completed,4);self.assertTrue(all(r["derivatives"]==r["activation_edits"]==0 for r in rows))
            self.assertEqual(rows[0]["descriptive_logits_only"]["actual_next_token_label"],"OTHER")
            self.assertEqual(verify_saved(root,expected_checks=5)["checks"],5)
            saved=core.read(core.HERE/"saved_features.json")
            train,test=score.collect_features(p,saved,rows);self.assertEqual((len(train),len(test)),(6,6))
