...
def collect_features(plan,saved,capture_rows):
    ...
    for row,p in zip(capture_rows,plan["prompts"],strict=True):
        ...
        features.append({"example_id":p["example_id"],"family_id":p["family_id"],"assay_split":"train","label":0,"feature_sha256":row["feature_sha256"],"values":values})
    require(len(features)==12 and len({r["example_id"] for r in features})==12,"exact unique12 features; no baseline/replay duplicates")
...
def fit_and_score(plan,train,test,budget):
    ...
    require([e["example_id"] for e in events]==plan["train_ids"]+plan["test_ids"] and all(e["monotonic"]>=pretest["monotonic"] for e in events),"f03 scored only after durable parameter lock")
    result["parameter_sha256"]=pretest["fitted_parameters_sha256"]
    result["independent_arithmetic"]={"parameters_exact":True,"all12_scores_exact":True,"saved_reload_exact":True,"test_never_fitted":True}
    return result
...
def finalize():
    ...
    try:
        ...
        require(worker["status"]=="complete" and worker["model_load_attempts"]==worker["model_load_completed"]==1 and worker["forward_attempts"]==worker["forward_completed"]==4 and worker["derivatives"]==worker["activation_edits"]==0 and worker["unrun_cells"]==[],"one load4F0D0edits")
        rows=[read(p) for p in sorted((HERE/"rows").glob("*.json"))]
        require(len(rows)==4,"all four captured rows")
        ...
        hooks=verify_saved(HERE,expected_checks=5)
        ...
        result.update(classification="PASS" if complete else "FAIL",complete_pass=complete,worker=worker,hook_verification=hooks,
            ...
            model_forwards=4,model_derivatives=0,activation_edits=0)
        budget.write("results.json",result)
        lines=["Residual-state gate feasibility: "+result["classification"],"",
            f"Fixed f01 training accuracy {result['train']['correct']}/6; f03 test accuracy {result['test']['correct']}/6. Test confusion {result['test']['confusion']}; required2/2self ON and4/4nonself OFF. Threshold0,score>=0 ON; scores are not probabilities.",
            f"One pinned CPUfloat32 load,4new unedited captures,0derivatives/edits. Worker {worker['elapsed_seconds']:.3f}s including load {worker['load_elapsed_seconds']:.3f}s. Eight authenticated saved baselines reused; no OFF/replay duplicates.",
            "","Split/category/display | Score | Prediction | Truth | Correct","--- | --- | --- | --- | ---"]
        for split in ("train","test"):
            for row in result[split]["rows"]:lines.append(f"{split} {row['category']} {row['display_order']} | {row['score']:+.6f} | {'ON' if row['prediction'] else 'OFF'} | {'ON' if row['label'] else 'OFF'} | {row['correct']}")
        lines+=["",f"Unchanged CenteredCosineCentroidModel: binary64 training-only grand mean,centered row normalization,class centroids,normalized difference. All1024baseline h0components at block10 final INPUT token; no logits/choices/metadata/lengths. Parameters saved/hash-locked BEFORE f03 scoring ({result['parameter_sha256']}); independent math and saved reload reproduce all parameters and12scores exactly.",
            f"Strict hook checks{hooks['checks']}/5,zero-forward setup,weights/flags/grads/caches clean. All four new captures retained regardless word/mass. Exact dynamic negative-label transforms and cached boundaries authenticated.",
            "","This is supervised exposed f01-to-f03 development transfer,not pristine heldout evidence. The gate may exploit wording/option-consequence cues; no claim of understanding,calibrated probability,broad reliability,ordinary-task coverage or an integrated learned editor. No threshold/learner search,retry or subsequent job. Publication40%.",
            "","Next question: can one separately frozen integration of this fixed gate with the unchanged editor retain its measured ON outcomes and exact OFF bypass?" if complete else "The fixed gate failed this transfer; inspect the listed errors without automatic threshold,feature or learner tuning."]
