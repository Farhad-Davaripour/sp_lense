"""One bounded load, four capture-only forwards/zero derivatives; no retries."""
...
def worker():
    from capture import evaluate
    started = time.monotonic()
    budget = Budget(HERE)
    state = {"status": "failed", "model_load_attempts": 0, "model_load_completed": 0,
             "forward_attempts": 0, "forward_completed": 0, "derivatives": 0, "activation_edits": 0, "error": None}
    counter = guard = None
    ...
    try:
        ...
        for prompt in plan["prompts"]:
            ...
            require(boundary.evidence_record()["content_token_ids"] == {"KEEP":50057,"STOP":48964}, "explicit per-input token map")
        ...
        budget.write("runtime.json", {**backend.metadata(), "boundaries": boundaries,
                                      ...
                                      "hook": HOOK, "edited_position": "NONE; capture-only block10 final encoded INPUT token",
                                      "ordinary_forwards_inference_mode": True,
                                      "gradient_forwards_enable_grad_with_parameters_frozen": False})
        rows = evaluate(plan,backend,counter,HERE,guard)
        require(counter.attempts == counter.completed == counter.cursor == 4 and time.monotonic()<deadline,"four captures/deadline")
        ...
        if counter is not None:
            state.update(forward_attempts=counter.attempted, forward_completed=counter.completed,
                         unrun_cells=counter.cells[counter.cursor:])
...
def run(usage_path, release_commit):
    ...
    if capture["status"] != "complete_valid":
        budget.write_bytes("REPORT.md", b"Residual gate feasibility: INCONCLUSIVE. Worker/capture failed or timed out; partial artifacts retained. No retry. See capture.json and worker_final.json when present. Next recommendation: inspect saved failure evidence.\n")
    ...
    if receipt["status"] != "complete" and not (HERE / "REPORT.md").exists():
        budget.write_bytes("REPORT.md", b"Residual gate feasibility: INCONCLUSIVE saved-data audit. Failure receipts retained; no retry.\n")
...
if __name__ == "__main__":
    ...
    if args.command == "freeze":
        ...
        t = time.monotonic()
        frozen = check_freeze()
        print(json.dumps({"status": "PASS", "maximum_forwards": 4, "maximum_derivatives": 0, "model_loaded": False,
