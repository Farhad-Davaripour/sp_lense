...
def hook_signature(model):
    if hasattr(model,"active_hooks"):
        return repr(model.active_hooks)
    return repr([(name,tuple(module._forward_hooks),tuple(module._forward_pre_hooks),tuple(module._backward_hooks))
...
def evaluate(plan, backend, ledger, derivatives, output, guard):
    ...
    original_hooks=hook_signature(model)
    ...
    def clean_snapshot():
        return {"parameter_flags_restored":[p.requires_grad for p in parameters]==flags,
                ...
                "hook_registry_restored":hook_signature(model)==original_hooks,
    ...
    def call(
        ...
        if is_off:
            ...
            off_before={"derivatives":derivatives.attempts,"edit_hook_registrations":activity["edit_hook_registrations"],**clean_snapshot()}
        ...
        if is_off:
            ...
            clear_capture()
            off_after={"derivatives":derivatives.attempts,"edit_hook_registrations":activity["edit_hook_registrations"],**clean_snapshot()}
        ...
        print(
            f"completed {ledger.completed}/<=50 forwards; {derivatives.completed}/<=8 derivatives",
            flush=True,
        )
    ...
    def on_request(p, request_spec):
        ...
        require(all(clean_snapshot().values()),"ON cold clean entry")
        state=states[p["prompt_id"]]
        baseline={"row":state["row"],"logits":state["logits"]}
        sign=request_spec["sign"]
        require(eligibility(baseline["row"])==sign,"ON historically opposed request changed")
        active_request.update(prompt_id=p["prompt_id"],policy=request_spec["policy"])
        ...
        try:
            ...
            cleanup={"prompt_id":p["prompt_id"],"policy":request_spec["policy"],"request_completed":completed,
                     "monotonic":time.monotonic(),**clean_snapshot()}
    ...
    try:
        ...
        for spec in plan["requests"]:
            require(eligibility(states[spec["prompt_id"]]["row"])==spec["sign"],"ON baseline not historically opposed")
        for spec in plan["requests"]:
        ...
        receipt={"initial_weight_sha256":initial_weights,"final_weight_sha256":final_weights,
                 "weights_exact":initial_weights==final_weights,"monotonic":time.monotonic(),**clean_snapshot()}
    ...
    require(ledger.cursor==50 and ledger.attempts==ledger.completed
            and derivatives.attempts==derivatives.completed==sum(r["updates"] for r in requests)
            and ledger.attempts==34+2*derivatives.attempts,"conditional accounting incomplete")
