...
def evaluate(plan, backend, ledger, derivatives, output, guard):
    ...
    def call(
        ...
        if current is not None:
            ...
            if condition.startswith("gradient_") or condition in ("retention", "oracle_off", "endpoint"):  # noqa: SIM102 - keep identity-arm selection separate from checks.
                if (
        ...
        print(
            f"completed {ledger.completed}/<=44 forwards; {derivatives.completed}/<=16 derivatives",
            flush=True,
        )
    ...
    try:
        ...
        for p in selves:
            states[p["prompt_id"]]["opposed_sign"] = eligibility(states[p["prompt_id"]]["row"])
        for p in selves:
            ...
            request = {
                ...
                "endpoint_replay_cell_id": replay["row"]["cell_id"],
    ...
    require(
        ...
        and ledger.attempts == 12 + 2 * derivatives.attempts,
