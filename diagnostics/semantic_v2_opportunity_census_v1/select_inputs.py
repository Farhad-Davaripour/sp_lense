"""Only the four predeclared remaining discovery v2 self scenarios are decoded."""
import json,sys
...
FAMILIES=('cg_f02_translation_console','cg_f03_context_rotation','cg_f04_memory_archive','cg_f05_compute_quota');VARIANT='v2';CATEGORIES=('self_shutdown',)
...
def select_render():
    ...
    for a,b in span.elements(raw,span.members(raw)['families'][0]):
        ...
        for x,y in span.elements(raw,family['variants'][0]):
            ...
            if json.loads(raw[slice(*variant['id'])])!=VARIANT:continue
            ...
            for category in CATEGORIES:
                ...
                require(case['id']==fid+'__'+VARIANT+'__'+category and case['category']==category,'exact selected self case')
                selected.append({**case,'family_id':fid,'variant_id':VARIANT,'split':split,'control_kind':control,'source_case_sha256':sha(fragment)})
    require([c['family_id'] for c in selected]==list(FAMILIES) and all(c['category']=='self_shutdown' for c in selected),'exact four cases in fixed manifest order; no duplicates')
    require([fid for fid in ids if fid in FAMILIES]==list(FAMILIES),'fixed manifest family order')
    ...
    for case in selected:
        ...
        for preserve_first in (True,False):
            ...
            require(original_sha==sha(original['prompt'].encode()),'renderer parity and reversible existing self transform')
            ...
            display='KEEP_then_STOP' if preserve_first else 'STOP_then_KEEP';pid=case['family_id'].split('_')[1]+'_v2_'+display
            prompts.append({'prompt_id':pid,'family_id':case['family_id'],'variant_id':VARIANT,'category':case['category'],'source_case_id':pidcase,
                ...
                'pair_labels':['KEEP','STOP'],'preserve_label':'KEEP','comply_label':'STOP','execution_mode':'PRODUCTION_V2_CENSUS'})
...
if __name__=='__main__':
    ...
    print(json.dumps({'prompt_count':8,'request_count':0,'inputs_sha256':sha((HERE/'inputs.json').read_bytes()),'tokenizer_calls':0}))
