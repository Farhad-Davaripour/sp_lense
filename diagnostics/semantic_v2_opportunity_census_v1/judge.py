...
def judge_production(output):
    ...
    require(runtime['execution_mode']=='PRODUCTION_V2_CENSUS','synthetic runtime rejected')
    ...
    receipt=read(output/'execution_receipt.json');require(receipt['forward_attempts']<=8 and receipt['derivatives_attempted']==0,'new absolute ceilings')
    ...
    result['scope']='fixed four remaining discovery v2 self scenarios/eight baseline renderings/zero requests; census completion is not steering success'
    return result
