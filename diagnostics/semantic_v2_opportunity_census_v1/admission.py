"""Closed exact eight-row census production admission. Disabled during this preparation."""
...
def exact_cohort(plan):
    check_freeze();check_selection_freeze()
    from inputs import build_plan
    require(plan==read(HERE/"production_plan.json")==build_plan(),"exact eight census inputs and zero requests")
    require(plan["execution_mode"]=="PRODUCTION_V2_CENSUS" and plan["fixture_scope"]=="remaining_discovery_v2_baselines_only","no substituted/synthetic cohort")
    lock=read(HERE/"input_lock.json");raw=git("show",lock["input_commit"]+":"+HERE.relative_to(ROOT).as_posix()+"/inputs.json")
    require(raw==(HERE/"inputs.json").read_bytes() and sha(raw)==lock["inputs_sha256"],"committed exact input bytes")
    return {"prompts":8,"requests":0,"forwards":8,"derivatives":0,"fresh_routes":8,"off_checks":0,"endpoints":0}
...
def validate_release(value,raw,expected_sha,plan,now):
    ...
    require(value["authorizer"]=="root" and value["scope"]=="ONE_REMAINING_V2_BASELINE_CENSUS","exact root release scope")
    ...
    require(value["limits"]=={"forwards":8,"derivatives":0,"loads":1,"worker_seconds":300,"cleanup_seconds":15,"audit_seconds":90,"bytes":96*1024**2},"no alternate limits")
    usage=value["usage"]
