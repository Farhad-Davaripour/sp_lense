...
def finalize(output,worker_capture,audit_capture):
    ...
    final["assessment_status"]=result.get("assessment_status") if isinstance(result,dict) else "CENSUS_INCOMPLETE"
    final["steering_success_tested"]=False
    ...
    report=("Final census execution status: "+str(final["classification"])+"\nRaw saved-judge verdict: "+str(final["raw_judge_classification"])+
        ...
        "\nSee final_closeout.json and immutable raw judge/technical/cleanup/UNRUN artifacts. Four exposed discovery v2 self scenarios/eight baselines; zero steering requests, flips, retentions or OFF identity checks. Execution PASS only means complete verified census, never steering success.\n")
