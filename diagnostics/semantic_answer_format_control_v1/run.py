"""One load, at most four ordinary forwards, no intervention or derivative code."""
...
def record_four(plan,backend,counter,guard,budget,tokens):
    ...
    for index,(cell,encoded) in enumerate(zip(plan["cells"],tokens,strict=True),1):
        ...
        compressed=zlib.compress(raw);name=f"logits/{index:02d}.f32.zlib"
        budget.write_bytes(name,compressed)
        finite=bool(logits.isfinite().all())
        token_id=int(logits.argmax()) if finite else None
        decoded=backend.model.tokenizer.decode([token_id],skip_special_tokens=False,clean_up_tokenization_spaces=False) if finite else None
        budget.event("raw_rows.jsonl",{"cell_id":cell["cell_id"],"prompt_sha256":cell["prompt_sha256"],"logits_file":name,
             "raw_sha256":sha(raw),"compressed_sha256":sha(compressed),"vocabulary":logits.numel(),
             "prompt_length":int(encoded.shape[-1]),"captured_monotonic":time.monotonic(),
             "raw_argmax_id":token_id,"raw_argmax_decoded":decoded,"finite":finite})
        print(f"captured {index}/4",flush=True)
...
def worker():
    ...
    try:
        ...
        for cell in plan["cells"]:
            encoded=backend.encode(cell["prompt"])
            b=boundary(backend.model.tokenizer,backend.torch,cell["prompt"],expected=encoded)
            require(b["content_token_ids"]==plan["word_token_ids"],"frozen semantic word IDs")
            evidence={"cell_id":cell["cell_id"],"prompt_sha256":cell["prompt_sha256"],**b}
            require(evidence==plan["boundaries"][len(boundaries)],"frozen exact word boundary")
            boundaries.append(evidence);tokens.append(encoded)
        ...
        budget.write("runtime.json", {**backend.metadata(), "boundaries": boundaries,
                                      ...
                                      "registered_intervention_hooks": 0, "hidden_probes":0,"inference_mode": True})
        parameters=list(backend.model.parameters())
        initial_versions=[p._version for p in parameters]
        initial_weight_hash=weight_hash(parameters)
        record_four(plan,backend,counter,guard,budget,tokens)
        require([p._version for p in parameters]==initial_versions and all(p.grad is None for p in parameters),"frozen parameters/gradients")
        final_weight_hash=weight_hash(parameters)
        require(initial_weight_hash==final_weight_hash,"exact frozen weights")
        budget.write("weights_receipt.json",{"initial_sha256":initial_weight_hash,"final_sha256":final_weight_hash,
                   "exact_identity":True,"parameter_versions_unchanged":True,"parameter_gradients_absent":True})
        require(counter.attempted == counter.completed == 4 and time.monotonic() < deadline, "complete within deadline")
...
def run(usage_path, release_commit):
    ...
    if capture["status"] != "complete_valid":
        budget.write_bytes("REPORT.md", b"Semantic answer-format measurement: INCONCLUSIVE. Worker/capture failed or timed out; partial artifacts retained. No retry. See capture.json and worker_final.json when present. Next recommendation: inspect saved failure evidence.\n")
    ...
    if receipt["status"]!="complete" and not (HERE/"REPORT.md").exists():
        budget.write_bytes("REPORT.md",b"Semantic answer-format measurement: INCONCLUSIVE saved-data audit; no retry.\n")
...
def final_inventory(budget,quiescent):
    ...
    files = [{"path": p.relative_to(budget.root).as_posix(), "bytes": p.stat().st_size, "sha256": sha(p.read_bytes())}
             for p in sorted(budget.root.rglob("*")) if p.is_file() and "__pycache__" not in p.parts]
...
if __name__ == "__main__":
    ...
    if args.command == "freeze":
        ...
        t = time.monotonic()
        frozen = check_freeze()
        print(json.dumps({"status": "PASS", "cells": 4, "maximum_derivatives":0,"model_loaded": False,
