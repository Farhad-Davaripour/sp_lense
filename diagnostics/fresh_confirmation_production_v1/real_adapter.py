...
class RealAdapter:
    def __init__(self, model, recorder, guard, *, width=1024, vocabulary=248320,
                 expected_weight_sha256=None, runtime_metadata=None):
        ...
        require(self.parameters and all(p.device.type == "cpu" and
            (not p.is_floating_point() or p.dtype == torch.float32) for p in self.parameters), "all actual parameters CPU float32")
        require(not model.training and all(p.grad is None for p in self.parameters), "eval and no inherited gradients")
        ...
        if expected_weight_sha256 is not None:
            require(self.initial_digest == expected_weight_sha256, "actual weights match frozen gate runtime")
        ...
        require(getattr(model, "_last_hf_cache", None) is None, "no inherited bridge cache")
...
def make_backend(writer, counters, deadline):
    # No import of a loader, TransformerBridge or tokenizer until separately
    # authenticated production authorization and owned bootstrap are admitted.
    from production_admission import admit_production
    admitted = admit_production("worker")
