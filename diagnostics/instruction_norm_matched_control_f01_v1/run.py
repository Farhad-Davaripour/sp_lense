"""One load, 4 neutral + 8 archived last controls + 8 norm-matched suffix forwards; zero derivatives."""
...
def worker():
    from transfer import CaptureHook, norm, write_capture
    ...
    try:
        ...
        budget.write("runtime.json", {**backend.metadata(), "boundaries": boundaries,
                                      ...
                                      "hook": HOOK, "edited_position": "frozen final-token control or norm-matched fixed65-token suffix",
                                      "inference_mode": True})
        ...
        for index, cell in enumerate(plan["cells"], 1):
            item = plan["candidates"].get(cell["cell_id"])
            if item is None:
                h0 = load_matrix(HERE,plan["archived_baselines"][cell["cell_id"]])
                hook = CaptureHook(backend.torch,plan["alignment"][cell["prompt_id"]],h0)
            else:
                h0 = captures[cell["baseline_cell_id"]]["post"]
                expected = item["archived_last_actual_norm"] if cell["kind"]=="control" else captures[cell["paired_control_cell_id"]]["actual_total_norm"]
                hook = CaptureHook(backend.torch,plan["alignment"][cell["prompt_id"]],h0,
                                   load_matrix(HERE,item),item["window_offset"],expected)
            guard.cell = cell
            ...
            try:
                ...
                if hook.state is not None:
                    budget.write(f"states/{index:02d}.failure.json", {k:v for k,v in hook.state.items() if k not in ("pre","post")})
                raise
            ...
            capture_name = write_capture(budget, index, hook.state, cell["kind"] != "neutral")
            if cell["kind"] == "neutral":
                require(all(norm(row) > 0 for row in hook.state["post"]), "zero per-token h0")
