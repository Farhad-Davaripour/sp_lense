...
SELECTED = tuple(range(1, 13)) + tuple(range(25, 37))
...
def main():
    ...
    need(complete and min(complete) == 1, "step1 is chronological first complete scored stage, not best selected")
    ...
    for line_no in SELECTED:
        ...
        condition, stage = ("baseline", 0) if line_no <= 12 else ("step_1", 1)
        position = line_no - 1 if line_no <= 12 else line_no - 25
    ...
    need(len({rows[("step_1", i)]["shared_w_sha256"] for i in range(12)}) == 1,
         "one common recorded step1 vector identity (not a vector or optimizer audit)")
    ...
    for position in range(12):
        baseline, step = rows[("baseline", position)], rows[("step_1", position)]
        ...
        results.append({"row_index": position + 1, "prompt_id": baseline["prompt_id"],
                        ...
                        "comply_label": baseline["comply_label"], "baseline": bscore, "step1": sscore,
    ...
    result = {
        ...
        "first_complete_scored_step": 1, "selection_reason": "first complete chronological step, not score selection",
