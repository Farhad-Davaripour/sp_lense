"""One supervised saved-score child; no model, artifact writes, or retry."""
import base64
import hashlib
import json
import subprocess
...
def main():
    ...
    receipt = {
        "schema": "sp_lense.fixed24_snapshot_supervision.v1",
        "child_limit_seconds": 120,
        "cleanup_limit_seconds": 15,
        "numerical_invocations_in_this_successor": 0,
        "prior_failed_snapshot_invocations": 1,
        "retry_performed": False,
        "original_run_status": "DEADLINE-INCONCLUSIVE",
    }
    try:
        ...
        assert lock["child_limit_seconds"] == 120 and lock["cleanup_limit_seconds"] == 15
        ...
        command = [sys.executable, "-I", "-B", str(HERE / "snapshot.py")]
        receipt["command"] = command
        child_started = time.monotonic()
        process = subprocess.Popen(
            command, cwd=ROOT, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        )
        receipt["numerical_invocations_in_this_successor"] = 1
        stdout, stderr = process.communicate(
            timeout=max(0.0, 120 - (time.monotonic() - child_started))
        )
        ...
        receipt.update({
            ...
            "finally_sample": "After child exit/join and stream capture; excludes wrapper receipt encoding. Child authentication, scoring and result serialization are included.",
