...
def main():
    ...
    frozen("TOKENIZER")
    tests=read(HERE/"PURE_TEST_RECEIPT.json")
    require(tests["status"]=="PASS" and tests["source_freeze_sha256"]==sha((HERE/"TOKENIZER_SOURCE_FREEZE.json").read_bytes()),"frozen pure regression prerequisite")
    text_root=(ROOT/bindings["text_lock"]["path"]).parent
    for record in text_lock["files"]:
        raw=(text_root/record["path"]).read_bytes()
        require(len(raw)==record["bytes"] and sha(raw)==record["sha256"],"exact committed text artifact: "+record["path"])
    inputs=json.loads(authenticate(bindings["exact_inputs"]))
    ...
    try:
        ...
        for i,prompt in enumerate(prompts,1):
            ...
            token_map=token_map_for(prompt)
        ...
        if status=="TOKEN_BOUND":
            write("INPUT_LOCK.json",{"status":"TOKEN_BOUND_MODEL_NOT_AUTHORIZED","text_lock":bindings["text_lock"],"exact_inputs":bindings["exact_inputs"],
