"""Exactly four pure cases and one complete in-process four-request fake traversal."""
...
def main():
    ...
    try:
        ...
        stop_after_failed_endpoint(True)
        from hook_record import finish_preserving_original
        order=[]
        try:
            try:
                order.extend(["failure_persisted","request_persisted"])
                stop_after_failed_endpoint(False)
            finally:finish_preserving_original(out,"pure_endpoint_failstop",lambda:order.append("cleanup_complete"))
        except ValueError:order.append("scientific_stop_propagated")
        require(order==["failure_persisted","request_persisted","cleanup_complete","scientific_stop_propagated"],"durable failure and cleanup precede propagated stop")
        import ast
        tree=ast.parse((HERE/"editor.py").read_text())
        stop_try=[node for node in ast.walk(tree) if isinstance(node,ast.Try) and any(isinstance(x,ast.Call) and isinstance(x.func,ast.Name) and x.func.id=="stop_after_failed_endpoint" for statement in node.body for x in ast.walk(statement))]
        require(any(any(isinstance(x,ast.Call) and isinstance(x.func,ast.Name) and x.func.id=="finish_preserving_original" for statement in node.finalbody for x in ast.walk(statement)) for node in stop_try),"actual evaluator fail-stop enclosed by unchanged cleanup finally")
        cases.append({"case":"disabled_production_exact_cohort_endpoint_fail_stop","status":"PASS","propagation_order":order})
        from prepare import authenticate_parent_source
        require(authenticate_parent_source("authorization.json",b"later root release",sha(b"earlier false")) is False,"only exact lifecycle exclusion")
        require(authenticate_parent_source("editor.py",b"locked science",sha(b"locked science")) is True,"science remains mandatory")
        reject(lambda:authenticate_parent_source("editor.py",b"tampered science",sha(b"locked science")))
        reject(lambda:authenticate_parent_source("subdir/authorization.json",b"tampered",sha(b"locked")))
        require(read(HERE/"authorization.json")=={"run_authorized":False,"root_release_sha256":None,"reason":"Preparation only. One separately authenticated root release is required."},"new independent authorization remains false")
        cases.append({"case":"only_exact_parent_authorization_excluded_science_tamper_rejected","status":"PASS"});budget.write("pure_cases.json",cases)
        ...
        receipt.update(status="PASS_PREPARATION_ONLY",pure_cases=4,fake_forwards=14,fake_derivatives=2,strict_requests=4,flips=2,retentions=2,routes=6,skips=28,unrun=0,strict_checks=13,independent_judge="PASS",production_authorized=False)
