...
def selection():
    ...
    Budget(HERE).write("authorization_lifecycle.json",{"only_excluded_path":"authorization.json","parent_inventory_sha256":inventory["authorization.json"]["sha256"],
        ...
        "v1_result_commit":"bb960f3658a6b22629b67e053b099821359948b3","v1_status":"immutable INCONCLUSIVE preparation; no tests or scientific result"})
    source={}
    changed={}
...
def final():
    ...
    budget.write("freeze.json",{"source_sha256":{p.name:sha(p.read_bytes()) for p in files},"production_authorized":False,
        "one_batch":"4pure cases plus one in-process14F2D four-request synthetic workflow and independent saved judge"})
