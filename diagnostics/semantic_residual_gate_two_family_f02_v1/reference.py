...
def fit(vectors,labels):
    if len(vectors)!=12 or len(labels)!=12 or sum(labels)!=4:raise ValueError("fixed training sample")
    mean=[math.fsum(r[j] for r in vectors)/12 for j in range(1024)]
    unit=[normalize([r[j]-mean[j] for j in range(1024)]) for r in vectors]
    positive=[math.fsum(r[j] for r,y in zip(unit,labels) if y==1)/4 for j in range(1024)]
    negative=[math.fsum(r[j] for r,y in zip(unit,labels) if y==0)/8 for j in range(1024)]
