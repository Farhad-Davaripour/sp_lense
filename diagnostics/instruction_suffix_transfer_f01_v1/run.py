...
def worker():
    from transfer import CaptureHook, prepare_window, norm, write_capture
    started = time.monotonic()
    budget = Budget(HERE)
    ...
    try:
        ...
        budget.write("runtime.json", {**backend.metadata(), "boundaries": boundaries,
                                      ...
                                      "hook": HOOK, "edited_position": "fixed matched65-token answer suffix",
                                      "inference_mode": True})
        ...
        for index, cell in enumerate(plan["cells"], 1):
            h0 = donor = transfer_plan = None
            if cell["kind"] == "edit":
                h0 = captures[cell["baseline_cell_id"]]["post"]
                donor = captures[cell["donor_cell_id"]]["post"]
                transfer_plan = prepare_window(h0, donor)
            hook = CaptureHook(backend.torch, plan["alignment"][cell["prompt_id"]], h0, donor, transfer_plan)
            guard.cell = cell
            ...
            capture_name = write_capture(budget, index, hook.state, cell["kind"] == "edit")
