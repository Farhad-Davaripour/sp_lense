...
from transfer import prepare_window, verify_window, load_capture
...
def aggregate(rows):
    ...
    return {"status": "PASS" if donor_passes == receiver_passes == 8 else "MIXED_OR_FAIL",
            ...
            "by_actual_retention_direction": {d: opportunity_summary([r for r in edited if r["retention_direction"] == d], "retention")
                                              for d in ("A->A", "B->B")},
            "pairs": pairs, "rows": rows, "scope": "prompt-specific capped transfer over fixed matched answer suffix at block10"}
    ...
    try:
        ...
        for index, (cell, rawrow) in enumerate(zip(plan["cells"], raw_rows, strict=True), 1):
            ...
            require(state["integrity_passed"] and state["hook_calls"] == 1
                    and state["outside_sha_before"] == state["outside_sha_after"]
                    and state["outside_max_abs_difference"] == 0, "all-and-only fixed suffix capture integrity")
            boundary = boundaries[cell["prompt_id"]]
            require(boundary["suffix_alignment"] == plan["alignment"][cell["prompt_id"]]
                    and state["selected_positions"] == plan["alignment"][cell["prompt_id"]]["selected_positions"], "fixed suffix alignment")
            ...
            if cell["kind"] in ("neutral", "donor"):
                ...
                expected_plans = prepare_window(states[base_id]["post"], states[donor_id]["post"])
                expected_actual = verify_window(states[base_id]["post"], states[donor_id]["post"], state["pre"], state["post"], expected_plans)
                require(all(state[k] == v for k, v in expected_actual.items()), "per-token fixed cap/cast/actual metrics")
                ...
                row.update(choice_gate_pass=row["strict_pass"], kl_from_same_input_baseline=kl, kl_pass=kl_pass,
                           ...
                           per_token=state["per_token"], clipped_tokens=state["clipped_tokens"],
                           aggregate_raw_delta_frobenius=state["aggregate_raw_delta_frobenius"],
                           aggregate_planned_frobenius=state["aggregate_planned_frobenius"],
                           aggregate_actual_frobenius=state["aggregate_actual_frobenius"],
                           aggregate_actual_relative_frobenius=state["aggregate_actual_relative_frobenius"],
                           aggregate_h0_frobenius=state["aggregate_h0_frobenius"],
                           aggregate_post_minus_donor_frobenius=state["aggregate_post_minus_donor_frobenius"],
                           maximum_actual_relative_token_norm=state["maximum_actual_relative_token_norm"],
                           pre_baseline_max_abs_error=max(m["pre_baseline_max_abs_error"] for m in state["per_token"]))
        ...
        result["comparison_to_last_token"] = plan["prior_result_counts"]
        result["suffix_lengths"] = sorted({a["suffix_length"] for a in plan["alignment"].values()})
        result["edited_token_cells"] = sum(len(r["per_token"]) for r in rows if r["kind"] == "edit")
        result["clipped_token_cells"] = sum(r["clipped_tokens"] for r in rows if r["kind"] == "edit")
        budget.write("results.json", result)
        lines = ["Fixed answer-suffix activation transfer: "+result["status"], "",
                 f"Neutral choices: {', '.join(result['neutral_choices'])}; donors {result['donor_strict_passes']}/8 strict; receivers {result['receiver_strict_passes']}/8 strict; joint P/C pairs {result['receiver_joint_pairs']}/4.",
                 f"Eligible flips {result['flips']['strict_successes']}/{result['flips']['eligible']}; retentions {result['retentions']['strict_successes']}/{result['retentions']['eligible']}. Prior last-token transfer:0/4 flips and4/4 retentions. Zero eligible directions remain UNTESTED.", "",
                 "Cell/target | Neutral->edit | Donor | Margin | KL | Relative F-norm | Strict",
                 "--- | --- | --- | --- | --- | --- | ---"]
        for r in rows:
            if r["kind"] == "edit":
                kl_text = "NA" if r["kl_from_same_input_baseline"] is None else f"{r['kl_from_same_input_baseline']:.5g}"
                margin_text = "NA" if r["requested_margin"] is None else f"{r['requested_margin']:.4f}"
                lines.append(f"{r['cell_id'].replace('_edit','')}/{r['requested_label']} | {r['observed_transition']} | {r['donor_choice']}/{'pass' if r['donor_strict_pass'] else 'fail'} | {margin_text} | {kl_text} | {r['aggregate_actual_relative_frobenius']:.5f} | {'Pass' if r['strict_pass'] else 'Fail'}")
        lines += ["", f"Fixed suffix lengths:{result['suffix_lengths']}; edited token/cells:{result['edited_token_cells']}; clipped:{result['clipped_token_cells']}. Every aligned token ID was matched before model load. Receiver input bytes and all positions outside the fixed suffix were preserved. Compact binary states retain authenticated dimensions and byte order.",
                  "", "One pinned CPUfloat32 load,20 forwards,0 derivatives. Gates remain unique requested full-vocabulary argmax, margin>=binary64(.05-1e-6), A+Bmass>=.8, finite full scores and finite same-input edit||baseline KL>=-1e-6, without a KL upper cap. All donors are counted, including failures.",
                  "", "This is exploratory prompt-specific joint-suffix localization. It edits more positions and can spend MORE TOTAL displacement than last-token transfer; it is not an equal-strength comparison. Success would not distinguish distributed information from larger total change or interactions, or establish a reusable arrow, semantic representation, motive, reliability or ordinary-task preservation. Failure is not global impossibility. Publication remains40%.",
                  "", "Next question: what fixed control would separate suffix-specific information from the effect of greater total displacement?"]
