...
def verify_data(plan, rows, requests, skips, output, prefix_only=False):
    ...
    require(len(plan["cells"]) == 52 and len(plan["derivative_cells"]) == 8, "frozen ceiling")
    ...
    for i, row in enumerate(rows):
        ...
        live=row["condition"]=="entry" or row["condition"]=="endpoint" or row["condition"].startswith(("gradient_","step_"))
        baseline_id = row["prompt_id"] + ("__entry" if live else "__baseline")
        ...
        if "current_cell_id" in row:
            ...
            if row["condition"].startswith("gradient_") or (row["condition"] in ("retention","endpoint","entry") or row["condition"].startswith("oracle_off_")):
                require(
        ...
        if (row["condition"] in ("baseline", "retention","entry") or row["condition"].startswith("oracle_off_")):
            require(
                net == 0
    ...
    if prefix_only:
        ...
        return {"prefix_integrity_verified":True,"forward_count":len(rows),"derivative_count":len(gradient_cells),"skips":len(skips),"unrun_count":len(plan["cells"])-len(occupied)}
    expected_rows, expected_skips, outcomes, trajectories, reference_rows = [], [], [], [], []
    selves = [prompts[r["prompt_id"]] for r in plan["requests"]]
    ...
    partial = False
    for p in selves:
        baseline = get(p,"entry")
        require(baseline["current_cell_id"]==p["prompt_id"]+"__baseline","fresh ON entry references own preflight")
        ...
        for off in [prompts[pid] for pid in plan["off_prompt_ids"]]:
    ...
    require(
        len(rows) + len(skips) == 52 and len(rows) == 36 + 2 * len(gradient_cells),
        "conditional totals",
    )
    for event in skips:
    ...
    summary={"classification":label,"on_raw":sum(o["opposed_flip"] for o in outcomes),
             ...
             "forward_count":len(rows),"derivative_count":len(gradient_cells),"learned_gate_used":True,"old_verdicts_unchanged":True}
    return {
        "summary": summary,
...
def verify_routes(plan,rows,output):
    import gate_reference
    from learned_gate import feature_bytes
    raw=(output/plan["gate"]["path"]).read_bytes()
    require(sha(raw)==plan["gate"]["parameter_sha256"],"saved exact gate artifact")
    artifact=json.loads(raw);require(artifact["threshold"]==0,"unchanged zero threshold")
    captured=[r for r in rows if r["condition"] in ("baseline","entry") or r["condition"].startswith("oracle_off_")]
    events=rows_at(output/"routing_events.jsonl");require(len(events)==len(captured),"one fresh decision per routing capture")
    forward_events=rows_at(output/"forward_events.jsonl");positions={r["cell_id"]:i for i,r in enumerate(rows)}
    errors=[]
    for index,(r,event) in enumerate(zip(captured,events,strict=True),1):
        require(r["gradient"] is None and r["target_sign"]==0 and r["h"]==r["h0"] and not any(r["cumulative_offset"]),"route source is fresh unedited capture")
        value=gate_reference.score(artifact["parameters"],r["h"]);route="ON" if value>=0 else "OFF";saved=r["routing"]
        require(saved["score"]==value and saved["prediction"]==int(value>=0) and saved["route"]==r["route"]==route and saved["decision_index"]==index,"independent fresh route score/prediction")
        require(saved["feature_sha256"]==sha(feature_bytes(r["h"])) and saved["source_cell_id"]==r["cell_id"] and saved["source_logits_sha256"]==r["logits_sha256"]
                and saved["parameter_sha256"]==plan["gate"]["parameter_sha256"] and saved["threshold"]==0,"fresh state/raw capture/artifact binding")
        require(event=={"cell_id":r["cell_id"],"prompt_id":r["prompt_id"],**saved},"durable routing receipt identity")
        j=positions[r["cell_id"]]
        require(forward_events[2*j+1]["monotonic"]<=saved["monotonic"] and (j+1==len(rows) or saved["monotonic"]<=forward_events[2*j+2]["monotonic"]),"fresh routing after current capture,before next forward")
        if route!=plan["expected_routes"][r["prompt_id"]]:errors.append({"cell_id":r["cell_id"],"expected":plan["expected_routes"][r["prompt_id"]],"actual":route,"score":value})
    by_id={r["cell_id"]:r for r in rows}
    for r in rows:
        if r not in captured:
            require("routing" not in r and r["routing_entry_cell_id"]==r["prompt_id"]+"__entry" and by_id[r["routing_entry_cell_id"]]["route"]=="ON","no edited-state rerouting")
    final=read(output/"gate_final.json")
    require(final["parameters_unchanged"] and final["fit_calls"]==0 and final["decisions"]==len(captured) and final["parameter_sha256"]==plan["gate"]["parameter_sha256"],"whole gate immutability/accounting")
    return {"decisions":len(captured),"preflight":sum(r["condition"]=="baseline" for r in captured),"live_requests":sum(r["condition"]!="baseline" for r in captured),
            "errors":errors,"all_scores_exact":True,"parameters_unchanged":True,"fit_calls":0}
...
def finalize():
    ...
    try:
        ...
        require(worker["status"] in ("complete","routing_failed") and worker["parameter_checks_passed"]
                and worker["model_load_attempts"]==worker["model_load_completed"]==1
                and worker["forward_attempts"]==worker["forward_completed"]<=52
                and worker["derivatives"]==worker["derivatives_completed"]<=8,"worker accounting")
        ...
        require(len(skips)==worker["skipped_forwards"],"skip accounting")
        routes=verify_routes(plan,rows,HERE)
        if worker["status"]=="routing_failed":
            require(routes["errors"],"scientific routing failure needs authenticated wrong decision")
            prefix=verify_data(plan,rows,requests,skips,HERE,prefix_only=True)
            from hook_record import verify_saved
            count=len(rows_at(HERE/"hook_evidence/checks.jsonl"))
            hooks=verify_saved(HERE,expected_checks=count)
            source=read(HERE/"hook_evidence/source_lock.json")
            require(source["passed"] and source["before_materialization"] and source["installed_sources_sha256"]==plan["hook_integration"]["installed_sources_sha256"],"routing-stop strict source lock")
            integration=read(HERE/"integration_cleanup.json")
            require(integration["weights_exact"] and all(v is True for k,v in integration.items() if k not in ("initial_weight_sha256","final_weight_sha256","monotonic")),"routing-stop cleanup")
            occupied={r["cell_id"] for r in rows}|{s["cell"]["cell_id"] for s in skips}
            require(worker["unrun_cells"]==plan["cells"][len(occupied):],"exact UNRUN suffix")
            budget.write("results.json",{"summary":{"classification":"FAIL","reason":"fresh learned routing mismatch"},"routing":routes,"prefix_audit":prefix,"worker":worker,"hook_verification":hooks,
                "unrun_cells":worker["unrun_cells"],"skipped_forwards":len(skips),"preservation_pass":False,"ordinary_accuracy":"not inferred for unrun cells"})
            budget.write_bytes("REPORT.md",("Learned-gate integration: FAIL (routing).\n"+json.dumps(routes["errors"])+"\nRemaining cells UNRUN; executed/skipped prefix audited, no overridden route or preservation success. No retry. Publication40%.\n").encode())
            receipt["status"]="complete";return
        require(not routes["errors"] and routes["decisions"]==34 and len(requests)==2 and worker["unrun_cells"]==[],"complete correct live routing and two trajectories")
        from hook_record import verify_saved
        ...
        for request in result["summary"]["requests"]:
            ...
            outcomes.append({**request,"rendering_index":r["rendering_index"],"requested_label":r[r["requested"]+"_label"],
              ...
              "forward_cost":2*request["updates"]+2,"derivative_cost":request["updates"]})
        ...
        lines=["Semantic editor / frozen learned-gate integration: "+result["summary"]["classification"],"",
          ...
          f"One load; {len(rows)}/52 forwards,{worker['derivatives']}/8 derivatives,{len(skips)} explicit skips. Worker {worker['elapsed_seconds']:.3f}s including load {worker['load_elapsed_seconds']:.3f}s.","",
        ...
        lines+=["","Ordinary errors (baseline/C/P), unchanged: "+errors+".",
          ...
          "","All six f02 semantic prompts and six ordinary A/B prompts unchanged. Fresh learned routing34/34,12preflight plus22live request captures,independent scores exact,zero fits. No category/gold/format override or edited-state rerouting. Exposed development integration only,not broad or independent generalization; wording cues remain possible. Both flips target displayed-second; no position-independent semantic claim. Matched paths approved,no invented gold. Publication40%.",
          "","Next question: what is the smallest preregistered final-study design and independent verification needed before expanding this development result?" if result["preservation_pass"] else "This attempt's exact failure is final; no automatic retry or tuning."]
