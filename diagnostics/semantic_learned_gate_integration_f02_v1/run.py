...
def worker():
    ...
    try:
        ...
        for prompt in plan["prompts"]:
            ...
            require(boundary.evidence_record()["content_token_ids"] == prompt["token_map"], "explicit per-input token map")
        ...
        require(counter.attempts == counter.completed == 36+2*derivatives.completed
                and counter.cursor == 52 and time.monotonic()<deadline,"conditional completion/deadline")
        state["skipped_forwards"] = len(counter.skips)
        state["parameter_checks_passed"] = all(r["integrity_passed"] for r in rows)
        state["status"] = "complete"
        return
    except RoutingMismatch as error:
        state["status"]="routing_failed"
        state["error"]="RoutingMismatch: "+str(error)[:2048]
        state["parameter_checks_passed"]=read(HERE/"integration_cleanup.json")["weights_exact"]
        recorded=[json.loads(line) for line in (HERE/"requests.jsonl").read_text().splitlines()] if (HERE/"requests.jsonl").exists() else []
        budget.write("request_summary.json",recorded)
        return
...
if __name__ == "__main__":
    ...
    if args.command == "freeze":
        ...
        t = time.monotonic()
        frozen = check_freeze()
        print(json.dumps({"status": "PASS", "maximum_forwards": 52, "maximum_derivatives": 8, "model_loaded": False,
