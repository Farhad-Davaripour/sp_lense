...
def evaluate(plan, backend, ledger, derivatives, output, guard):
    ...
    def call(
        ...
        if current is not None:
            ...
            if (condition in ("retention","entry") or is_off) and (
        ...
        print(
            f"completed {ledger.completed}/<=52 forwards; {derivatives.completed}/<=8 derivatives",
            flush=True,
        )
    ...
    def on_request(p, request_spec, entry):
        ...
        require(eligibility(baseline["row"])==sign and baseline["row"]["actual_next_token_label"]==p["expected_baseline_word"],"ON expected baseline/opposed request changed")
        active_request.update(prompt_id=p["prompt_id"],policy=request_spec["policy"],route=entry["row"]["routing"]["route"],entry_cell_id=entry["row"]["cell_id"])
        ...
        try:
            ...
            request={"prompt_id":p["prompt_id"],"opposed_sign":sign,"policy":request_spec["policy"],
            ...
            def on_cleanup():
                ...
                cleanup={"prompt_id":p["prompt_id"],"policy":request_spec["policy"],"request_completed":completed,
                         "monotonic":time.monotonic(),**clean_snapshot("ON-finally:"+p["prompt_id"])}
                Budget(output).event("cleanup_events.jsonl",cleanup)
                require(all(v is True for k,v in cleanup.items() if k not in ("prompt_id","policy","monotonic","request_completed")),"ON cleanup failed")
    ...
    try:
        ...
        require_expected([states[p["prompt_id"]]["row"] for p in plan["prompts"]],plan["expected_routes"])
        for spec in plan["requests"]:
            require(eligibility(states[spec["prompt_id"]]["row"])==spec["sign"] and states[spec["prompt_id"]]["row"]["actual_next_token_label"]==prompt_by_id[spec["prompt_id"]]["expected_baseline_word"],"ON baseline expected word/opposed eligibility")
        for spec in plan["requests"]:
            p=prompt_by_id[spec["prompt_id"]]
            require(all(clean_snapshot("ON-entry:"+p["prompt_id"]).values()),"fresh ON capture begins cold and clean")
            preflight=states[p["prompt_id"]]
            entry=call(p,"entry",current={"row":preflight["row"],"logits":preflight["logits"]})
            clear_capture()
            require_expected([entry["row"]],plan["expected_routes"])
            if entry["row"]["routing"]["route"]=="ON":
                on_request(p,spec,entry)
            else:
                raise RoutingMismatch("unexpected OFF request: no edit")
            for pid in plan["off_prompt_ids"]:
                off=prompt_by_id[pid];state=states[pid]
                fresh=call(off,"oracle_off_"+spec["policy"],current={"row":state["row"],"logits":state["logits"]})
                require_expected([fresh["row"]],plan["expected_routes"])
                require(fresh["row"]["route"]=="OFF","OFF output must be this fresh capture,without extra inference")
    finally:
        def matrix_cleanup():
            clear_capture()
            active_request.clear()
    ...
    require(ledger.cursor==52 and ledger.attempts==ledger.completed
            and derivatives.attempts==derivatives.completed==sum(r["updates"] for r in requests)
            and ledger.attempts==36+2*derivatives.attempts,"conditional accounting incomplete")
    require(gate.calls==34,"complete live routing count")
