...
def judge_production(output):
    ...
    require(runtime['execution_mode']=='PRODUCTION_F05','synthetic runtime rejected')
    ...
    receipt=read(output/'execution_receipt.json');require(receipt['forward_attempts']<=42 and receipt['derivatives_attempted']<=16,'new absolute ceilings')
    ...
    result['scope']='exact f05 discovery self pair, four requests; first-position flip only if natural opportunity; no ordinary preservation'
    return result
