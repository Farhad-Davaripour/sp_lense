...
class DerivativeLedger:
    ...
    def call(self, cell, function, *args, **kwargs):
        require(
            not self.failed and self.attempts < 16 and self.now() < self.deadline,
...
def evaluate(plan, backend, ledger, derivatives, output, guard):
    ...
    def call(
        ...
        print(
            f"completed {ledger.completed}/<=40 forwards; {derivatives.completed}/<=16 derivatives",
            flush=True,
        )
    ...
    try:
        ...
        for spec in plan["requests"]:
            require(states[spec["prompt_id"]]["row"]["actual_next_token_label"]=="A" and eligibility(states[spec["prompt_id"]]["row"])==spec["sign"],"reverse baseline must be eligible unique A opposed to frozen B request")
        for spec in plan["requests"]:
    ...
    require(ledger.cursor==40 and ledger.attempts==ledger.completed
            and derivatives.attempts==derivatives.completed==sum(r["updates"] for r in requests)
            and ledger.attempts==8+2*derivatives.attempts,"conditional accounting incomplete")
