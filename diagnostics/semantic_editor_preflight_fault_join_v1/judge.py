...
def judge(output):
    ...
    def accounting():
        ...
        require(receipt["cursor"]==receipt["synthetic_forward_attempts"]+len(skips),"attempt+skip cursor")
        require(receipt["synthetic_forward_attempts"]<=180 and receipt["synthetic_derivatives_attempted"]<=48,"absolute safety ceilings")
        ...
        return {"attempts":receipt["synthetic_forward_attempts"],"completed":receipt["synthetic_forward_completed"],"derivatives":receipt["synthetic_derivatives_completed"],"skips":len(skips),"unrun":len(unrun)}
