    ...
    for i, row in enumerate(rows):
        ...
        if "current_cell_id" in row:
            ...
            if row["condition"].startswith("gradient_") or (row["condition"] in ("retention","endpoint","entry") or row["condition"].startswith("oracle_off_")):
                require(
                    difference <= (2e-5 if row["condition"]=="endpoint" and not row.get("retention_endpoint") else EPS)
                    and row["h"] == current["h"]
                ...
                require(
                    all(
                        abs(row[k] - current[k]) <= (2e-5 if row["condition"]=="endpoint" and not row.get("retention_endpoint") else EPS)
                        for k in (
                            "preserve_log_odds",
