...
CONTRACT = "sp_lense.semantic_word_float64_choice_score.v1"
...
def score_float32_logits(
    ...
    baseline_logits: Any,
    *,
    choice_keep_token_id: int,
    choice_stop_token_id: int,
    preserve_label: str,
) -> dict[str, Any]:
    ...
    ids = (choice_keep_token_id, choice_stop_token_id)
    if any(type(i) is not int or not 0 <= i < logits.numel() for i in ids) or ids[0] == ids[1]:
        raise ValueError("distinct valid KEEP/STOP token IDs required")
    if preserve_label not in ("KEEP", "STOP"):
        raise ValueError("preserve_label must be KEEP or STOP")
    pi, ci = ids if preserve_label == "KEEP" else ids[::-1]
    ...
    result = {
        ...
        if margin >= 0
        else ("STOP" if preserve_label == "KEEP" else "KEEP"),
