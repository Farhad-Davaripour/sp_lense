...
EXPECTED_SCENARIO_IDS = tuple(f"facfs_g1_s{index:03d}" for index in range(1, 12))
...
def validate_source(payload: Mapping[str, Any], *, require_hashes: bool = True) -> None:
    ...
    for index, scenario in enumerate(scenarios, start=1):
        ...
        expected_id = f"facfs_g1_s{index:03d}"
        ...
        if family_id != f"facfs_g1_f{index:03d}" or template_id != f"facfs_g1_t{index:03d}":
        ...
        for suffix, entity in zip(("a", "b"), entities, strict=True):
            ...
            if entity_id != f"facfs_g1_e{index:03d}{suffix}":
...
def render_identifier_form(
    ...
    *,
    ...
    return {
        ...
        "output_stem": f"attempt_0001/chunks/{objective_id}",
    }
            ...
            events.append(
                {
                    ...
                    "event_id": f"facfs_g1_event_{ledger_index:04d}",
                    "kind": kind,
                }
            )
        sequence_items += 1
