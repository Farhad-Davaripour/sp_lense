...
HERE=Path(__file__).resolve().parent;ROOT=HERE.parents[2];MIB=1024**2
ATTEMPT='prechoice_diagnostic_relocated_capture_attempt_001'
def require(ok,code):
...
GROUP_CAPS={'logits':16*248320*4,'rows':16*128*1024,'traces':16*16*1024,'steps':0,
...
def write_new(name,value,*,raw=False,critical=False):
    ...
    require(used+len(data)<=64*MIB-(0 if critical else 65536),'OUTPUT_CAP_CLOSEOUT_RESERVE')
