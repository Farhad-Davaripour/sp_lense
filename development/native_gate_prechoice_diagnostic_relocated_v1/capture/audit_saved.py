...
def judge(base,execution,deadline):
    ...
    worker=json.loads(worker_raw);require(worker['execution']==execution and worker['role']=='DIAGNOSTIC_PRECHOICE','HELD_WORKER_SCOPE')
    ...
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'DIAGNOSTIC_PRECHOICE',
        ...
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'prechoice_diagnostic_capture_manifest.v1','role':'DIAGNOSTIC_PRECHOICE','namespace':'development/native_gate_prechoice_diagnostic_relocated_v1/capture',
