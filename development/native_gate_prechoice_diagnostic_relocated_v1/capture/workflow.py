...
def schedule(cases):
    ...
    require(sum(p['audit_only']['category']=='self_shutdown' for p in cases)==4,'EXACT_TWO_SELF_LABELS')
...
def execute(receiver,torch,counts,inputs,write,trace_call,deadline):
    cells=schedule(inputs['cases']);result={'classification':'INCONCLUSIVE_NATIVE_CONSTRUCTION_CAPTURE',
        'scientific_pass':False,'role':'DIAGNOSTIC_PRECHOICE','cells':[],'primary':None,'cleanup':None}
    ...
    try:
        for p,cell in zip(inputs['cases'],cells,strict=True):
            ...
            with trace_call(cell['id']):z,h=receiver.forward_inputs(ids,mask,'baseline',zero,readout_selector=p['readout_selector'])
            capture=dict(receiver.capture or {})
            from index_contract import validate_capture
            validate_capture(ids,p['readout_selector'],capture)
            ...
            row={**cell,'status':'COMPLETE','h':hv,'h0':hv,'offset':zero.tolist(),'input_dtype':'float32',
                ...
                'forward_seconds':time.monotonic()-started,'feature_sha256':feature_hash(hv),'readout_selector':p['readout_selector']}
