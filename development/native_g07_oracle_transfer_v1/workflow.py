"""Six G07 development views, fixed opposite-choice oracle transfer."""
import math,time
from science import need,sha,norm,score,valid,accepted,eligibility,step_recipe,EPS,STEP_CAP,TOTAL_CAP

def schedule(cases):
    from input_reader import schema_cases
    need([(p['case_key'],p['audit_only']['category']) for p in cases]==[(p['case_key'],p['audit_only']['category']) for p in schema_cases()],'EXACT_SIX_G07_CASES')
    ...
    need(len(cells)==30,'FIXED_30_CELLS');return cells
...
def execute(receiver,torch,counts,inputs,write,trace_call,deadline):
    from input_reader import require_oracle
    require_oracle(inputs.get('oracle_authority'))
    cases=inputs['cases'];cells=schedule(cases);cursor=0;rows=[];requests=[];baselines={}
    result={'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT','scientific_pass':False,'cells':[], 'requests':requests,
            'primary':None,'cleanup':None,'gate_validity':'NOT_VALIDATED_ORACLE_APPLICABILITY_CONTROL',
            'learned_gate_success_claimed':False,'oracle_authority':inputs['oracle_authority']}
    ...
    def call(p,cell,delta,baseline=None,current=None,sign=0):
        ...
        if p is not None:
            ...
            applicable=inputs['oracle_authority']['applicability'][p['case_key']]
            row.update(effective_route='ON' if applicable else 'OFF',external_applicable=applicable,applicability_source='EXTERNAL_ORACLE')
        ...
        if p is not None:
            ...
            # Oracle dispatch is fixed by admitted case category, never a learned score.
            need(row['external_applicable'] is inputs['oracle_authority']['applicability'][p['case_key']]
                and row['effective_route']==('ON' if row['external_applicable'] else 'OFF'),'ORACLE_DISPATCH')
    ...
    try:
        ...
        for p in cases:
            ...
            for policy in ('OPPOSE',):
                ...
                if entry[0]['effective_route']=='OFF':
                ...
                sign=1 if baseline[0]['actual_next_token_id']==48964 else -1;wanted=50057 if sign==1 else 48964
                retained=accepted(entry[0],sign,wanted);need(not retained,'OPPOSE_REQUIRES_GENUINE_FLIP');delta=zero.clone();current=entry;path=0.;updates=0
                ...
                request={'case':p['case_key'],'policy':policy,'kind':'retention' if retained else 'flip', 'actual_policy':'P' if sign==1 else 'C', 'updates':updates,
