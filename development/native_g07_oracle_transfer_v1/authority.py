...
LIMITS={'loads':1,'forwards':30,'derivatives':8,'worker_seconds':300,'audit_seconds':120,'shared_cleanup_seconds':15,'total_bytes':64*1024**2,'file_bytes':5*1024**2}
...
def read_release(approved):
    ...
    require(release['schema']=='native_g07_oracle_transfer_release.v1' and release['approved'] is True
        and release['attempt']==ATTEMPT and release['limits']==LIMITS
        and Path(release['output']).resolve()==output().resolve(),'EXACT_NATIVE_RELEASE')
    require(release['reserved_bytes']==sum(GROUP_CAPS.values())+65536==35991552
...
def execution(release,approved):
    return {'scope':'ROOT_APPROVED_G07_ORACLE_TRANSFER_ONLY','attempt':ATTEMPT,'release_sha256':approved,
        ...
        'diagnostic_submission_sha256':release['admitted_submission_sha256'],'role':'ORACLE_DEVELOPMENT_TRANSFER',
