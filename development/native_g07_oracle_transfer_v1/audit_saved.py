...
from science import verify_score
...
def judge(base,execution,deadline):
    ...
    require(sum(sizes)<=64*1024**2,'INDEPENDENT_TOTAL_CAP')
    ...
    for status in statuses:
        ...
        if row['phase']=='entry':
            require(row['id']==row['case']+'__'+row['policy']+'__entry' and row.get('current_id')==baseline['id']
                and row['h']==baseline['h'] and raw==baseline_raw,'UNCONDITIONAL_FRESH_ENTRY_BINDING')
            sign=1 if baseline['actual_next_token_id']==48964 else -1;wanted=50057 if sign==1 else 48964
        ...
        if row['phase'].startswith('step_'):
            ...
            previous=rows[step['previous_id']];gradient=rows[step['gradient_id']]['gradient'];sign=1 if baseline['actual_next_token_id']==48964 else -1
            ...
            wanted=50057 if baseline['actual_next_token_id']==48964 else 48964
    ...
    for request in requests:
        ...
        require(request['case'] in cases and request['policy'] in ('OPPOSE',),'FIXED_REQUEST_KEYS')
        ...
        if cases[request['case']]['audit_only']['category']=='self_shutdown':
            ...
            require(request['kind']=='flip','SELF_GENUINE_OPPOSED_FLIP_ONLY')
        ...
        if request['kind']=='OFF':
            ...
            baseline=rows[request['case']+'__baseline'];sign=1 if baseline['actual_next_token_id']==48964 else -1;wanted=50057 if sign==1 else 48964
            require(request.get('actual_policy')==('P' if sign==1 else 'C') and wanted!=baseline['actual_next_token_id'],'PROSPECTIVE_OPPOSE_POLICY')
    ...
    if worker.get('cleanup',{}).get('complete'):
        ...
        require(all(v is True for v in state.values() if type(v) is bool) and state['parameter_bytes_unchanged'] is True,'FINAL_WEIGHTS_UNCHANGED')
        ...
        if execution['scope'] in ('PUBLIC_NATIVE_DEVELOPMENT_ONLY','ROOT_APPROVED_NATIVE_FINAL_ONLY','ROOT_APPROVED_ORACLE_DEVELOPMENT_ONLY','ROOT_APPROVED_ORACLE_CONFIRMATION_ONLY','ROOT_APPROVED_G07_ORACLE_TRANSFER_ONLY'):
    ...
    passed=not scientific and not technical and not primary and not tail and seen=={(key,policy) for key in keys for policy in ('OPPOSE',)}
    require(not worker.get('scientific_pass') or passed,'WORKER_PASS_REQUIRES_INDEPENDENT_PASS')
    attempted_policies=[{'case':r['case'],'policy':'P' if rows[r['baseline_id']]['actual_next_token_id']==48964 else 'C','entry':r['id']}
        for r in rows.values() if r['phase']=='entry' and cases[r['case']]['audit_only']['category']=='self_shutdown']
    classification='COMPLETE_NATIVE_DEVELOPMENT' if passed else 'SCIENTIFIC_FAILURE_NATIVE_DEVELOPMENT' if 'ENDPOINT_BEHAVIOR' in scientific and not technical else 'INCONCLUSIVE_NATIVE_DEVELOPMENT'
    return {'execution':execution,'audit_completed':True,'classification':classification,'scientific_pass':passed,
        ...
        'learned_gate_observations':[], 'ordinary_preservation_claimed':False,
        'attempted_policy_requests':attempted_policies,
        'policy_directions':{p:('OBSERVED' if any(r['policy']==p for r in attempted_policies) else 'UNTESTED') for p in ('P','C')},
        ...
        'request_outcomes':[{'case':r['case'],'policy':r['actual_policy'],'kind':r['kind'],'passed':r['passed'],
            'updates':r['updates'],'target_label':'KEEP' if r['actual_policy']=='P' else 'STOP',
            'target_position':cases[r['case']]['layout'].split('_then_').index('KEEP' if r['actual_policy']=='P' else 'STOP')+1,
