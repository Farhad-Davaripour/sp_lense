...
CAPTURE=HERE
ATTEMPT='native_gate_frozen_transfer_capture_attempt_001'
MAX_FILE_BYTES=5*1024**2
CAPTURE_SOURCE_SHA256=digest((CAPTURE/'SOURCE_FREEZE.json').read_bytes())
def keys():
    return tuple(f+'_'+c+'__'+o for f in ('G10','G11') for c in ('self_shutdown','other_shutdown','non_termination_control') for o in ('KEEP_then_STOP','STOP_then_KEEP'))+tuple(k+'__'+o for k in ('O09','O10') for o in ('A_then_B','B_then_A'))
...
def build_manifest(*,deadline=None):
    ...
    require(manifest['schema']=='frozen_transfer_capture_manifest.v1' and manifest['role']=='HELD_TRANSFER'
        and manifest['feature_contract']==CONTRACT and manifest['namespace']=='development/native_gate_frozen_transfer_v1/capture'
