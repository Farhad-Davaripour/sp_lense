...
CAP='development/native_gate_frozen_transfer_v1/capture';PREP='development/native_gate_frozen_transfer_v1/preparation';MAIN='development/native_gate_coverage_increment_v1'
...
def main():
    ...
    literal(cap/'score.py','SUBMISSION_SHA256',repr(sub))
    substitutions.append({'source':str(cap/'score.py'),'constant':'MAIN','value':str(ROOT/MAIN)})
    substitutions.append({'source':str(cap/'score.py'),'constant':'SUBMISSION_SHA256','from':'1dc9c16498d64a676238232b2b7b37462c53e6af742e8018835f9d00a1f23d61','to':sub,'all_function_class_ASTs_unchanged':True})
    ...
    cohort=lock['cohort'];sub=sha(jb(cohort));put(mirror/'development/native_gate_frozen_transfer_v1/SUBMISSION.json',cohort)
    ...
    with isolated((fit,cap,prep)):
        ...
        release={'schema':'frozen_transfer_capture_release.v1','approved':True,'attempt':authority.ATTEMPT,'limits':authority.LIMITS,
            'output':str(authority.output()),'reserved_bytes':19628032,'source_freeze_sha256':cs,
        ...
        assert len(input_reader.read_bundle(bundle,authority.read_release(ex['release_sha256']))['cases'])==16
        ...
        def publish_capture(change=None,worker_change=None):
            ...
            manifest={'schema':'frozen_transfer_capture_manifest.v1','role':'HELD_TRANSFER','namespace':CAP,'attempt':native.ATTEMPT,'execution':ex,'feature_contract':gate.CONTRACT,'selection':selected}
            ...
            binding={'schema':'frozen_transfer_capture_binding.v1','approved':True,'role':'HELD_TRANSFER','namespace':CAP,'submission_sha256':sub,'source_freeze_sha256':cs,'manifest_sha256':sha(gate.canonical(manifest))}
        ...
        try:
            ...
            released={'schema':'frozen_transfer_score_release.v1','approved':True,'model_permission':False,'tokenizer_permission':False,'fit_permission':False,
...
def audit(event,args):
 if event=='open' and args and isinstance(args[0],str):
  ...
  if p==Path(os.environ['REVIEW_REAL_CAPTURE']).parent/'SUBMISSION.json' or p.is_relative_to(Path(os.environ['REVIEW_REAL_CAPTURE'])/'real_evidence'):raise RuntimeError('ACTUAL_HELD_EVIDENCE_FORBIDDEN')
...
"""
            ...
            assert not (cap/'scoring_attempt_001').exists()
            positive,observed=invoke('unmocked_full_score_entry',args,0,16)
            attempt=cap/'scoring_attempt_001';saved=get(attempt/'RESULT.json');admission=get(attempt/'ADMISSION.json')
            assert saved['completed'] is True and saved['score_calls']==16 and saved['fits']==0
            ...
            with score.environment() as (c,auth,guard):
                ...
                replay=score.replay(rows,saved,models,time.monotonic()+10);assert replay['recomputed_calls']==16
            checks.append({'name':'saved_stdlib_replay','status':'PASS','recomputed_calls':16,'payload_bytes':payload,'result_sha256':sha((attempt/'RESULT.json').read_bytes())})
    ...
    report={'status':'PASS','checks':checks,'production_function_mocks':0,'actual_coordinate_reads':0,'actual_old52_metadata_authentication':False,'actual_frozen_training_metadata_only':True,
        'new16':'ARTIFICIAL_ONLY','substitutions':substitutions,'source_function_and_class_ASTs_unchanged':True,'actual_held_content_reads':0,'real_tokenizer_calls':0,'model_calls':0,'fits':0,'elapsed_seconds':time.monotonic()-start,
...
def storage_ceiling_check():
    """No allocation: synthetic filesystem reports 65MiB in legal-size files."""
    from types import SimpleNamespace
    import audit_saved
    class Entry:
        def is_file(self):return True
        def stat(self):return SimpleNamespace(st_size=1024**2)
    class Base:
        def rglob(self,pattern):return [Entry() for _ in range(65)]
    try:audit_saved.judge(Base(),{},time.monotonic()+10)
    except ValueError as error:assert str(error)=='CAPTURE_STORAGE',str(error)
    else:raise AssertionError('AUDITOR_ACCEPTED_OVER64MIB')
    print(json.dumps({'storage_ceiling':'PASS','reported_bytes':65*1024**2,'allocated_evidence_bytes':0}))
