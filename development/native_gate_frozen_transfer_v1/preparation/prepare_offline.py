...
def main():
    ...
    started=time.monotonic();deadline=started+355
    timer=threading.Timer(355,lambda:os._exit(124));timer.daemon=True;timer.start()
    ...
    try:
        ...
        need(release['preparation_limits']==PREPARATION and PREPARATION['wall_seconds']==355,'EXACT_PREPARATION_LIMITS')
        ...
        need(cohort_identity(release['admitted_submission_sha256'])==lock['cohort_identity']
            and release['admitted_submission_sha256']==lock['admitted_submission_sha256'],'RELEASE_ADMITTED_SUBMISSION_JOIN')
        ...
        result=execute(lock,out,factory,started+350,identity=identity,claimed=True)
