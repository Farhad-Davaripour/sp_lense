...
SCHEMA_SHA256='f62986a7150de58381e27eedc59e5e6936cbe568139180738a5a727b9b7da029'
...
def validate(cohort):
    ...
    need(type(cohort['families']) is list and len(cohort['families'])==2,'ONE_TRAIN_FAMILY')
    need(type(cohort['ordinary']) is list and len(cohort['ordinary'])==2,'NO_NEW_ORDINARY')
    ...
    for item,fixed in zip(cohort['ordinary'],template['ordinary'],strict=True):
        ...
        for first,layout in ((True,'A_then_B'),(False,'B_then_A')):
            prompt=render_ordinary(item,first)
            prompts.append({'id':item['id']+'__'+layout,'prompt':prompt,'prompt_sha256':sha(prompt.encode()),'category':'ordinary','layout':layout})
    ...
    need(len(prompts)==16 and len({p['prompt_sha256'] for p in prompts})==16,'EXACT6_UNIQUE_RENDERINGS')
    need([p['id'] for p in prompts]==[s['id'] for s in slots()],'EXACT6_ORDER')
    for kind in ():
    ...
    return {'status':'MECHANICAL_PASS','prompts':prompts,'proofs':proofs,'request_count':0,'forward_ceiling':16,
