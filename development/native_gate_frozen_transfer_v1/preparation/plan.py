...
PREPARATION={'wall_seconds':355,'external_wait_ms':350000,'external_cleanup_ms':5000,
    'operation_limit':209,'expected_operations':209,'total_bytes':32*1024**2,
...
COHORT_IDENTITY={'arm':'frozen_transfer.HELD_TRANSFER.v1',
    'cohort_namespace':'development/native_gate_frozen_transfer_v1'}
...
def slots():
    ...
    result += [{'id':key+'__'+layout,'category':'ordinary','family_id':None,'layout':layout,
        'type':kind,'labels':{'A':32,'B':33}} for key,kind in ORDINARY for layout in ('A_then_B','B_then_A')]
    return result
def operations():
