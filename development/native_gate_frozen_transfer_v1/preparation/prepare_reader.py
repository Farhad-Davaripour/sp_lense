...
def execution_binding(source_sha):
    return {'namespace':'development/native_gate_frozen_transfer_v1/capture','source_freeze_sha256':source_sha,
...
def validate(data,lock,records,result,journal,source_sha,raw_lock_sha,*,synthetic=False):
    ...
    require(len(journal)==418,'EXACT79_OPERATION_PAIRS')
    ...
    planned=slots();require(len(data['cases'])==16 and len(records)==16 and len(lock['rendered_prompts'])==16,'EXACT12_INPUTS')
    gold={p['id']:p['proof']['gold_label'] for p in lock['cohort']['ordinary']}
    require(set(gold)=={s['id'].split('__')[0] for s in planned if s['category']=='ordinary'},'EXACT_ORDINARY_GOLD_KEYS')
    lengths={}
    for p,s,text,r in zip(data['cases'],planned,lock['rendered_prompts'],records,strict=True):
        ...
        if s['category']=='ordinary':
            require(gold[key.split('__')[0]] in ('A','B'),'ORDINARY_GOLD_AB');audit['correct_token_id']=s['labels'][gold[key.split('__')[0]]]
