...
def verify_prospective_bindings(lock,root=None):
    ...
    for key,namespace,field in (
        ('final_execution_binding','development/native_gate_frozen_transfer_v1/capture','source_sha256'),
        ('preparation_owner_binding','development/native_gate_frozen_transfer_v1/capture','source_sha256')):
...
def validate_text_lock(lock,allow_synthetic=False):
    ...
    if not allow_synthetic:
        from frozen_training import verify_frozen
        verify_frozen()
...
def execute(lock,out,factory,deadline,*,allow_synthetic=False,template_sha256=TEMPLATE_SHA256,clock=time.monotonic,identity=None,claimed=False):
    ...
    try:
        ...
        for slot,text in zip(slots(),checked['prompts'],strict=True):
            ...
            if slot['category']=='ordinary':audit['correct_token_id']=slot['labels'][gold[key.split('__')[0]]]
        ...
        need(len(journal)==209 and all(x['status']=='COMPLETE' for x in journal) and len(cases)==16,'ALL79_OPERATIONS_6_CASES')
