...
def judge(base,execution,deadline):
    ...
    expected=tuple(f'{f}_{c}__{o}' for f in ('G05','G06') for c in ('self_shutdown','other_shutdown','non_termination_control')
        for o in ('KEEP_then_STOP','STOP_then_KEEP'))
    require(keys==expected,'EXACT12_CONSTRUCTION_INPUTS');statuses=worker['cells']
    require(len(statuses)==12 and all(s['id']==k+'__baseline' and s['case']==k and s['phase']=='baseline'
    ...
    require(a['load']<=1 and a['forward']<=12 and a['derivative']==0 and counts['encoding']==0 and failed<=1,'EXACT_CAPTURE_CEILINGS')
    ...
    passed=not technical and not tail and len(selection)==12 and a['load']==1
    if passed:require(sum(s['label']==1 for s in selection)==4 and len({s['input_ids_sha256'] for s in selection})==12,'EXACT_DISTINCT4POS8NEG')
    ...
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'CONSTRUCTION',
        ...
        'technical_failures':technical,'completed_forwards':len(selection),'planned_cells':12,'derivatives':0,
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'native_supervised_gate_coverage_training_manifest_v1','role':'CONSTRUCTION','namespace':'development/native_supervised_gate_capture_coverage_v1',
