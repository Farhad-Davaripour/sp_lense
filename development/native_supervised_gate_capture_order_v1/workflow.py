...
def schedule(cases):
    require(tuple(p['case_key'] for p in cases)==keys(),'EXACT8_CONSTRUCTION_ORDER')
    require(sum(p['audit_only']['category']=='self_shutdown' for p in cases)==0,'ZERO_SELF_LABELS')
...
def execute(receiver,torch,counts,inputs,write,trace_call,deadline):
    cells=schedule(inputs['cases']);result={'classification':'INCONCLUSIVE_NATIVE_CONSTRUCTION_CAPTURE',
        'scientific_pass':False,'role':'CONSTRUCTION','cells':[],'primary':None,'cleanup':None}
