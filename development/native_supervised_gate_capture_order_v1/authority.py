...
LIMITS={'loads':1,'forwards':8,'derivatives':0,'worker_seconds':300,'audit_seconds':120,'shared_cleanup_seconds':15,'total_bytes':64*1024**2,'file_bytes':5*1024**2}
...
def read_release(approved):
    ...
    require(release['schema']=='native_supervised_gate_capture_order_release.v1' and release['approved'] is True
        and release['attempt']==ATTEMPT and release['limits']==LIMITS
        and Path(release['output']).resolve()==output().resolve(),'EXACT_NATIVE_RELEASE')
    require(release['reserved_bytes']==sum(GROUP_CAPS.values())+65536==10502144
