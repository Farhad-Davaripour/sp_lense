...
CAPTURE=HERE
ATTEMPT='native_supervised_gate_capture_order_attempt_001'
MAX_FILE_BYTES=5*1024**2
CAPTURE_SOURCE_SHA256=digest((CAPTURE/'SOURCE_FREEZE.json').read_bytes())
def keys():
    return tuple('O0'+str(i)+'__B_then_A' for i in range(1,9))
...
def build_manifest(*,deadline=None):
    ...
    require(manifest['schema']=='native_supervised_gate_order_training_manifest_v1' and manifest['role']=='CONSTRUCTION'
        and manifest['feature_contract']==CONTRACT and manifest['namespace']=='development/native_supervised_gate_capture_order_v1'
        and manifest['attempt']==ATTEMPT and manifest['execution']==execution,'CONSTRUCTION_MANIFEST_SCOPE')
    selection=manifest['selection'];require(tuple(x['case'] for x in selection)==keys(),'EXACT8_FRESH_SELECTION')
    expected=tuple(1 if '_self_shutdown__' in key else -1 for key in keys())
    require(tuple(x['label'] for x in selection)==expected and expected.count(1)==0,'EXACT_ZERO_POSITIVE_EIGHT_NEGATIVE')
...
def extract_features(manifest,*,deadline):
    ...
    require(len(rows)==8 and len(set(x['input_ids_sha256'] for x in manifest['selection']))==8,'NO_DUPLICATE_TRAINING_INPUT')
