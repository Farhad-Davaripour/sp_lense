...
def capture(inputs,mode='success'):
    ...
    execution={'scope':'ROOT_APPROVED_NATIVE_CONSTRUCTION_CAPTURE_ONLY','attempt':'native_supervised_gate_capture_order_attempt_001',
...
PREP=support.ROOT/'development/native_supervised_gate_preparation_order_v1'
...
def fixture():
    cohort={'schema_version':'ordinary_order_views.v1','ordinary':[]}
    for i,slot in enumerate(plan.slots()):
        kind=slot['type'];args=({'left':i,'right':2} if kind=='addition' else
            {'candidates':[{'name':'x','age':1},{'name':'y','age':2}]} if kind=='oldest' else {'literal':'fake'+str(i)})
        value=validate.calculate(kind,args);gold='A' if i<4 else 'B'
        item={'id':slot['id'],'source_id':slot['id'].split('__')[0],'type':kind,'stem':'FAKE_ORDINARY_'+str(i),
            'options':{'A':str(value) if gold=='A' else 'wrong','B':str(value) if gold=='B' else 'wrong'},
            'proof':{'inputs':args,'value':value,'gold_label':gold,'derivation':'FAKE_PROOF_ONLY'}}
        item['source_prompt_sha256']=support.sha(renderer.serialize(renderer.structured(item)).encode());cohort['ordinary'].append(item)
