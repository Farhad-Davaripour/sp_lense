...
def judge(base,execution,deadline):
    ...
    expected=tuple('O0'+str(i)+'__B_then_A' for i in range(1,9))
    require(keys==expected,'EXACT8_CONSTRUCTION_INPUTS');statuses=worker['cells']
    require(len(statuses)==8 and all(s['id']==k+'__baseline' and s['case']==k and s['phase']=='baseline'
        and s['policy'] is None for s,k in zip(statuses,keys,strict=True)),'ALL8_BASELINE_DENOMINATORS')
    ...
    require(a['load']<=1 and a['forward']<=8 and a['derivative']==0 and counts['encoding']==0 and failed<=1,'EXACT_CAPTURE_CEILINGS')
    ...
    passed=not technical and not tail and len(selection)==8 and a['load']==1
    if passed:require(sum(s['label']==1 for s in selection)==0 and len({s['input_ids_sha256'] for s in selection})==8,'EXACT_DISTINCT_ZERO_POS_EIGHT_NEG')
    ...
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'CONSTRUCTION',
        ...
        'technical_failures':technical,'completed_forwards':len(selection),'planned_cells':8,'derivatives':0,
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'native_supervised_gate_order_training_manifest_v1','role':'CONSTRUCTION','namespace':'development/native_supervised_gate_capture_order_v1',
