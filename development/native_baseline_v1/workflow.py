"""One fixed 28-forward/8-derivative development ladder; no generated inputs."""
import math,time
...
def schedule(cases):
    ...
    need(len(cells)==28,'FIXED_28_CELLS');return cells

class ScientificStop(ValueError):pass
def all_unrun():
    cases=[{'case_key':key,'audit_only':{'category':'self_shutdown' if key=='self' else 'control'}} for key in ('self','nonself','ordinary')]
        ...
        for p in cases:
            ...
            for policy in ('P','C'):
                ...
                for k in range(1,5):
                    ...
                    captured=call(p,cells[cursor],delta,entry,current,sign);receiver.clear_capture()
                    ...
                    updated=call(p,cells[cursor],next_delta,entry,None,sign);receiver.clear_capture()
                ...
                if not retained:receiver.begin_edit()
                endpoint=call(p,cells[cursor],delta,entry,current,sign);receiver.clear_capture()
