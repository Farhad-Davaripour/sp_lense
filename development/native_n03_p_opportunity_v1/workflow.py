"""Six baselines, then P only iff a natural STOP opportunity exists; no screening."""
import math,time
...
def execute(receiver,torch,counts,inputs,write,trace_call,deadline):
    ...
    result={'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT','scientific_pass':False,'cells':[], 'requests':requests,
            ...
            'learned_gate_success_claimed':False,'oracle_authority':inputs['oracle_authority'],'opportunity_decision':None}
    zero=torch.zeros(1024,dtype=torch.float32);current_cell=None;no_opportunity=False
    ...
    def call(p,cell,delta,baseline=None,current=None,sign=0):
        ...
        if p is not None:
            ...
            # The learned observation is retained, not overwritten or certified.
    ...
    try:
        ...
        self_keys=[p['case_key'] for p in cases if p['audit_only']['category']=='self_shutdown']
        need(len(self_keys)==2 and cursor==6,'ALL6_BASELINES_BEFORE_OPPORTUNITY')
        natural_stop=[k for k in self_keys if baselines[k][0]['actual_next_token_id']==48964]
        result['opportunity_decision']={'self_cases':self_keys,'natural_stop_cases':natural_stop,
            'branch':'P_REQUESTS' if natural_stop else 'NO_NATURAL_P_OPPORTUNITY'}
        if not natural_stop:
            no_opportunity=True;result['classification']='NO_NATURAL_P_OPPORTUNITY'
            return result
        for p in cases:
        ...
        result['cells'] += [{**c,'status':'UNRUN',**({'reason':'NO_NATURAL_P_OPPORTUNITY'} if no_opportunity else {})} for c in cells[cursor:]]
