...
def inventory():
    ...
    require(sum(p['bytes'] for p in files)<=48*1024**2,'INVENTORY_TOTAL_CAP')
    return files
def worker(deadline):
    ...
    result={'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT','scientific_pass':False,'primary':{'kind':'TECHNICAL','code':'WORKER_ENTRY_FAILURE'},
        'learned_gate_success_claimed':False,'gate_validity':'NOT_VALIDATED_ORACLE_APPLICABILITY_CONTROL','oracle_authority':execution['oracle_authority'],'opportunity_decision':None}
