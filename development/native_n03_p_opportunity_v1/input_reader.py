"""Exact fixed N03 family and prospective natural-P opportunity rule; no encoding."""
import json,os
from support import HERE,ROOT,require,sha,json_bytes
CASE_KEYS=tuple('N03_'+category+'__'+layout for category in ('self_shutdown','other_shutdown','non_termination_control')
...
SCOPE='development/native_oracle_family_panel_v1/ROOT_NEXT_P_OPPORTUNITY_SCOPE.md'
def oracle():
...
def source_cases():
    ...
    require(tuple(p['case_key'] for p in matches)==CASE_KEYS,'EXACT6_ORIGINAL_ORDER');return matches
def schema_cases():return [{'case_key':p['case_key'],'audit_only':{'category':p['audit_only']['category']}} for p in source_cases()]
def prepared():
    return {'schema':'native_n03_p_opportunity_inputs.v1','scope':'OUTCOME_INFORMED_DEVELOPMENT_ONLY',
        'source_path':SOURCE,'source_sha256':SOURCE_SHA,'oracle_authority':oracle(),'cases':source_cases(),
        'encoding_calls':0,'numeric_state_reused':False,'input_token_ceiling':320,'requested_policies':['P'],
        'opportunity_rule':'AFTER_ALL6_BASELINES_REQUEST_P_IFF_ANY_ELIGIBLE_SELF_STOP'}
def validate(value):
    require_oracle(value.get('oracle_authority'))
    require(value==prepared(),'EXACT6_FIXED_INPUTS_AND_P_ONLY_RULE')
