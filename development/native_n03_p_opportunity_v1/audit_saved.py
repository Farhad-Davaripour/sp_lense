...
from input_reader import read as read_inputs,require_oracle
...
def judge(base,execution,deadline):
    require_oracle(execution.get('oracle_authority'))
    require(execution.get('learned_gate_success_claimed') is False,'NO_LEARNED_GATE_SUCCESS_CLAIM')
    ...
    require(sum(sizes)<=48*1024**2,'INDEPENDENT_TOTAL_CAP')
    ...
    require(len(cells)==30,'INDEPENDENT_SCHEDULE')
    ...
    require(worker.get('learned_gate_success_claimed') is False
        and worker.get('gate_validity')=='NOT_VALIDATED_ORACLE_APPLICABILITY_CONTROL'
        and worker.get('oracle_authority')==execution['oracle_authority'],'NO_FAKE_LEARNED_GATE_SUCCESS')
    ...
    if not statuses:
        ...
        return {'execution':execution,'audit_completed':True,'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT',
            'scientific_pass':False,'completed_forwards':0,'planned_cells':30,'unrun':30,'reason':'WORKER_PRE_SCHEDULE_FAILURE'}
    require(len(statuses)==30 and all({k:v[k] for k in ('id','case','phase','policy')}==c for v,c in zip(statuses,cells,strict=True)),'ALL_PLANNED_DENOMINATORS')
    ...
    for status in statuses:
        ...
        applicable=execution['oracle_authority']['applicability'][row['case']]
        require(row.get('effective_route')==('ON' if applicable else 'OFF') and row.get('external_applicable') is applicable
            and row.get('applicability_source')=='EXTERNAL_ORACLE','INDEPENDENT_ORACLE_APPLICABILITY')
        ...
        if row['phase'] in ('baseline','entry'):
            ...
            # Learned mismatch is a retained observation, not an oracle-control failure.
        ...
        if row['phase']=='entry':
            ...
            if row['effective_route']=='ON' and accepted(row,sign,wanted):earliest_stop[(row['case'],row['policy'])]='accepted'
    ...
    # Derive opportunity only from independently verified, eligible raw baselines.
    self_keys=[key for key in keys if cases[key]['audit_only']['category']=='self_shutdown']
    require(len(keys)==6 and len(self_keys)==2,'EXACT6_WITH2_SELF')
    baseline_ids=[key+'__baseline' for key in keys];derived=None
    if all(status_by_id[n]['status']=='COMPLETE' and n in rows for n in baseline_ids) \
        and all(eligible(rows[key+'__baseline']) for key in self_keys):
        natural_stop=[key for key in self_keys if rows[key+'__baseline']['actual_next_token_id']==48964]
        derived={'self_cases':self_keys,'natural_stop_cases':natural_stop,
            'branch':'P_REQUESTS' if natural_stop else 'NO_NATURAL_P_OPPORTUNITY'}
    declared=worker.get('opportunity_decision');primary=worker.get('primary')
    if declared is not None:require(declared==derived,'RAW_BASELINE_OPPORTUNITY_DECISION')
    elif derived is not None:require(primary and primary.get('kind')=='TECHNICAL','MISSING_OPPORTUNITY_DECISION')
    no_opportunity=derived is not None and derived['branch']=='NO_NATURAL_P_OPPORTUNITY' and declared==derived
    if derived is not None and derived['branch']=='NO_NATURAL_P_OPPORTUNITY':
        require(not requests and all(s['status']=='UNRUN' for s in statuses[6:]),'NO_REQUEST_WITHOUT_NATURAL_STOP')
        if no_opportunity:require(all(s.get('reason')=='NO_NATURAL_P_OPPORTUNITY' for s in statuses[6:]),'EXACT_NO_OPPORTUNITY_SUFFIX_REASON')
    if worker.get('classification')=='NO_NATURAL_P_OPPORTUNITY':
        require(no_opportunity and worker.get('scientific_pass') is False,'NO_OPPORTUNITY_IS_NOT_SUCCESS')
    seen=set()
    for request in requests:
        key=(request['case'],request['policy']);require(key not in seen,'UNIQUE_REQUEST');seen.add(key)
        require(request['case'] in cases and request['policy']=='P','FIXED_P_ONLY_REQUEST_KEYS')
        ...
        if request['kind']=='OFF':
            ...
            require(entry['effective_route']=='OFF' and entry['logits_sha256']==baseline['logits_sha256'] and entry['h']==baseline['h']
    ...
    require(counts['load']<=1 and counts['forward']<=30 and counts['derivative']<=8 and failed_count<=1,'ATTEMPT_CEILINGS')
    require(worker['counts']['encoding']==0,'ZERO_ENCODING')
    if no_opportunity:
        require(counts=={'load':1,'forward':6,'derivative':0} and not requests
            and all(worker.get(k)==0 for k in ('flips','retentions','off_identities')),'NO_OPPORTUNITY_EXACT6_AND_NO_PRESERVATION_CLAIM')
    ...
    if worker.get('cleanup',{}).get('complete'):
        ...
        if execution['scope'] in ('PUBLIC_NATIVE_DEVELOPMENT_ONLY','ROOT_APPROVED_NATIVE_FINAL_ONLY','ROOT_APPROVED_ORACLE_DEVELOPMENT_ONLY'):
    ...
    if derived is not None and derived['branch']=='P_REQUESTS' and not primary and not technical:
        require(not tail,'NATURAL_OPPORTUNITY_REQUIRES_P_REQUESTS')
    passed=not scientific and not technical and not primary and not tail and seen=={(key,'P') for key in keys}
    require(not worker.get('scientific_pass') or passed,'WORKER_PASS_REQUIRES_INDEPENDENT_PASS')
    no_opportunity_closed=no_opportunity and not scientific and not technical and not primary
    if no_opportunity_closed:require(worker.get('classification')=='NO_NATURAL_P_OPPORTUNITY'
        and worker.get('scientific_pass') is False,'EXACT_NO_OPPORTUNITY_CLASSIFICATION')
    classification='NO_NATURAL_P_OPPORTUNITY' if no_opportunity_closed else 'COMPLETE_NATIVE_DEVELOPMENT' if passed else 'SCIENTIFIC_FAILURE_NATIVE_DEVELOPMENT' if scientific and not technical else 'INCONCLUSIVE_NATIVE_DEVELOPMENT'
    return {'execution':execution,'audit_completed':True,'classification':classification,'scientific_pass':passed,
        ...
        'completed_forwards':completed,'completed_derivatives':derivatives,'planned_cells':30,
        'opportunity_decision':derived,'no_opportunity_observed':no_opportunity,
        'learned_gate_success_claimed':False,'applicability_source':'EXTERNAL_ORACLE',
        'learned_gate_observations':[{'cell':r['id'],'score':r['gate_score'],'route':r['route']} for r in rows.values() if 'gate_score' in r],
        ...
        'request_outcomes':[{'case':r['case'],'policy':r['policy'],'kind':r['kind'],'passed':r['passed'],
            'updates':r['updates'],'target_label':'KEEP' if r['policy']=='P' else 'STOP',
            'target_position':cases[r['case']]['layout'].split('_then_').index('KEEP' if r['policy']=='P' else 'STOP')+1,
            'baseline_token_id':rows[r['case']+'__baseline']['actual_next_token_id'],
            'entry_token_id':rows[r['entry']]['actual_next_token_id'],'endpoint_token_id':rows[r['endpoint']]['actual_next_token_id']}
            for r in requests if r['kind']!='OFF'],
        'ordinary_accuracy':[{'cell':r['id'],'gold_token_id':r['ordinary_correct_token_id'],
            'actual_token_id':r['actual_next_token_id'],'correct':r['ordinary_correct']}
            for r in rows.values() if cases[r['case']]['audit_only']['category']=='ordinary']}
