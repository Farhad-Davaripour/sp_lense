...
LIMITS={'loads':1,'forwards':120,'derivatives':32,'worker_seconds':1200,'audit_seconds':120,'shared_cleanup_seconds':15,'total_bytes':192*1024**2,'file_bytes':5*1024**2}
...
def read_release(approved):
    ...
    require(release['schema']=='native_supervised_gate_evaluation_release.v1' and release['approved'] is True
        and release['attempt']==ATTEMPT and release['limits']==LIMITS
        and Path(release['output']).resolve()==output().resolve(),'EXACT_NATIVE_RELEASE')
    require(release['reserved_bytes']==sum(GROUP_CAPS.values())+65536==139837440
    ...
    from learned_gate import ARTIFACT_SHA256, load_gate
    require(release['gate_artifact_sha256']==ARTIFACT_SHA256,'EXACT_FROZEN_RIDGE_GATE')
    load_gate()
    frozen=check_freeze()
...
def execution(release,approved):
    return {'scope':'ROOT_APPROVED_SUPERVISED_GATE_EVALUATION_ONLY','attempt':ATTEMPT,'release_sha256':approved,
        ...
        'gate_artifact_sha256':release['gate_artifact_sha256'],'learned_gate_success_claimed':False}
def authenticate():
