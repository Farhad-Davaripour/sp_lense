...
from learned_gate import load_gate,independent_score,ARTIFACT_SHA256
...
def judge(base,execution,deadline):
    ...
    require(sum(sizes)<=192*1024**2,'INDEPENDENT_TOTAL_CAP')
    ...
    require(len(cells)==120,'INDEPENDENT_SCHEDULE')
    ...
    if not statuses:
        ...
        return {'execution':execution,'audit_completed':True,'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT',
            'scientific_pass':False,'completed_forwards':0,'planned_cells':120,'unrun':120,'reason':'WORKER_PRE_SCHEDULE_FAILURE'}
    require(len(statuses)==120 and all({k:v[k] for k in ('id','case','phase','policy')}==c for v,c in zip(statuses,cells,strict=True)),'ALL_PLANNED_DENOMINATORS')
    tail=False;failed_count=0;rows={};scientific=[];technical=[]
    status_by_id={v['id']:v for v in statuses};earliest_stop={}
    frozen_gate=load_gate();census_outcomes=[];confusion={'TP':0,'TN':0,'FP':0,'FN':0}
    def logits(row):
    ...
    for status in statuses:
        ...
        if row['phase'] in ('baseline','entry'):
            ...
            gs=independent_score(frozen_gate,row['h']);route='ON' if gs>0 else 'OFF'
            require(abs(gs-row['gate_score'])<=1e-12 and row['route']==route,'INDEPENDENT_GATE')
            if row['phase']=='baseline':
                expected_route=cases[row['case']]['audit_only']['expected_route']
                bucket=('TP' if route=='ON' else 'FN') if expected_route=='ON' else ('FP' if route=='ON' else 'TN')
                confusion[bucket]+=1
                census_outcomes.append({'case':row['case'],'score':gs,'route':route,'expected_route':expected_route,'correct':route==expected_route})
    ...
    census_complete=len(census_outcomes)==16
    census_passed=census_complete and confusion=={'TP':4,'TN':12,'FP':0,'FN':0}
    expected_census={'complete':census_complete,'outcomes':census_outcomes,'confusion':confusion}
    if census_complete:expected_census['passed']=census_passed
    require(worker.get('census')==expected_census,'INDEPENDENT_COMPLETE_CENSUS')
    require(worker.get('gate_validity')==('FRESH_CENSUS_PASS' if census_passed else 'FRESH_CENSUS_FAILURE' if census_complete else 'FRESH_CENSUS_PENDING'),'INDEPENDENT_GATE_VALIDITY')
    if census_complete and not census_passed:scientific.append('GATE_CENSUS_FAILURE')
    eligible_all=census_passed and not any(x=='FINITE_SELF_ELIGIBILITY' for x in scientific)
    if not eligible_all:
        require(all(s['status']=='UNRUN' for s in statuses[16:]) and not worker.get('requests')
            and worker['counts']['attempts']['derivative']==0,'NO_POLICY_BEFORE_CENSUS_AND_ELIGIBILITY')
    requests=worker.get('requests',[])
    request_order=[(key,policy) for key in keys for policy in ('P','C')]
    require([(r['case'],r['policy']) for r in requests]==request_order[:len(requests)],'FIRST_FAILURE_REQUEST_PREFIX')
    seen=set()
    for request in requests:
        ...
        if request['kind']=='OFF':
            ...
            require(entry['route']=='OFF' and entry['logits_sha256']==baseline['logits_sha256'] and entry['h']==baseline['h']
                and request['returned']==entry['id'] and request['before']==request['after'] and request['additional_forwards']==0,'OFF_IDENTITY')
        else:
            ...
            if not passed:
                scientific.append('ENDPOINT_BEHAVIOR')
                index=next(i for i,s in enumerate(statuses) if s['id']==endpoint['id'])
                require(request is requests[-1] and all(s['status']=='UNRUN' for s in statuses[index+1:]),'FIRST_ENDPOINT_FAILURE_STOPS')
    ...
    require(counts['load']<=1 and counts['forward']<=120 and counts['derivative']<=32 and failed_count<=1,'ATTEMPT_CEILINGS')
    require(worker['counts']['encoding']==0,'ZERO_EXECUTION_ENCODING')
    ...
    if worker.get('cleanup',{}).get('complete'):
        ...
        require(all(v is True for v in state.values() if type(v) is bool) and state['parameter_bytes_unchanged'] is True
            ...
            and gate['parameter_sha256']==ARTIFACT_SHA256,'FINAL_WEIGHTS_GATE')
        ...
        if execution['scope'] in ('PUBLIC_NATIVE_DEVELOPMENT_ONLY','ROOT_APPROVED_NATIVE_FINAL_ONLY','ROOT_APPROVED_SUPERVISED_GATE_EVALUATION_ONLY'):
    ...
    require(worker.get('learned_gate_success_claimed') is passed,'INDEPENDENT_SUCCESS_CLAIM')
    ...
    return {'execution':execution,'audit_completed':True,'classification':classification,'scientific_pass':passed,
        ...
        'completed_forwards':completed,'completed_derivatives':derivatives,'planned_cells':120,
        ...
        'census':expected_census,'learned_gate_success_claimed':passed,
        'opportunities':opportunities(cases,requests),
        'off_identity_scope':'Own fresh unedited entry returned by construction; no active-editor collateral claim',
        'ordinary_accuracy':[{'cell':r['id'],'policy':r['policy'],'gold_token_id':r['ordinary_correct_token_id'],
            'actual_token_id':r['actual_next_token_id'],'correct':r['ordinary_correct']} for r in rows.values() if cases[r['case']]['audit_only']['category']=='ordinary']}


def opportunities(cases,requests):
    result=[]
    for policy in ('P','C'):
        target='KEEP' if policy=='P' else 'STOP'
        for position in (1,2):
            selected=[r for r in requests if r['kind']!='OFF' and r['policy']==policy
                and cases[r['case']]['layout'].split('_then_').index(target)+1==position]
            flips=[r for r in selected if r['kind']=='flip'];retentions=[r for r in selected if r['kind']=='retention']
            result.append({'policy':policy,'target_position':position,'flips':len(flips),
                'successful_flips':sum(r['passed'] for r in flips),'flip_status':'TESTED' if flips else 'UNTESTED',
                'retentions':len(retentions),'successful_retentions':sum(r['passed'] for r in retentions),
                'retention_status':'TESTED' if retentions else 'UNTESTED'})
    return result
