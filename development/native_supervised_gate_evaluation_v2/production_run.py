...
def inventory():
    ...
    require(sum(p['bytes'] for p in files)<=192*1024**2,'INVENTORY_TOTAL_CAP')
    return files
def worker(deadline):
    ...
    result={'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT','scientific_pass':False,'primary':{'kind':'TECHNICAL','code':'WORKER_ENTRY_FAILURE'},
        'learned_gate_success_claimed':False,'gate_validity':'FRESH_CENSUS_PENDING',
        'census':{'complete':False,'outcomes':[],'confusion':{'TP':0,'TN':0,'FP':0,'FN':0}}}
