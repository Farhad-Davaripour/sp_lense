...
def execution_binding(source_sha):
    return {'namespace':'development/native_gate_coverage_increment_train_capture_v1','source_freeze_sha256':source_sha,
...
def validate(data,lock,records,result,journal,source_sha,raw_lock_sha,*,synthetic=False):
    ...
    require(result['status']=='PASS' and result['scope']==scope and result['retry_allowed'] is False
        and result['complete_input_publication'] is True and result['completed_cases']==6
        and result['attempted_operations']==result['completed_operations']==result['planned_operations']==79
    ...
    require(len(journal)==158,'EXACT79_OPERATION_PAIRS')
    ...
    planned=slots();require(len(data['cases'])==6 and len(records)==6 and len(lock['rendered_prompts'])==6,'EXACT12_INPUTS')
