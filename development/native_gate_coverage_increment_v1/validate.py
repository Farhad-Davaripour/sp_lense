...
SCHEMA_SHA256='6b99cce31f360eb521280f8a041cbf69fb4866f58e58a2e127ec9277c669c965'
...
def validate(cohort):
    ...
    need(sha(raw)==SCHEMA_SHA256,'IMMUTABLE_SCHEMA_BYTES');template=json.loads(raw)['TRAIN_SUBMISSION']
    exact_keys(cohort,template,'COHORT');need(cohort['schema_version']==template['schema_version'],'SCHEMA')
    need(type(cohort['families']) is list and len(cohort['families'])==1,'ONE_TRAIN_FAMILY')
    ...
    need(len(prompts)==6 and len({p['prompt_sha256'] for p in prompts})==6,'EXACT6_UNIQUE_RENDERINGS')
