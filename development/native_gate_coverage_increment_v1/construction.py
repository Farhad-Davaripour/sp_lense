...
TRAINING_SOURCE=HERE.parent/'native_gate_order_invariant_v1/CLOSED_52_VIEW_MANIFEST.json'
TRAINING_SOURCE_SHA=source_auth.PAIR_MANIFEST_SHA
LIMITS={'worker_seconds':60,'cleanup_seconds':5,'maximum_fits':6,'iterations_per_fit':10000,
...
def schedule():
    return [{'id':'BASELINE26','train':list(range(26)),'held':[]},
        {'id':'AUGMENTED29','train':list(range(29)),'held':[]}]
...
def input_contract():
    ...
    return {'schema':'coverage_increment_conjunction_inputs.v1','role':'EXPOSED_DEVELOPMENT_ONLY',
...
def lock(inputs,sources):
    return {'schema':'coverage_increment_conjunction_contract.v1','method':METHOD,'feature_contract':gate.CONTRACT,
        'limits':LIMITS,'inputs_sha256':gate.digest(inputs),'sources_sha256':gate.digest(sources),
        'scope':'ONE_OWNED_ATTEMPT_MAX6_SOLVES','arm_order':('BASELINE26','AUGMENTED29'),
        'requires':'baseline26_allcorrect_then_augmented29_allcorrect_before_ANY_held_work','retry':False,
...
def candidate():
    ...
    release=gate.canonical({'schema':'coverage_increment_conjunction_release.v1','approved':False,
    ...
    return {'status':'METADATA_ONLY_DEFAULT_DENY','maximum_optimizations':6}
def authorize(release_raw,expected_sha):
    ...
    expected={'schema':'coverage_increment_conjunction_release.v1','approved':True,'operation':'one_construction_fit',
...
def artifact(model,bindings):
    return gate.canonical({'schema':'coverage_increment_conjunction_gate.v1','method':METHOD,'feature_contract':gate.CONTRACT,
...
def load_artifact(raw,*,expected_sha256,expected_bindings):
    ...
    gate.require(value['schema']=='coverage_increment_conjunction_gate.v1' and gate.canonical(value['method'])==gate.canonical(METHOD)
...
def empty_result():
    return {'schema':'coverage_increment_conjunction_result.v1','role':'EXPOSED_DEVELOPMENT_ONLY','scientific_pass':False,
    ...
    for stage in result['stages']:
        ...
        try:
            ...
            for record in stage['heads']:
                ...
                gate.require(len(indices)==({'ordinary':14}.get(record['id'],12) if stage['id']=='BASELINE26' else {'ordinary':15}.get(record['id'],14)),'EXACT_HEAD_TRAINING_SIZE')
                gate.require(time.monotonic()<deadline and result['fits_attempted']<6,'SHARED_HEAD_BUDGET')
                result['fits_attempted']+=1;record.update(status='ATTEMPTED',train=indices)
                if publish:publish('FIT_ATTEMPT_'+str(result['fits_attempted'])+'.json',{'stage':stage['id'],'head':record['id'],'attempt':result['fits_attempted'],'maximum':6})
            ...
            if stage['id']=='AUGMENTED29':result.update(status='BOTH_TRAINING_ARMS_PASS',scientific_pass=True)
    ...
    try:
        ...
        if result['scientific_pass']:
            ...
            for arm,model in models.items():
                ...
                eligible=rows[:26] if arm=='BASELINE26' else rows
