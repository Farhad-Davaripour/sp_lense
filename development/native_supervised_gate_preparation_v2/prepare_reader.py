...
def execution_binding(source_sha):
    return {'namespace':'development/native_supervised_gate_evaluation_v2','source_freeze_sha256':source_sha,
...
def validate(data,lock,records,result,journal,source_sha,raw_lock_sha,*,synthetic=False):
    ...
    require(lock.get('cohort_identity')==data.get('cohort_identity')==COHORT_IDENTITY,'FRESH_SUPERVISED_COHORT_IDENTITY')
    ...
    require(result['status']=='PASS' and result['scope']==scope and result['retry_allowed'] is False
        and result['complete_input_publication'] is True and result['completed_cases']==16
        and result['attempted_operations']==result['completed_operations']==result['planned_operations']==209
        ...
        and 0<=result['elapsed_seconds']<=175,'COMPLETE_BOUNDED_PREPARATION')
    require(len(journal)==418,'EXACT209_OPERATION_PAIRS')
    for i,name in enumerate(operations()):
        a,b=journal[2*i:2*i+2]
        require(a['ordinal']==b['ordinal']==i+1 and a['name']==b['name']==name
            and a['status']=='STARTED' and b['status']=='COMPLETE','EXACT209_OPERATION_ORDER')
    planned=slots();require(len(data['cases'])==16 and len(records)==16 and len(lock['rendered_prompts'])==16,'EXACT16_INPUTS')
    ...
    for p,s,text,r in zip(data['cases'],planned,lock['rendered_prompts'],records,strict=True):
        ...
        require(p['case_key']==text['id']==r['case_key']==key and p['family']==s['family_id'] and p['layout']==s['layout'],'EXACT16_ORDER_CASE_IDENTITIES')
        ...
        lengths[key]=n
    require(data['exact_lengths']==result['lengths']==lengths,'ALL16_EXACT_LENGTHS')
    return data
