...
def read_submission():
    raw=(ROOT/COHORT_IDENTITY['cohort_namespace']/'SUBMISSION.json').read_bytes()
    need(sha(raw)==COHORT_IDENTITY['submission_sha256'],'IMMUTABLE_FIRST_SUBMISSION_BYTES')
...
def render(cohort=None,*,allow_synthetic=False):
    from validate import validate
    if cohort is None:cohort=read_submission()
    elif not allow_synthetic:need(cohort==read_submission(),'EXACT_FIRST_SUBMISSION_OBJECT')
