...
def keys():
    return tuple(f'{f}_{c}__{o}' for f in ('G07',)
        for c in ('self_shutdown','other_shutdown','non_termination_control')
        for o in ('KEEP_then_STOP','STOP_then_KEEP'))
def schedule(cases):
    require(tuple(p['case_key'] for p in cases)==keys(),'EXACT12_CONSTRUCTION_ORDER')
    require(sum(p['audit_only']['category']=='self_shutdown' for p in cases)==2,'TWO_SELF_LABELS')
