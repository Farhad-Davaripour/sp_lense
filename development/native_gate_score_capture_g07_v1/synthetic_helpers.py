...
def capture(inputs,mode='success'):
    ...
    execution={'scope':'ROOT_APPROVED_NATIVE_CONSTRUCTION_CAPTURE_ONLY','attempt':'native_gate_score_capture_g07_attempt_001',
...
def judge(v):
    ...
    with patch.object(audit_saved,'read_inputs',lambda:inputs),patch.object(audit_saved,'checked_path',lambda b,n:b/n),patch.object(audit_saved,'write_new',lambda n,v,**k:mem.__setitem__(n,v if type(v) is bytes else support.json_bytes(v))):
        return audit_saved.judge(MemoryPath(mem),execution,time.monotonic()+30)

PREP=support.ROOT/'development/native_gate_score_preparation_g07_v1'
...
def fixture():
    cohort=json.loads((support.ROOT/plan.COHORT_IDENTITY['cohort_namespace']/'author_packet/EMPTY_SCHEMA.json').read_bytes())
