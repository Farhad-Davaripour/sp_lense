...
LIMITS={'worker_seconds':60,'cleanup_seconds':5,'maximum_fits':7,'iterations_per_fit':10000,
...
def input_contract():
    return {'schema':'hardmargin_coverage_inputs.v1','role':'EXPOSED_DEVELOPMENT_ONLY',
        'capture_binding':source_auth.frozen_binding(),
        'keys':source_auth.keys(),'labels':source_auth.expected_labels(),'stages':schedule(),
        'failed_ridge_provenance':source_auth.FAILED_RIDGE_PROVENANCE,'starting_weights':'none'}
def source_lock():return {n:gate.digest((HERE/n).read_bytes()) for n in CORE_FILES}
def lock(inputs,sources):
    return {'schema':'hardmargin_coverage_contract.v1','method':gate.METHOD,'feature_contract':gate.CONTRACT,
        'limits':LIMITS,'inputs_sha256':gate.digest(inputs),'sources_sha256':gate.digest(sources),
        'scope':'ONE_OWNED_FAMILYDEV_ATTEMPT_MAX7_OPTIMIZATIONS','fold_order':FAMILIES,
        'requires':'each38train+6held_then_full44_allcorrect_with_certificates','retry':False,
        'method_selection':'post_ridge_failure_development;lambda0.001_rejected_before_any_fit',
...
def candidate():
    ...
    source_auth.metadata()  # authenticate closed manifests only, never coordinates
    inputs=gate.canonical(input_contract());sources=gate.canonical(source_lock());contract=gate.canonical(lock(inputs,sources))
    release=gate.canonical({'schema':'hardmargin_coverage_release.v1','approved':False,
    ...
    return {'status':'METADATA_ONLY_DEFAULT_DENY','maximum_optimizations':7}
def authorize(release_raw,expected_sha):
    ...
    expected={'schema':'hardmargin_coverage_release.v1','approved':True,'operation':'one_construction_fit',
...
def run(rows,labels,*,deadline,solve_fn=None,publish=None):
    ...
    result={'schema':'hardmargin_coverage_result.v1','role':'EXPOSED_DEVELOPMENT_ONLY','scientific_pass':False,
    ...
    for stage in stages:
        try:
            ...
            gate.require(attempts<7,'MAXIMUM_SEVEN_FITS')
            attempts+=1;result['fits_attempted']=attempts;stage['status']='ATTEMPTED'
            if publish:publish('FIT_ATTEMPT_'+str(attempts)+'.json',{'stage':stage['id'],'attempt':attempts,'maximum':7})
...
def construct(release_path,release_sha256):
    ...
    result={'schema':'hardmargin_coverage_result.v1','role':'EXPOSED_DEVELOPMENT_ONLY',
    ...
    try:
        ...
        if result['scientific_pass']:
            bindings={'input_contract_sha256':release['training_manifest_sha256'],
                ...
                'feature_sha256':source_auth.frozen_binding()['combined_feature_sha256'],'development_only':True}
