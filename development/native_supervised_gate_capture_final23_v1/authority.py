...
LIMITS={'loads':1,'forwards':32,'derivatives':0,'worker_seconds':1200,'audit_seconds':120,'shared_cleanup_seconds':15,'total_bytes':64*1024**2,'file_bytes':5*1024**2}
...
def read_release(approved):
    ...
    require(release['schema']=='native_supervised_gate_capture_final23_release.v1' and release['approved'] is True
        and release['attempt']==ATTEMPT and release['limits']==LIMITS
        and Path(release['output']).resolve()==output().resolve(),'EXACT_NATIVE_RELEASE')
    require(release['reserved_bytes']==sum(GROUP_CAPS.values())+65536==37879808
...
def execution(release,approved):
    return {'scope':'ROOT_APPROVED_NATIVE_FINAL23_CONSTRUCTION_CAPTURE_ONLY','attempt':ATTEMPT,'release_sha256':approved,
        ...
        'training_submission_sha256':release['admitted_submission_sha256'],'role':'CONSTRUCTION',
        'reused_inputs_sha256':release['reused_inputs_sha256'],
        'readout':'model.language_model.layers.23/output_before_final_norm/final_original_input'}
def authenticate():
...
def admit_once(approved):
    ...
    admission={'execution':execution(release,approved),'source_freeze_sha256':release['source_freeze_sha256'],
        'started_monotonic':started,'absolute_end':started+1335,'controller_pid':os.getpid()}
