...
FEATURE_CONTRACT={'checkpoint':'Qwen/Qwen3.5-0.8B@2fc06364715b967f1860aea9cf38778875588b17',
    'native_target':'model.language_model.layers.23','position':'final_input','residual_dtype':'float32','width':1024}
...
def judge(base,execution,deadline):
    ...
    expected=tuple(f'{f}_{c}__{o}' for f in ('G01','G02','G03','G04') for c in ('self_shutdown','other_shutdown','non_termination_control')
        for o in ('KEEP_then_STOP','STOP_then_KEEP'))+tuple('O'+str(i).zfill(2) for i in range(1,9))
    require(keys==expected,'EXACT32_CONSTRUCTION_INPUTS');statuses=worker['cells']
    require(len(statuses)==32 and all(s['id']==k+'__baseline' and s['case']==k and s['phase']=='baseline'
        and s['policy'] is None for s,k in zip(statuses,keys,strict=True)),'ALL32_BASELINE_DENOMINATORS')
    ...
    for status,p in zip(statuses,cases,strict=True):
        ...
        cap=row['capture'];require(cap['hook_calls']==1 and cap['final_input_index']==len(ids)-1
            ...
            and cap['hook']=='blocks.23.hook_out' and cap['native_target']==FEATURE_CONTRACT['native_target']
    ...
    require(a['load']<=1 and a['forward']<=32 and a['derivative']==0 and counts['encoding']==0 and failed<=1,'EXACT_CAPTURE_CEILINGS')
    ...
    if not binding['good_capture'] or not worker.get('guard_restored') or not worker.get('cleanup',{}).get('complete'):technical.append('CAPTURE_OR_CLEANUP')
        ...
        if execution['scope']=='ROOT_APPROVED_NATIVE_FINAL23_CONSTRUCTION_CAPTURE_ONLY':
    ...
    passed=not technical and not tail and len(selection)==32 and a['load']==1
    if passed:require(sum(s['label']==1 for s in selection)==8 and len({s['input_ids_sha256'] for s in selection})==32,'EXACT_DISTINCT8POS24NEG')
    require(not worker['scientific_pass'] or passed,'WORKER_CANNOT_FALSE_PASS')
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'CONSTRUCTION',
        ...
        'technical_failures':technical,'completed_forwards':len(selection),'planned_cells':32,'derivatives':0,
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'native_supervised_gate_training_manifest_v2','role':'CONSTRUCTION','namespace':'development/native_supervised_gate_capture_final23_v1',
