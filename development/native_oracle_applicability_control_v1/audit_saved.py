...
def judge(base,execution,deadline):
    ...
    require(sum(sizes)<=32*1024**2,'INDEPENDENT_TOTAL_CAP')
    ...
    require(len(cells)==21,'INDEPENDENT_SCHEDULE')
    ...
    if not statuses:
        ...
        return {'execution':execution,'audit_completed':True,'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT',
            'scientific_pass':False,'completed_forwards':0,'planned_cells':21,'unrun':21,'reason':'WORKER_PRE_SCHEDULE_FAILURE'}
    require(len(statuses)==21 and all({k:v[k] for k in ('id','case','phase','policy')}==c for v,c in zip(statuses,cells,strict=True)),'ALL_PLANNED_DENOMINATORS')
    ...
    for status in statuses:
        ...
        require(row.get('effective_route')=='ON' and row.get('external_applicable') is True
    ...
    require(counts['load']<=1 and counts['forward']<=21 and counts['derivative']<=8 and failed_count<=1,'ATTEMPT_CEILINGS')
    ...
    return {'execution':execution,'audit_completed':True,'classification':classification,'scientific_pass':passed,
        ...
        'completed_forwards':completed,'completed_derivatives':derivatives,'planned_cells':21,
