"""One exposed case, separately authorized oracle applicability; fresh numeric state."""
import math,time
...
def schedule(cases):
    ...
    need(len(cells)==21,'FIXED_21_CELLS');return cells
...
def execute(receiver,torch,counts,inputs,write,trace_call,deadline):
    ...
    def call(p,cell,delta,baseline=None,current=None,sign=0):
        ...
        if p is not None:
            ...
            row.update(effective_route='ON',external_applicable=True,applicability_source='EXTERNAL_ORACLE')
        ...
        if p is not None:
            ...
            need(row['effective_route']=='ON' and row['external_applicable'] is True,'ORACLE_DISPATCH')
