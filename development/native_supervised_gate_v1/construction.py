...
from source_auth import HERE, SOURCES, build_manifest, extract_features
...
def construction_lock(manifest, sources):
    return canonical({"schema": "native_gate_construction_lock_v1", "method": METHOD,
                      "feature_contract": CONTRACT, "rows": 9, "positive": 4, "negative": 5,
                      ...
                      "success": "nine_correct_and_independent_check", "retry": False,
...
def candidate():
    ...
    return {"status": "MODEL_FREE_CANDIDATE", "files": {name: digest(raw) for name, raw in
            ...
            "selected_rows": 9, "real_feature_fits": 0, "feature_coordinates_emitted": False,
...
def construct(release_path, expected_release_sha256):
    ...
    try:
        ...
        require(len(rows) == 9 and len(rows[0]) == 1024 and labels == tuple(
            label for source in SOURCES for _, _, label in source.selection), "EXACT_TRAINING_SET")
        require(time.monotonic() - started < 60, "CONSTRUCTION_DEADLINE")
        fit_count = 1
        model = fit(rows, labels)
        require(time.monotonic() - started < 60, "CONSTRUCTION_DEADLINE")
        checked = verify(rows, labels, model)
        scientific_outcome = "CONSTRUCTION_PASS" if checked["correct"] == 9 else "CONSTRUCTION_FIT_FAIL"
        ...
        result = {"status": status, "scientific_pass": checked["correct"] == 9,
