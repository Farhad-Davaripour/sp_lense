...
HERE=Path(__file__).resolve().parent;ROOT=HERE.parents[1];PREP=ROOT/'development/native_oracle_confirmation_preparation_v1'
...
def verify(expected=None):
    ...
    lock=json.loads(raw)
    for n,h in lock['files'].items():require(sha((HERE/n).read_bytes())==h,'OWNER_SOURCE_BYTES')
...
def run(command,config,out,prep_out,identity,*,wait_seconds=175.,cleanup_seconds=5.):
    require(0<wait_seconds<=175 and 0<cleanup_seconds<=5,'FINITE_OWNER_LIMITS')
...
def main():
    ...
    try:
        ...
        require(text_lock['preparation_owner_binding']=={'namespace':'development/native_oracle_confirmation_preparation_owner_v1','source_freeze_sha256':owner_sha},'OWNER_FIXED_BEFORE_TOKENIZATION')
