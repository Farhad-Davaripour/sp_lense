...
NAMES=('dependencies','plan','storage','renderer','validate','prepare_core','prepare_reader','native_supervised_prepared_reader')
def keys():
    return tuple(f'{f}_{c}__{o}' for f in ('G01','G02','G03','G04','G05','G06') for c in ('self_shutdown','other_shutdown','non_termination_control') for o in ('KEEP_then_STOP','STOP_then_KEEP'))+tuple(k for i in range(1,9) for k in (f'O{i:02}',f'O{i:02}__B_then_A'))+tuple(f'G08_{c}__{o}' for c in ('self_shutdown','other_shutdown','non_termination_control') for o in ('KEEP_then_STOP','STOP_then_KEEP'))
...
def data():
    ...
    require(lock['schema']=='prechoice_retained_training_inputs.v1' and lock['role']=='TRAIN_PRECHOICE'
...
def chain(pin):
    ...
    try:
        ...
        sys.path[:]=path
def build_inputs():
    lock,cert=data();cases={};witnesses={k:w for w in cert['pairs'] if w['role']=='TRAIN' for k in w['view_keys']}
    require(set(witnesses)==set(keys()) and len(witnesses)==58,'EXACT_TRAINING_SELECTOR_KEYS')
    for index in lock['training_chain_indexes']:
    ...
    result={'schema':'prechoice_training_inputs.v1','role':'TRAIN_PRECHOICE','cases':[cases[k] for k in keys()],
