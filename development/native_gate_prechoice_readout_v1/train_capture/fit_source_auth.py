...
CAPTURE=HERE
ATTEMPT='prechoice_train_capture_attempt_001'
...
def build_manifest(*,deadline=None):
    ...
    require(manifest['schema']=='prechoice_train_capture_manifest.v1' and manifest['role']=='TRAIN_PRECHOICE'
        and manifest['feature_contract']==CONTRACT and manifest['namespace']=='development/native_gate_prechoice_readout_v1/train_capture'
    ...
    require(tuple(x['label'] for x in selection)==expected and expected.count(1)==14,'EXACT_TWO_POSITIVE_FOUR_NEGATIVE')
...
def extract_features(manifest,*,deadline):
    ...
    require(len(rows)==58 and len(set(x['input_ids_sha256'] for x in manifest['selection']))==58,'NO_DUPLICATE_TRAINING_INPUT')
