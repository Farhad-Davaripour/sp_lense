...
from index_contract import CONTRACT as FEATURE_CONTRACT,validate_capture
...
def judge(base,execution,deadline):
    ...
    require(all(n<=5*1024**2 for n in sizes) and sum(sizes)<=80*1024**2,'CAPTURE_STORAGE')
    def raw(name):
    ...
    worker=json.loads(worker_raw);require(worker['execution']==execution and worker['role']=='TRAIN_PRECHOICE','HELD_WORKER_SCOPE')
    ...
    require(len(statuses)==58 and all(s['id']==k+'__baseline' and s['case']==k and s['phase']=='baseline'
    ...
    for status,p in zip(statuses,cases,strict=True):
        ...
        require(row['readout_selector']==p['readout_selector'],'RETAINED_SELECTOR_ROW_JOIN')
        validate_capture(ids,p['readout_selector'],row['capture'])
        ...
        selection.append({'case':p['case_key'],'category':category,'label':label,'row':name,'sha256':sha(row_raw),'bytes':len(row_raw),
            ...
            'readout_index':cap['readout_index'],'readout_selector_sha256':cap['readout_selector_sha256']})
    counts=worker['counts'];a=counts['attempts'];dispatch=worker.get('dispatch',{})
    require(a['load']<=1 and a['forward']<=58 and a['derivative']==0 and counts['encoding']==0 and failed<=1,'EXACT_CAPTURE_CEILINGS')
    ...
    passed=not technical and not tail and len(selection)==58 and a['load']==1
    if passed:require(sum(s['label']==1 for s in selection)==14 and len({s['input_ids_sha256'] for s in selection})==58,'EXACT_DISTINCT_TWO_POS_FOUR_NEG')
    require(not worker['scientific_pass'] or passed,'WORKER_CANNOT_FALSE_PASS')
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'TRAIN_PRECHOICE',
        ...
        'technical_failures':technical,'completed_forwards':len(selection),'planned_cells':58,'derivatives':0,
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'prechoice_train_capture_manifest.v1','role':'TRAIN_PRECHOICE','namespace':'development/native_gate_prechoice_readout_v1/train_capture',
