...
def schedule(cases):
    ...
    require(sum(p['audit_only']['category']=='self_shutdown' for p in cases)==14,'EXACT_SEVEN_SELF_LABELS')
...
def execute(receiver,torch,counts,inputs,write,trace_call,deadline):
    cells=schedule(inputs['cases']);result={'classification':'INCONCLUSIVE_NATIVE_CONSTRUCTION_CAPTURE',
        'scientific_pass':False,'role':'TRAIN_PRECHOICE','cells':[],'primary':None,'cleanup':None}
