"""One fixed baseline26 versus augmented29 training-only construction."""
...
TRAINING_SOURCE=HERE.parent/'train_capture/DATA_LOCK.json'
TRAINING_SOURCE_SHA=source_auth.DATA_LOCK_SHA
LIMITS={'worker_seconds':60,'cleanup_seconds':5,'maximum_fits':3,'iterations_per_fit':10000,
...
def schedule():
    return [{'id':'PRECHOICE29','train':list(range(29)),'held':[]}]
...
def input_contract():
    ...
    original=gate.decode(raw);binding=source_auth.frozen_binding()
    gate.require(original['training_views']==58 and original['role']=='TRAIN_PRECHOICE','EXACT_ADMITTED58_TO29')
    return {'schema':'prechoice29_conjunction_inputs.v1','role':'EXPOSED_DEVELOPMENT_ONLY',
        'capture_binding':binding,'retained_input_data_lock_sha256':TRAINING_SOURCE_SHA,
...
def lock(inputs,sources):
    return {'schema':'prechoice29_conjunction_contract.v1','method':METHOD,'feature_contract':gate.CONTRACT,
        'limits':LIMITS,'inputs_sha256':gate.digest(inputs),'sources_sha256':gate.digest(sources),
        'scope':'ONE_OWNED_ATTEMPT_MAX3_SOLVES','arm_order':('PRECHOICE29',),
        'requires':'prechoice29_allcorrect_and_certified_before_diagnostic_capture','retry':False,
...
def candidate():
    ...
    release=gate.canonical({'schema':'prechoice29_conjunction_release.v1','approved':False,
    ...
    return {'status':'METADATA_ONLY_DEFAULT_DENY','maximum_optimizations':3}
def authorize(release_raw,expected_sha):
    ...
    expected={'schema':'prechoice29_conjunction_release.v1','approved':True,'operation':'one_construction_fit',
...
def artifact(model,bindings):
    return gate.canonical({'schema':'prechoice29_conjunction_gate.v1','method':METHOD,'feature_contract':gate.CONTRACT,
...
def load_artifact(raw,*,expected_sha256,expected_bindings):
    ...
    gate.require(value['schema']=='prechoice29_conjunction_gate.v1' and gate.canonical(value['method'])==gate.canonical(METHOD)
...
def empty_result():
    return {'schema':'prechoice29_conjunction_result.v1','role':'EXPOSED_DEVELOPMENT_ONLY','scientific_pass':False,
...
def run(rows,labels,*,deadline,solve_fn=None,publish=None):
    ...
    gate.require(len(rows)==29 and tuple(labels)==source_auth.expected_labels(),'EXACT29_TRAINING_ONLY_SET')
    rows=gate.finite_rows(rows);result=empty_result();models={}
    for stage in result['stages']:
        ...
        try:
            ...
            for record in stage['heads']:
                ...
                gate.require(len(indices)==({'ordinary':15}.get(record['id'],14)),'EXACT_HEAD_TRAINING_SIZE')
                gate.require(time.monotonic()<deadline and result['fits_attempted']<3,'SHARED_HEAD_BUDGET')
                result['fits_attempted']+=1;record.update(status='ATTEMPTED',train=indices)
                if publish:publish('FIT_ATTEMPT_'+str(result['fits_attempted'])+'.json',{'stage':stage['id'],'head':record['id'],'attempt':result['fits_attempted'],'maximum':3})
            ...
            if stage['id']=='PRECHOICE29':result.update(status='PRECHOICE29_TRAINING_PASS',scientific_pass=True)
    ...
    return result,models if result['scientific_pass'] else {}
def construct(release_path,release_sha256):
    ...
    try:
        input_raw=(HERE/'TRAINING_MANIFEST.json').read_bytes()
        gate.require(gate.digest(input_raw)==release['training_manifest_sha256'],'RELEASED_INPUTS_BEFORE_FEATURES')
        rows,labels=source_auth.load_saved(deadline=deadline,expected_binding=gate.decode(input_raw)['capture_binding'])
        gate.require(all(len(r)==1024 for r in rows),'NATIVE_ARTIFACT_WIDTH')
        result,models=run(rows,labels,deadline=deadline,publish=lambda n,v:write_new(destination/n,v))
        if result['scientific_pass']:
            bindings={'input_contract_sha256':release['training_manifest_sha256'],
                ...
                'retained_input_data_lock_sha256':TRAINING_SOURCE_SHA,'development_only':True}
            result['artifacts']={}
            for arm,model in models.items():
                bound={**bindings,'arm':arm};raw=artifact(model,bound)
                loaded=load_artifact(raw,expected_sha256=gate.digest(raw),expected_bindings=bound)
                eligible=rows
                gate.require([model.scores(r) for r in eligible]==[loaded.scores(r) for r in eligible],'ARTIFACT_RELOAD_EXACT')
                gate.require(time.monotonic()<deadline,'SHARED_WORKER_DEADLINE')
                write_new(destination/(arm+'_GATE.json'),raw);result['artifacts'][arm]=gate.digest(raw)
