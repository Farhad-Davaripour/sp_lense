...
from .admission import require_code, admit_accelerate_forward
...
def _code(fn, expected, path):
    require_code(fn, expected, path, need)
...
def _live_sources(module, original_type, conv, chunk, root):
    ...
    admit_accelerate_forward(original_type.forward, module, hf_codes["Qwen3_5GatedDeltaNet.forward"],
                             hf_path, root, need)
