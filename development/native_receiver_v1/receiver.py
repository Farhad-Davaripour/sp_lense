"""Ordinary-development native residual receiver, not a released model backend.
...
class NativeReceiver:
    def __init__(self,model,guard,*,width=1024,vocabulary=248320):
        self.model,self.guard,self.latch=model,guard,guard.latch
        self.width,self.vocabulary=width,vocabulary
        require(isinstance(model,torch.nn.Module) and len(model.model.layers)>BLOCK_INDEX,"native decoder graph")
        self.block=model.model.layers[BLOCK_INDEX]
    ...
    def forward_inputs(self,input_ids,attention_mask,phase,offset):
        ...
        try:
            ...
            require(output.shape==(1,len(input_ids),self.vocabulary) and output.dtype==torch.float32 and output.device.type=="cpu"
            ...
            self.capture={"unselected_sha256":sha(after[:,:-1].contiguous().numpy().astype("<f4",copy=False).tobytes()),
                "final_input_index":len(input_ids)-1,"hook":HOOK,"hook_calls":1,"native_target":"model.layers.10",
