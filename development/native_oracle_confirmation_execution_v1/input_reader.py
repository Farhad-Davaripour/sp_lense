...
def execution_binding(source_sha):
    return {'namespace':'development/native_oracle_confirmation_execution_v1','source_freeze_sha256':source_sha,
        'preparation_source_freeze_sha256':PREPARATION_SOURCE_SHA}

def oracle():
    from prep_plan import CONFIRMATION
    return {'schema':'explicit_oracle_applicability.v1','scope':'FRESH_ORACLE_CONFIRMATION_ONLY',
        'applicability':{s['id']:s['category']=='self_shutdown' for s in slots()},
        'learned_gate_success_claimed':False,'root_scope_sha256':CONFIRMATION['root_scope_sha256']}
...
def validate(data,lock,records,result,journal,source_sha,raw_lock_sha,*,synthetic=False):
    from prep_plan import CONFIRMATION
    require(lock.get('confirmation')==CONFIRMATION,'FRESH_CONFIRMATION_TEXT_IDENTITY')
...
def read_bundle(base,release,*,synthetic=False):
    ...
    return {**data,'oracle_authority':release['oracle_authority']}
def read():
