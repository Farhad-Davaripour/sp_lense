...
LIMITS={'loads':1,'forwards':180,'derivatives':48,'worker_seconds':1800,'audit_seconds':180,'shared_cleanup_seconds':15,'total_bytes':288*1024**2,'file_bytes':5*1024**2}
...
def read_release(approved):
    ...
    require(release['schema']=='native_oracle_confirmation_release.v1' and release['approved'] is True
...
def execution(release,approved):
    return {'scope':'ROOT_APPROVED_ORACLE_CONFIRMATION_ONLY','attempt':ATTEMPT,'release_sha256':approved,
...
def admit_once(approved):
    ...
    admission={'execution':execution(release,approved),'source_freeze_sha256':release['source_freeze_sha256'],
        'started_monotonic':started,'absolute_end':started+1995,'controller_pid':os.getpid()}
