...
def judge(base,execution,deadline):
    ...
    for key in keys:
        for policy in ('P','C'):
            phases=['entry']+([v for n in range(1,5) for v in ('gradient_'+str(n),'step_'+str(n))]+['endpoint'] if key.startswith('self_') else [])
            cells += [{'id':key+'__'+policy+'__'+phase,'case':key,'phase':phase,'policy':policy} for phase in phases]
    require(len(cells)==52,'INDEPENDENT_SCHEDULE')
    ...
    if not statuses:
        ...
        return {'execution':execution,'audit_completed':True,'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT',
            'scientific_pass':False,'completed_forwards':0,'planned_cells':52,'unrun':52,'reason':'WORKER_PRE_SCHEDULE_FAILURE'}
    require(len(statuses)==52 and all({k:v[k] for k in ('id','case','phase','policy')}==c for v,c in zip(statuses,cells,strict=True)),'ALL_PLANNED_DENOMINATORS')
    ...
    for status in statuses:
        ...
        if row['phase']=='baseline' and row['case'].startswith('self_') and not eligible(row):scientific.append('FINITE_SELF_ELIGIBILITY')
        if row['phase']=='entry':
    ...
    for request in requests:
        ...
        if request['case'].startswith('self_'):
    ...
    require(counts['load']<=1 and counts['forward']<=52 and counts['derivative']<=16 and failed_count<=1,'ATTEMPT_CEILINGS')
    ...
    return {'execution':execution,'audit_completed':True,'classification':classification,'scientific_pass':passed,
        ...
        'completed_forwards':completed,'completed_derivatives':derivatives,'planned_cells':52,
