...
def main():
    ...
    names=[p.name for p in HERE.glob('*.py')]+['inputs.json','CHECKPOINT.json','REUSED_SOURCES.json','OWNED_IDENTITY.json','PROTOCOL.md','INPUT_PROVENANCE.json','FIXED_TEXTS.json','TOKENIZER_PINS.json','PREPARATION_CONTRACT.json','TOKENIZER_PREPARATION_PROCESS_OBSERVATION.json','TOKENIZER_PREP_STDOUT.log','TOKENIZER_PREP_STDERR.log']+[p.relative_to(HERE).as_posix() for p in sorted((HERE/'tokenizer_preparation_attempt_001').glob('*')) if p.is_file()]
    ...
    (HERE/'TEST_RESULTS.json').write_bytes(json_bytes({'status':'PASS','reports':reports,
        ...
        'scope':'ORDINARY_ENGINEERING_TINY_AUTOGRAD_AND_SAVED_AUDIT','new_production_owned_launch_observed':False}))
    print(json.dumps({'status':'PROSPECTIVE_CANDIDATE_ONLY','real_authorized':False,'source_freeze_sha256':draft['source_freeze_sha256'],
