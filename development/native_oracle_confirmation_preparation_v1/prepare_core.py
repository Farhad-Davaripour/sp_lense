...
def verify_prospective_bindings(lock,root=None):
    ...
    for key,namespace,field in (
        ('final_execution_binding','development/native_oracle_confirmation_execution_v1','source_sha256'),
        ('preparation_owner_binding','development/native_oracle_confirmation_preparation_owner_v1','files')):
...
def validate_text_lock(lock,allow_synthetic=False):
    from plan import CONFIRMATION
    need(lock.get('confirmation')==CONFIRMATION,'FRESH_CONFIRMATION_TEXT_IDENTITY')
    if not allow_synthetic:verify_prospective_bindings(lock)
    ...
    if not allow_synthetic:need(lock['blind_semantic_review_approved'] is True and lock['final_text_locked'] is True,'BLIND_REVIEW_THEN_TEXT_LOCK')
...
def execute(lock,out,factory,deadline,*,allow_synthetic=False,template_sha256=TEMPLATE_SHA256,clock=time.monotonic,identity=None,claimed=False):
    ...
    publish=Publisher(out);started=clock()
    ...
    try:
        ...
        need(len(journal)==313 and all(x['status']=='COMPLETE' for x in journal) and len(cases)==24,'ALL313_OPERATIONS_24_CASES')
        data={'schema_version':'native_final_prepared_inputs_v1','scope':result['scope'],'model':MODEL,'study':STUDY,
