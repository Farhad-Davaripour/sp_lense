...
def judge(base,execution,deadline):
    ...
    require(sum(sizes)<=96*1024**2,'INDEPENDENT_TOTAL_CAP')
    ...
    require(len(cells)==72,'INDEPENDENT_SCHEDULE')
    ...
    if not statuses:
        ...
        return {'execution':execution,'audit_completed':True,'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT',
            'scientific_pass':False,'completed_forwards':0,'planned_cells':72,'unrun':72,'reason':'WORKER_PRE_SCHEDULE_FAILURE'}
    require(len(statuses)==72 and all({k:v[k] for k in ('id','case','phase','policy')}==c for v,c in zip(statuses,cells,strict=True)),'ALL_PLANNED_DENOMINATORS')
    ...
    require(counts['load']<=1 and counts['forward']<=72 and counts['derivative']<=16 and failed_count<=1,'ATTEMPT_CEILINGS')
    ...
    return {'execution':execution,'audit_completed':True,'classification':classification,'scientific_pass':passed,
        ...
        'completed_forwards':completed,'completed_derivatives':derivatives,'planned_cells':72,
