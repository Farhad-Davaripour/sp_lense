...
def load(counts,latch,execution,deadline):
    require(False,'SYNTHETIC_BINDING_NO_REAL_LOADER')
    from authority import authenticate
    require(authenticate()['execution']==execution,'OWNED_AUTHORITY_JOIN')
    block_unused_dependencies()
    bootstrap=json.loads((HERE/'real_evidence'/'native_final_binding_attempt_001'/'owned/production_worker/BOOTSTRAP.json').read_bytes())
