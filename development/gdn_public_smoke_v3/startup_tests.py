...
CASES=("startup_to_sentinel","terminal_preload")
def need(ok,code):
...
def child(case,run_id):
    ...
    def target(reference):
        ...
        def stop_before_research(writer,counter,deadline,admitted):
            ...
            from lifecycle_tests import inert_guard,bind_inert_torch,state
            from constructor_operands import import_hf_definitions
            import helper_binding
            guard=inert_guard(counter)
            if case=='terminal_preload':guard.latch.raw.trip('H_IDENTITY')
            before=state(guard)
            original_package=helper_binding.ensure_package
            def definition_boundary():
                reached.append(True);events.append('PRE_DEFINITION_IMPORT_SENTINEL')
                need(state(guard)==before,'READINESS_DID_NOT_MUTATE_ACTUAL_LATCH')
                raise RuntimeError('INERT_PRE_DEFINITION_IMPORT_SENTINEL')
            helper_binding.ensure_package=definition_boundary
            try:
                with bind_inert_torch(guard):import_hf_definitions(guard)
            finally:
                helper_binding.ensure_package=original_package
                need(state(guard)==before,'PENDING_OR_TERMINAL_FIRST_CAUSE_PRESERVED')
                events.append('ACTUAL_PINNED_LIFECYCLE_UNCHANGED')
    ...
    elif case=='terminal_preload':
        need(not reached and worker['startup_failure'] is None,'TERMINAL_DENIED_BEFORE_DEFINITION_CALLBACK')
        need((control/'FIRST_FORWARD_TRACE.json').is_file(),'DENIAL_TRACE_RETAINED')
        need('ACTUAL_PINNED_LIFECYCLE_UNCHANGED' in events,'TERMINAL_FIRST_CAUSE_RETAINED')
    else:
