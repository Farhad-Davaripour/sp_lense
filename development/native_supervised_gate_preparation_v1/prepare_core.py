...
from plan import STUDY,PREPARATION,MODEL,HEADER,HEADER_TEXT,END,TEMPLATE_SHA256,COHORT_IDENTITY,slots,operations
...
def verify_prospective_bindings(lock,root=None):
    ...
    for key,namespace,field in (
        ('final_execution_binding','development/native_supervised_gate_evaluation_v1','source_sha256'),
        ('preparation_owner_binding','development/native_supervised_gate_evaluation_v1','source_sha256')):
...
def validate_text_lock(lock,allow_synthetic=False):
    ...
    need(lock.get('cohort_identity')==COHORT_IDENTITY,'FRESH_SUPERVISED_COHORT_IDENTITY')
    if not allow_synthetic:
        verify_prospective_bindings(lock)
        from renderer import read_submission
        need(lock['cohort']==read_submission(),'EXACT_FIRST_SUBMISSION_OBJECT')
    ...
    need([p['id'] for p in checked['prompts']]==[s['id'] for s in slots()],'EXACT16_LOCKED_ORDER')
...
def execute(lock,out,factory,deadline,*,allow_synthetic=False,template_sha256=TEMPLATE_SHA256,clock=time.monotonic,identity=None,claimed=False):
    ...
    publish=Publisher(out);started=clock();deadline=min(deadline,started+175)
    ...
    try:
        ...
        need(len(journal)==209 and all(x['status']=='COMPLETE' for x in journal) and len(cases)==16,'ALL209_OPERATIONS_16_CASES')
        data={'cohort_identity':COHORT_IDENTITY,'schema_version':'native_final_prepared_inputs_v1','scope':result['scope'],'model':MODEL,'study':STUDY,
        ...
        result.update(attempted_operations=len(journal),completed_operations=sum(x['status']=='COMPLETE' for x in journal),
            failed_operations=sum(x['status']=='FAILED' for x in journal),planned_operations=209,unrun_operations=209-len(journal),
