...
def execution_binding(source_sha):
    return {'namespace':'development/native_supervised_gate_capture_order_v1','source_freeze_sha256':source_sha,
...
def validate(data,lock,records,result,journal,source_sha,raw_lock_sha,*,synthetic=False):
    ...
    require(result['status']=='PASS' and result['scope']==scope and result['retry_allowed'] is False
        and result['complete_input_publication'] is True and result['completed_cases']==8
        and result['attempted_operations']==result['completed_operations']==result['planned_operations']==105
    ...
    require(len(journal)==210,'EXACT105_OPERATION_PAIRS')
    for i,name in enumerate(operations()):
        a,b=journal[2*i:2*i+2]
        require(a['ordinal']==b['ordinal']==i+1 and a['name']==b['name']==name
            and a['status']=='STARTED' and b['status']=='COMPLETE','EXACT105_OPERATION_ORDER')
    planned=slots();require(len(data['cases'])==8 and len(records)==8 and len(lock['rendered_prompts'])==8,'EXACT8_INPUTS')
    ...
    for p,s,text,r in zip(data['cases'],planned,lock['rendered_prompts'],records,strict=True):
        ...
        require(p['case_key']==text['id']==r['case_key']==key and p['family']==s['family_id'] and p['layout']==s['layout'],'EXACT8_ORDER_CASE_IDENTITIES')
        ...
        lengths[key]=n
    require(data['exact_lengths']==result['lengths']==lengths,'ALL8_EXACT_LENGTHS')
    return data
