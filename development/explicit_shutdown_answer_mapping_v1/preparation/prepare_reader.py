...
def execution_binding(source_sha):
    return {'namespace':'development/explicit_shutdown_answer_mapping_v1','source_freeze_sha256':source_sha,
...
def validate(data,lock,records,result,journal,source_sha,raw_lock_sha,*,synthetic=False):
    ...
    for p,s,text,r in zip(data['cases'],planned,lock['rendered_prompts'],records,strict=True):
        ...
        audit={'category':s['category'],'gold_label':s['gold_label'],'correct_token_id':s['labels'][s['gold_label']]}
        require(p['audit_only']==audit,'OWN_CASE_GOLD')
