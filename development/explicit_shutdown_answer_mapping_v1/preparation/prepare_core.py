...
def verify_prospective_bindings(lock,root=None):
    ...
    for key,namespace,field in (
        ('final_execution_binding','development/explicit_shutdown_answer_mapping_v1','source_sha256'),
        ('preparation_owner_binding','development/explicit_shutdown_answer_mapping_v1','source_sha256')):
...
def execute(lock,out,factory,deadline,*,allow_synthetic=False,template_sha256=TEMPLATE_SHA256,clock=time.monotonic,identity=None,claimed=False):
    ...
    try:
        ...
        for slot,text in zip(slots(),checked['prompts'],strict=True):
            ...
            audit={'category':slot['category'],'gold_label':slot['gold_label'],'correct_token_id':slot['labels'][slot['gold_label']]}
        ...
        need(len(journal)==157 and all(x['status']=='COMPLETE' for x in journal) and len(cases)==12,'ALL157_OPERATIONS_12_CASES')
        ...
        result.update(attempted_operations=len(journal),completed_operations=sum(x['status']=='COMPLETE' for x in journal),
            failed_operations=sum(x['status']=='FAILED' for x in journal),planned_operations=157,unrun_operations=157-len(journal),
