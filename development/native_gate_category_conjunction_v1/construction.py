"""One fixed category-conjunction development construction; no native model calls."""
...
TRAINING_SOURCE=HERE.parent/'native_gate_hardmargin_coverage_v1/TRAINING_MANIFEST.json'
TRAINING_SOURCE_SHA='4c0443844f096df7ffb6e23cb2bb236d5c146d66e339494d27d66f78e099df70'
FEATURE_SHA='95e9311c1b371d2206017237ebe8e878178c6812de4b4c298d2d510d69a17719'
...
METHOD={'name':'fixed_category_conjunction_v1','head_order':HEADS,'head_method':gate.METHOD,
...
def schedule():
    ...
    for family in FAMILIES:
        ...
        train=[i for i in range(44) if i not in held]
        gate.require(len(held)==6 and len(train)==38,'EXACT_FOLD_COUNTS')
        out.append({'id':family,'train':train,'held':held})
    return out+[{'id':'FULL44','train':list(range(44)),'held':[]}]
...
def input_contract():
    raw=TRAINING_SOURCE.read_bytes();gate.require(gate.digest(raw)==TRAINING_SOURCE_SHA,'EXACT_ADMITTED44_SOURCE')
    original=gate.decode(raw);binding=source_auth.frozen_binding()
    gate.require(original['capture_binding']==binding and binding['combined_feature_sha256']==FEATURE_SHA,
        'EXACT_ADMITTED44_CAPTURE_BINDING')
    gate.require(tuple(original['keys'])==source_auth.keys() and tuple(original['labels'])==source_auth.expected_labels(),
        'EXACT_ADMITTED44_KEYS_LABELS')
    return {'schema':'category_conjunction_inputs.v1','role':'EXPOSED_DEVELOPMENT_ONLY',
        'capture_binding':binding,'original_training_manifest_sha256':TRAINING_SOURCE_SHA,
...
def lock(inputs,sources):
    return {'schema':'category_conjunction_contract.v1','method':METHOD,'feature_contract':gate.CONTRACT,
        ...
        'requires':'certified_heads_and_joint38train+6held_then_full44_allcorrect','retry':False,
...
def candidate():
    ...
    release=gate.canonical({'schema':'category_conjunction_release.v1','approved':False,
...
def authorize(release_raw,expected_sha):
    ...
    expected={'schema':'category_conjunction_release.v1','approved':True,'operation':'one_construction_fit',
...
def artifact(model,bindings):
    return gate.canonical({'schema':'category_conjunction_gate.v1','method':METHOD,'feature_contract':gate.CONTRACT,
...
def load_artifact(raw,*,expected_sha256,expected_bindings):
    ...
    gate.require(value['schema']=='category_conjunction_gate.v1' and gate.canonical(value['method'])==gate.canonical(METHOD)
...
def empty_result():
    return {'schema':'category_conjunction_result.v1','role':'EXPOSED_DEVELOPMENT_ONLY','scientific_pass':False,
...
def run(rows,labels,*,deadline,solve_fn=None,publish=None):
    ...
    gate.require(len(rows)==44 and tuple(labels)==source_auth.expected_labels(),'EXACT44_DEVELOPMENT_SET')
    ...
    for stage in result['stages']:
        ...
        try:
            ...
            for record in stage['heads']:
                ...
                gate.require(len(indices)==({'ordinary':18}.get(record['id'],20) if stage['held'] else {'ordinary':20}.get(record['id'],24)),'EXACT_HEAD_TRAINING_SIZE')
            ...
            if stage['id']=='FULL44':final_model=model;result.update(status='DEVELOPMENT_PASS',scientific_pass=True)
    ...
    try:
        ...
        if result['scientific_pass']:
            bindings={'input_contract_sha256':release['training_manifest_sha256'],
                ...
                'feature_sha256':FEATURE_SHA,'development_only':True}
