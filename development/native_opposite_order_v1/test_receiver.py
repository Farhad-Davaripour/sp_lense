...
def main():
    ...
    class TinyNative(torch.nn.Module):
        def __init__(self,mode="clean",keep=1):
            super().__init__();self.model=torch.nn.Module();self.model.rope_deltas=None;self.model.language_model=torch.nn.Module()
            self.model.language_model.layers=torch.nn.ModuleList([Block(2. if i==11 else 1.) for i in range(24)])
            self.register_buffer('identity_sentinel',torch.zeros(1,dtype=torch.float32))
            self.mode=mode;self.body_calls=0;self.foreign=None;self.eval()
        def forward(self,*,input_ids,attention_mask,past_key_values,use_cache,logits_to_keep,return_dict,**kw):
            need(input_ids.dtype==attention_mask.dtype==torch.int64 and torch.equal(attention_mask,torch.ones_like(input_ids))
                 and past_key_values is None and use_cache is False and logits_to_keep in (0,1) and return_dict is True and all(v is None for v in kw.values()),"NATIVE_ARGS")
            self.body_calls+=1
            ...
            for i,layer in enumerate(self.model.language_model.layers):
                if self.mode=="mix" and i==11:h=h.cumsum(dim=1)
                h=layer(h)
                if i==10 and self.mode=="duplicate":h=layer(h)
            if logits_to_keep:h=h[:,-1:,:]
            z=torch.full((*h.shape[:2],248320),-100.,dtype=torch.float32)
            z[:,:,50057]=h[:,:,0].square();z[:,:,48964]=0.
            if self.mode=="foreign":self.foreign=self.model.language_model.layers[0].register_forward_hook(lambda m,a,o:None)
    ...
    def fixture(mode="clean",keep=1):
        ...
        model=TinyNative(mode);r=NativeReceiver(model,guard,logits_to_keep=keep);details={}
    ...
    def buffer_mutation():
        with fixture() as (r,c,m,g,d):
            original=m.identity_sentinel._version;m.identity_sentinel.data[0]+=1
            need(m.identity_sentinel._version==original,'VERSION_SILENT_BUFFER_FIXTURE')
            need(r.parameter_state(digest=True)['buffer_bytes_unchanged'] is False,'BUFFER_CONTENT_NOT_ONLY_METADATA')
        need(d['latch_failed'] and d['forward_dispatches']==0,'BUFFER_MUTATION_STOP')
        return d
    for name,body in (("zero_OFF_exact_identity",identity),("last_position_current_gradient",gradients),("stale_graph_rejected",stale),
        ...
        run(name,body)
    run('silent_buffer_mutation',buffer_mutation)
    for variant in ('module_hook','global_hook','instance_forward'):
        count=Counts();latch=DispatchLatch();guard=ForwardDerivativeGuard(TinyNative,count,latch,time.monotonic()+30);guard.install()
        model=TinyNative();handle=None
        try:
            if variant=='module_hook':handle=model.model.language_model.layers[0].register_forward_hook(lambda m,a,o:None)
            elif variant=='global_hook':handle=torch.nn.modules.module.register_module_forward_hook(lambda m,a,o:None)
            else:model.model.language_model.layers[0].forward=lambda x:x
            expected={'module_hook':'NO_INITIAL_MODULE_HOOKS','global_hook':'NO_INITIAL_GLOBAL_HOOKS','instance_forward':'NO_INITIAL_INSTANCE_FORWARD'}[variant]
            negative(lambda:NativeReceiver(model,guard),expected)
            need(model.body_calls==0,'INITIAL_OVERRIDE_REJECTED_BEFORE_FORWARD')
        finally:
            if handle is not None:handle.remove()
            guard.restore()
    results.append({'case':'initial_foreign_hooks_and_instance_forward_rejected','status':'PASS'})
    for amount in (0.,.25):
        comparisons=[]
        for keep in (0,1):
            with fixture("mix",keep) as (r,c,m,g,d):
                r.start_request("ON");r.begin_edit();delta=zero.clone();delta[0]=amount
                z,h=forward(r,c,"gradient_1",delta)
                leaf=r.graph_leaf
                c.reserve("derivative")
                with g.permit("derivative"):
                    full=torch.autograd.grad(z[50057]-z[48964],leaf,retain_graph=False,create_graph=False)[0]
                need(full.shape==(1,3,1024) and bool(full[0,0].abs().any()),"FULL_POSITION_MIXING_GRAPH")
                comparisons.append((z.detach().clone(),full.detach().clone(),h.detach().clone(),r.capture["nonfinal_positions"]))
                r.last_graph_result=None;r.finish_request()
            need(not d["latch_failed"] and d["parameter_gradients_absent"],"KEEP_CLEAN")
        need(torch.equal(comparisons[0][0],comparisons[1][0]) and torch.equal(comparisons[0][1],comparisons[1][1])
             and torch.equal(comparisons[0][2],comparisons[1][2]) and comparisons[0][3]==comparisons[1][3]==2,"KEEP_ZERO_ONE_PARITY")
        results.append({"case":"keep0_keep1_fullleaf_offset_"+str(amount),"status":"PASS"})
    ...
    return {"status":"PASS","cases":results,"elapsed_seconds":time.monotonic()-started,
        ...
        "production_authorized":False,"native_HF_or_gate_parity":"UNVERIFIED","logits_to_keep_0_1_tiny_fullleaf_parity":True}
if __name__=="__main__":
    ...
    (HERE/'TEST_RECEIVER_RESULT.json').write_text(json.dumps(result,sort_keys=True)+'\n')
