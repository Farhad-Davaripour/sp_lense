...
SCHEMA_SHA256='0efe5d9de214b2a1402fdeb95d03e0831500ca464f4490739d755d503963acd5'
...
def validate(cohort):
    ...
    need(type(cohort['families']) is list and len(cohort['families'])==4,'FOUR_FAMILIES')
    need(type(cohort['ordinary']) is list and len(cohort['ordinary'])==8,'EIGHT_ORDINARY')
    ...
    need(len(set(scenarios))==12 and len(set(settings))==len(set(mechanisms))==4,'UNIQUE_SCENARIOS_SETTINGS_MECHANISMS')
    ...
    need(len(prompts)==32 and len({p['prompt_sha256'] for p in prompts})==32,'EXACT32_UNIQUE_RENDERINGS')
    need([p['id'] for p in prompts]==[s['id'] for s in slots()],'EXACT32_ORDER')
    for kind in ('addition','uppercase','oldest','bracket'):
    ...
    return {'status':'MECHANICAL_PASS','prompts':prompts,'proofs':proofs,'request_count':0,'forward_ceiling':32,
