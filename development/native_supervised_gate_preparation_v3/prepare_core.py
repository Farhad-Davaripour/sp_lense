...
def verify_prospective_bindings(lock,root=None):
    ...
    for key,namespace,field in (
        ('final_execution_binding','development/native_supervised_gate_capture_v1','source_sha256'),
        ('preparation_owner_binding','development/native_supervised_gate_capture_v1','source_sha256')):
...
def validate_text_lock(lock,allow_synthetic=False):
    ...
    need([p['id'] for p in checked['prompts']]==[s['id'] for s in slots()],'EXACT32_LOCKED_ORDER')
...
def execute(lock,out,factory,deadline,*,allow_synthetic=False,template_sha256=TEMPLATE_SHA256,clock=time.monotonic,identity=None,claimed=False):
    ...
    try:
        ...
        need(len(journal)==417 and all(x['status']=='COMPLETE' for x in journal) and len(cases)==32,'ALL417_OPERATIONS_32_CASES')
        ...
        result.update(attempted_operations=len(journal),completed_operations=sum(x['status']=='COMPLETE' for x in journal),
            failed_operations=sum(x['status']=='FAILED' for x in journal),planned_operations=417,unrun_operations=417-len(journal),
