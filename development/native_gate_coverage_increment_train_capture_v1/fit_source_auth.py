...
CAPTURE=HERE
ATTEMPT='native_gate_coverage_increment_train_capture_attempt_001'
MAX_FILE_BYTES=5*1024**2
CAPTURE_SOURCE_SHA256=digest((CAPTURE/'SOURCE_FREEZE.json').read_bytes())
def keys():
    return tuple('G08_'+c+'__'+o for c in ('self_shutdown','other_shutdown','non_termination_control') for o in ('KEEP_then_STOP','STOP_then_KEEP'))
...
def build_manifest(*,deadline=None):
    ...
    require(manifest['schema']=='coverage_increment_train_capture_manifest.v1' and manifest['role']=='CONSTRUCTION'
        and manifest['feature_contract']==CONTRACT and manifest['namespace']=='development/native_gate_coverage_increment_train_capture_v1'
    ...
    require(tuple(x['label'] for x in selection)==expected and expected.count(1)==2,'EXACT_TWO_POSITIVE_FOUR_NEGATIVE')
...
def extract_features(manifest,*,deadline):
    ...
    require(len(rows)==6 and len(set(x['input_ids_sha256'] for x in manifest['selection']))==6,'NO_DUPLICATE_TRAINING_INPUT')
