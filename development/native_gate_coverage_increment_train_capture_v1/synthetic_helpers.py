...
def capture(inputs,mode='success'):
    ...
    execution={'scope':'ROOT_APPROVED_NATIVE_CONSTRUCTION_CAPTURE_ONLY','attempt':'native_gate_coverage_increment_train_capture_attempt_001',
...
PREP=support.ROOT/'development/native_gate_coverage_increment_train_preparation_v1'
...
def fixture():
    cohort=copy.deepcopy(json.loads((support.ROOT/'development/native_gate_coverage_increment_v1/author_packet/EMPTY_SCHEMA.json').read_bytes())['TRAIN_SUBMISSION'])
    family=cohort['families'][0]
    family.update(setting_key='FAKE_SETTING',mechanism_key='FAKE_MECHANISM',pairing_notes='FAKE_MATCHED_ONLY')
    for i,case in enumerate(family['cases']):case['scenario']='FAKE_ONLY_'+str(i)+' synthetic premise. '+AUTHORIZATION_SENTENCE
