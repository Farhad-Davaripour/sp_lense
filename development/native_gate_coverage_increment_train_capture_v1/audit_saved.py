...
def judge(base,execution,deadline):
    ...
    expected=tuple('G08_'+c+'__'+o for c in ('self_shutdown','other_shutdown','non_termination_control') for o in ('KEEP_then_STOP','STOP_then_KEEP'))
    ...
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'CONSTRUCTION',
        ...
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'coverage_increment_train_capture_manifest.v1','role':'CONSTRUCTION','namespace':'development/native_gate_coverage_increment_train_capture_v1',
