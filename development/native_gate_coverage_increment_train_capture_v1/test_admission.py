"""Unmocked new synthetic capture/old authenticated52 metadata join; no real coordinates."""
...
CAP='development/native_gate_coverage_increment_train_capture_v1';PREP='development/native_gate_coverage_increment_train_preparation_v1';MAIN='development/native_gate_coverage_increment_v1'
NAMES=('gate','source_auth','fit_source_auth','support','input_reader','authority','audit_saved','dependencies','plan','storage','renderer','validate','prepare_core','prepare_reader','native_supervised_prepared_reader')
...
def main():
    ...
    fakecapture=h.capture(get(original/'preparation/inputs.json'));assert h.judge(fakecapture)['scientific_pass']
    ...
    pf=get(ROOT/PREP/'SOURCE_FREEZE.json');pf['source_sha256']['DEPENDENCIES.json']=sha((prep/'DEPENDENCIES.json').read_bytes())
    ...
    cf=get(ROOT/CAP/'SOURCE_FREEZE.json');cf['source_sha256']['input_reader.py']=sha((cap/'input_reader.py').read_bytes())
    ...
    for n in ('gate.py','source_auth.py'):clone(ROOT/MAIN/n,fit/n)
    cohort=lock['cohort'];sub=sha(jb(cohort));put(fit/'TRAIN_SUBMISSION.json',cohort)
    literal(fit/'source_auth.py','OLD',"Path("+repr(str(ROOT/'development/native_gate_order_invariant_v1'))+")")
    literal(fit/'source_auth.py','TRAIN_SUBMISSION_SHA',repr(sub))
    with isolated((fit,cap,prep)):
        import gate,plan,prepare_reader,authority,input_reader,fit_source_auth as native,source_auth as outer
        ...
        release={'schema':'coverage_increment_train_capture_release.v1','approved':True,'attempt':authority.ATTEMPT,'limits':authority.LIMITS,
        ...
        def publish_capture(change=None):
            mem=copy.deepcopy(fakecapture[2]);worker=get_bytes(mem['WORKER_RESULT.json']);worker['execution']=ex
            worker['cleanup']['state'].update(parameter_sha256=outer.STATE[0],buffer_sha256=outer.STATE[1]);mem['WORKER_RESULT.json']=jb(worker)
            ready=get_bytes(mem['LOADER_READY.json']);ready.update(execution=ex,native_initial_sha256=outer.STATE[0],native_initial_buffer_sha256=outer.STATE[1]);mem['LOADER_READY.json']=jb(ready)
            ...
            manifest={'schema':'coverage_increment_train_capture_manifest.v1','role':'CONSTRUCTION','namespace':CAP,'attempt':native.ATTEMPT,'execution':ex,'feature_contract':gate.CONTRACT,'selection':selected}
            ...
            binding={'schema':'coverage_increment_train_capture_binding.v1','approved':True,'role':'TRAIN','namespace':CAP,'submission_sha256':sub,'source_freeze_sha256':cs,'manifest_sha256':sha(gate.canonical(manifest))}
            put(fit/'root_release/TRAIN_CAPTURE_BINDINGS.json',binding)
            return manifest
        ...
        try:
            for n in outer.CHAIN_NAMES:
                if n not in sys.modules:
                    import types
                    sys.modules[n]=types.ModuleType('synthetic_collision_'+n)
            def check(name,expected,operation):
                modules={n:sys.modules.get(n) for n in outer.CHAIN_NAMES};path=list(sys.path)
                try:operation();ok=True;code=None
                except Exception as e:ok=False;code=str(e)
                assert sys.path==path and all(sys.modules.get(n) is m for n,m in modules.items()),'MODULE_PATH_RESTORATION'
                checks.append({'name':name,'accepted':ok,'code':code});print(json.dumps(checks[-1]),flush=True);assert ok==expected,checks[-1]
            check('unmocked_old52_plus_synthetic6_nested_cwd',True,lambda:outer.build_manifest(deadline=time.monotonic()+90))
            def extract():
                with outer.chain('train',outer.train_binding()) as a:
                    m=a.build_manifest(deadline=time.monotonic()+30);r,y=a.extract_features(m,deadline=time.monotonic()+30)
            ...
            check('wrong_source_restores_modules',False,lambda:outer.build_manifest(deadline=time.monotonic()+90));put(cap/'authority.py',raw)
        finally:os.chdir(previous)
    report={'status':'PASS','checks':checks,'production_function_mocks':0,'actual_coordinate_reads':0,'actual_old52_metadata_authentication':True,
        'new6':'ARTIFICIAL_ONLY','held_content_reads':0,'real_tokenizer_calls':0,'model_calls':0,'fits':0,'elapsed_seconds':time.monotonic()-start,
