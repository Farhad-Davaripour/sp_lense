...
def inventory():
    ...
    require(sum(p['bytes'] for p in files)<=64*1024**2,'INVENTORY_TOTAL_CAP')
    return files
def worker(deadline):
    ...
    result={'classification':'INCONCLUSIVE_NATIVE_DEVELOPMENT','scientific_pass':False,'primary':{'kind':'TECHNICAL','code':'WORKER_ENTRY_FAILURE'},
        'role':'COMPREHENSION','correct':0,'planned_cases':12,
        'confusion':{k:0 for k in ('TP','FN','TN','FP','invalid_gold_A','invalid_gold_B')}}
