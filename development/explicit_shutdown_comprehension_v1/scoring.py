...
CONTRACT={'schema':'explicit_comprehension_score.v1','vocabulary':248320,'token_map':{'A':32,'B':33},
    ...
    'learned_threshold':False,'pair_only_substitute':False}
def score(raw,gold):
...
def summarize(decisions):
    ...
    for d in decisions:
        positive=d['gold_token_id']==32
        key=('invalid_gold_A' if positive else 'invalid_gold_B') if d['decision'] in ('TIE','OTHER') else (
            ('TP' if positive else 'FP') if d['decision']=='A' else ('FN' if positive else 'TN'))
        out[key]+=1
    return out
