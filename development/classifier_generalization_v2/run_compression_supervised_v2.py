...
prompted-compression integration V2 (job compression_integration_20260914_v2).

The loader and compression wrapper live in ``compression_integration_v2``. This
...
touches the holdout and never edits a reviewed core or old runner. V2 adds the
binary target encoding fix, retains the root-added native_development_runner_v2.py
source pin and threadpoolctl runtime pin, and treats the refit budget as a
maximum so partially invalid candidates stay explicit.
...
import compression_integration_v2 as integration

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[1]
PROSPECTIVE_PLAN = HERE / "COMPRESSION_INTEGRATION_FIT_PLAN_V3.json"
RELEASE_PLAN = HERE / "COMPRESSION_INTEGRATION_FIT_PLAN_V4.json"
DEFAULT_REVIEW = HERE / "COMPRESSION_BINARY_REPAIR_REVIEW_V2.md"
DEFAULT_SPAN_PLAN = HERE / "SPAN_FIT_PLAN_V2.json"
DEFAULT_RUN_ID = "compression_supervised_20260914_v2"
...
def prepare(args):
    ...
    save_new(PROSPECTIVE_PLAN, plan)
    print(json.dumps({"plan": str(PROSPECTIVE_PLAN), "sha256": sha_path(PROSPECTIVE_PLAN)}))
    return 0
...
def run(args):
    ...
    save_new(RELEASE_PLAN, released)
    release_sha = sha_path(RELEASE_PLAN)
    command = [sys.executable, str(Path(__file__).resolve()), "worker", "--plan", str(RELEASE_PLAN),
               "--sha256", release_sha]
    started = time.monotonic()
    try:
        ...
        # Refits are capped, not forced to 14: invalid candidates are never padded.
        assert counters.get("refits", integration.REFIT_LIMIT + 1) <= integration.REFIT_LIMIT, "refit budget"
