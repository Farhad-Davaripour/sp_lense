...
Run from the repository root with the study runtime:
    ...
    development\\classifier_generalization_v2\\.runtime\\Scripts\\python.exe \
        -W error development/classifier_generalization_v2/test_prompted_span_runner_v1.py -v
"""
import ast
import hashlib
...
import prompted_span_runner_v1 as module
...
class SpanFixture:
    def __init__(self, directory):
        ...
        self.pins["prompt_sanity"] = self.put("PROMPTED_TOKENIZER_SANITY_V1.json", dict(
            status="PASS", query_sha256=module.tokens.QUERY_SHA256,
            adapter_sha256=self.sources[(STUDY / "prompted_input_adapter_v1.py").as_posix()],
            ...
            rows=[dict(case_id=c["case_id"], order=o) for c in sanity_cases for o in ("AB", "BA")]))
        self.new_lock = dict(
...
def make_capture_ctx(root, cases, *, raw_bytes=None, caps=None, run_id="toy_span_run"):
    ...
    return dict(
        lock=dict(run_id=run_id, caps=caps, threads=dict(intra=1, inter=1),
                  inputs={'prompt_sanity': {'sha256': 'a' * 64}}),
        lock_sha256="c" * 64,
        cases=cases,
        root=root,
...
class PreflightTests(unittest.TestCase):
    def test_prompt_sanity_query_length_boundary_and_coverage(self):
        checks = [('query_sha256', '0'*64, 'QUERY_PIN'),
                  ('max_full_tokens', 321, 'SANITY_TOKEN_CAP'),
                  ('last_shared_token_ids', [205], 'SANITY_BOUNDARY'),
                  ('rows', [], 'SANITY_CASE_BINDING')]
        for n, (key, value, error) in enumerate(checks):
            f = SpanFixture(Path(self.temporary.name)/('sanity_'+str(n)))
            doc = json.loads((f.root/f.pins['prompt_sanity']['path']).read_bytes())
            doc[key] = value
            f.new_lock['inputs']['prompt_sanity'] = f.put('BAD_SANITY.json', doc)
...
class ModuleHygieneTests(unittest.TestCase):
    def test_no_real_provider_imports_and_documented_binary_contract(self):
        ...
        self.assertEqual(module.JOB_ID, "prompted_span_runner_root_20260914")
