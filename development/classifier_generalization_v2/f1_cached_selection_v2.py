...
JOB_ID = "f1_cached_selection_root_repair_v2"
PLAN_SCHEMA = "f1_cached_selection_plan.v1"
RESULT_SCHEMA = "f1_cached_selection_result.v2"
SPLIT_SCHEMA = "f1_cached_selection_split_metrics.v2"
...
# --------------------------------------------------------------------------- #
def _verified_post_freeze(frozen):
    """Bind every post-freeze instruction to the canonical selection payload."""
    payload = frozen.get("frozen_identity_payload")
    _require(type(payload) is dict, "FROZEN_PAYLOAD")
    try:
        canonical = json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")
    except (TypeError, ValueError):
        raise F1SelectionError("FROZEN_PAYLOAD_ENCODING") from None
    _require(hashlib.sha256(canonical).hexdigest() == frozen.get("frozen_identity"), "FROZEN_IDENTITY_MISMATCH")
    _require(payload.get("schema") == RESULT_SCHEMA and payload.get("policy_id") == POLICY_ID, "FROZEN_POLICY")
    winner, match = payload.get("winner"), payload.get("artifact_match")
    _require(type(winner) is dict and type(match) is dict, "FROZEN_SELECTION")
    _require(frozen.get("winner") == winner and frozen.get("artifact_match") == match, "FROZEN_SELECTION_MISMATCH")
    _require(match.get("status") == "MATCHING_C_ARTIFACT", "REFIT_REQUIRED")
    cell = cell_key(winner["source"], winner["condition"], winner["dimension"], winner["family"])
    artifact = payload.get("artifacts", {}).get(cell)
    _require(type(artifact) is dict and match.get("cell") == cell, "FROZEN_CELL")
    _require(winner.get("C") == match.get("C") == artifact.get("C"), "FROZEN_C")
    _require(match.get("model_sha256") == artifact.get("model_sha256")
             and match.get("predictions_sha256") == artifact.get("predictions_sha256"), "FROZEN_ARTIFACT_PIN")
    post = {"action": "recompute_saved_predictions_at_tau", "cell": cell, "tau": winner["tau"],
            "C": winner["C"], "predictions": match["predictions"],
            "predictions_sha256": match["predictions_sha256"], "splits": list(POST_FREEZE_SPLITS)}
    _require(frozen.get("post_freeze") == post, "FROZEN_POST_MISMATCH")
    return post
...
def recompute_split_metrics(frozen, split, case_ids, truth, p_self, *, expected_case_ids=None,
    ...
    post = _verified_post_freeze(frozen)
    ...
    # Reporting preserves undefined precision and zero F1; selection eligibility does not apply.
