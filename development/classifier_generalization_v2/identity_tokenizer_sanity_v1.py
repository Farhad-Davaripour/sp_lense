"""Pinned identity tokenizer-only measurement; no model factory, no weights or fitting.

Minimal versioned copy of ``prompted_tokenizer_sanity_v1.py`` for the identity
condition. It uses the identity input adapter and a separate identity output
file and condition pin; the frozen old prompted sanity and its output are never
read or written. Tokenizer-only: 1 tokenizer load, 0 model loads, 0 fits.
"""
import argparse
import hashlib
import importlib
...
OUTPUT=HERE/'IDENTITY_TOKENIZER_SANITY_V1.json'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def preflight(adapter_sha):
    assert sha(HERE/'identity_input_adapter_v1.py')==adapter_sha
...
def worker(adapter_sha):
    ...
    for case in ctx['cases']:
        ...
        # Independently check that decoded model inputs contain the approved identity query.
        for order,ids in [('AB',ab),('BA',ba)]:
            decoded=tokenizer.decode(ids,skip_special_tokens=False,clean_up_tokenization_spaces=False)
            assert adapter.IDENTITY_QUERY in decoded and decoded.startswith(case['context_before_options']+'\n')
    ...
    result=dict(status='PASS',condition=adapter.IDENTITY_CONDITION,source_lock_sha256=OLD_LOCK_SHA,
        adapter_sha256=adapter_sha,reference_locks=ctx['lock']['reference_locks'],
        supervisor_source_sha256=sha(Path(__file__)),query=adapter.IDENTITY_QUERY,query_sha256=adapter.QUERY_SHA256,
...
def main():
    ...
    review=(HERE/'IDENTITY_PLAN_INPUT_REVIEW_V1.md').read_text().lower()
