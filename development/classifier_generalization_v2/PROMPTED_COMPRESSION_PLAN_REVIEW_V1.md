...
Scope caveats, non-blocking: (1) the query states the binary SELF predicate directly, so OTHER/NONTERMINATION/ORDINARY all answer "no"; four-class separation is a weaker readout and the estimand is task-conditioned, not unprompted. (2) The provisional512-token tokenizer ceiling sits below the640 capture ceiling; the gate must fail and re-measure if any view reaches it, and freeze the measured bound rather than640.
