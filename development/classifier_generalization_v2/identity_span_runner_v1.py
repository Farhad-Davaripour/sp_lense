"""Root versioned extension of the reviewed span runner: identity condition only.
Old source/caps remain unchanged. Numeric window schema is generic; identity
render, binding, index, receipt and query pins distinguish this condition. This
minimal versioned copy imports ``identity_input_adapter_v1`` (never the prompted
control) and pins the identity condition, query and sanity file separately.
...
Scope
-----
* One new finite lock schema ``identity_span_development_execution.v1``:
...
JOB_ID = "identity_span_runner_root_20260914"
SCHEMA = "identity_span_development_execution.v1"
INDEX_SCHEMA = "identity_span_capture_index.v1"
RECEIPT_SCHEMA = "identity_span_capture_receipt.v1"
BINARY_SCHEMA = "identity_span_capture_windows.v1"
STUDY = runner.STUDY
...
def preflight(lock_path, expected_sha256, root=runner.ROOT):
    ...
    sanity = data["identity_sanity"]
    need(sanity.get("status") == "PASS", "IDENTITY_SANITY")
    need(sanity.get("condition") == tokens.IDENTITY_CONDITION, "SANITY_CONDITION")
    need(sanity.get("query_sha256") == tokens.QUERY_SHA256, "QUERY_PIN")
    need(sanity.get("adapter_sha256") == sources[(STUDY / "identity_input_adapter_v1.py").as_posix()], "SANITY_SOURCE")
    ...
    shared_ids = sanity.get("last_shared_token_ids")
    need(
        type(shared_ids) is list
        and len(shared_ids) >= 1
        and all(type(v) is int and 0 <= v < contract.MAX_VOCAB_ID for v in shared_ids),
        "SANITY_BOUNDARY",
    )
    ...
    need(all(type(v.get("last_shared_token_id")) is int for v in rows), "SANITY_BOUNDARY")
...
def _validate_boundary(ids, view):
    """Re-check the shared-prefix/label token boundaries.

    The frozen contract cap (320) is enforced here. The last shared token is the
    adapter-recorded value, never assumed to be a fixed id.
    """
    contract.validate_token_ids(list(ids))
    readout = view["readout_index"]
    need(ids[readout] == view["last_shared_token_id"], "LAST_SHARED_ID")
...
def capture(ctx, started, *, verify=verify_snapshot, build=build_span_adapter):
    ...
    with windows_path.open("xb") as stream:
        for case in cases:
            ...
            need(prepared["provenance"]["condition"] == tokens.IDENTITY_CONDITION, "IDENTITY_CONDITION")
                    ...
                    records.append(
                        {
                            ...
                            "condition": tokens.IDENTITY_CONDITION,
                            ...
                            "readout_index": view["readout_index"],
                            "final_input_index": view["final_input_index"],
                            "last_shared_token_id": view["last_shared_token_id"],
    ...
    receipt_doc = {
        ...
        "condition": tokens.IDENTITY_CONDITION,
        "query_sha256": tokens.QUERY_SHA256,
        "identity_sanity_sha256": ctx["lock"]["inputs"]["identity_sanity"]["sha256"],
        "job_id": JOB_ID,
