"""Compact model-free prompted-compression comparison core V2 (job compression_core_implementation_20260914_v2).

V2 is a minimal successor of ``compression_comparison_v1``: it adds the missing
binary target encoding at both fit sites (CV and full-TRAIN refit), so the binary
family trains on integer 0/1 SELF-vs-rest targets and the four-class family keeps
the canonical string targets, exactly as ``linear_span_control_v1`` does. No
selection, cap, feature, ranking or serialization behavior changes.
...
JOB_ID = "compression_core_implementation_20260914_v2"
                    ...
                    for C in C_VALUES:
                        ...
                        try:
                            ...
                            target = ([1 if labels[case_id] == "SELF" else 0 for case_id in fit_ids]
                                      if family == "binary" else [labels[case_id] for case_id in fit_ids])
        ...
        for dim in CONDITION_DIMS[condition]:
            for family in FAMILIES:
                ...
                try:
                    ...
                    target = ([1 if case_data["labels"][case_id] == "SELF" else 0
                               for case_id in case_data["train_ids"]] if family == "binary"
                              else [case_data["labels"][case_id] for case_id in case_data["train_ids"]])
                    with warnings.catch_warnings():
                        warnings.simplefilter("error", ConvergenceWarning)
