"""Synthetic zero-provider tests for identity_span_runner_v1.py.
...
still required before any real execution. The toy-child test launches the true
identity runner entrypoint in a real child process, never the old span runner.

Run from the repository root with the study runtime:

    development\\classifier_generalization_v2\\.runtime\\Scripts\\python.exe \
        -W error development/classifier_generalization_v2/test_identity_span_runner_v1.py -v
"""
import ast
import hashlib
...
import identity_span_runner_v1 as module
...
class SpanFixture:
    def __init__(self, directory):
        ...
        sanity_cases = []
        for old_path in self.old_lock_paths:
            old_doc = json.loads(old_path.read_bytes())
            for role in ("train", "validation"):
                document = json.loads((self.root / old_doc["inputs"][role]["path"]).read_bytes())
                sanity_cases.extend(document["cases"])
        self.pins["identity_sanity"] = self.put("IDENTITY_TOKENIZER_SANITY_V1.json", dict(
            status="PASS", condition=module.tokens.IDENTITY_CONDITION, query_sha256=module.tokens.QUERY_SHA256,
            adapter_sha256=self.sources[(STUDY / "identity_input_adapter_v1.py").as_posix()],
            reference_locks=self.references, max_full_tokens=100, last_shared_token_ids=[198],
            exact_decode_reencode=True, query_in_shared_prefix=True,
            counts=dict(cases=320, views=640, tokenizer_loads=1, model_loads=0, forwards=0, fits=0),
            rows=[dict(case_id=c["case_id"], order=o, last_shared_token_id=198) for c in sanity_cases for o in ("AB", "BA")]))
        self.new_lock = dict(
...
def make_capture_ctx(root, cases, *, raw_bytes=None, caps=None, run_id="toy_span_run"):
    ...
    return dict(
        lock=dict(run_id=run_id, caps=caps, threads=dict(intra=1, inter=1),
                  inputs={'identity_sanity': {'sha256': 'a' * 64}}),
        lock_sha256="c" * 64,
        cases=cases,
        root=root,
...
class PreflightTests(unittest.TestCase):
    def test_identity_sanity_query_length_boundary_and_coverage(self):
        checks = [('query_sha256', '0'*64, 'QUERY_PIN'),
                  ('condition', 'prompted_fixed_query_v1', 'SANITY_CONDITION'),
                  ('max_full_tokens', 321, 'SANITY_TOKEN_CAP'),
                  ('last_shared_token_ids', [], 'SANITY_BOUNDARY'),
                  ('rows', [], 'SANITY_CASE_BINDING')]
        for n, (key, value, error) in enumerate(checks):
            f = SpanFixture(Path(self.temporary.name)/('sanity_'+str(n)))
            doc = json.loads((f.root/f.pins['identity_sanity']['path']).read_bytes())
            doc[key] = value
            f.new_lock['inputs']['identity_sanity'] = f.put('BAD_SANITY.json', doc)
...
class CaptureTests(unittest.TestCase):
    ...
    def test_streams_binary_index_and_receipt_without_json_float_arrays(self):
        ...
        self.assertEqual(index["condition"], module.tokens.IDENTITY_CONDITION)
        self.assertEqual(index["query"], module.tokens.IDENTITY_QUERY)
        self.assertEqual(index["query_sha256"], module.tokens.QUERY_SHA256)
        ...
        for record in index["records"]:
            ...
            self.assertEqual(record["last_shared_token_id"], 198)
        ...
        self.assertEqual(receipt["condition"], module.tokens.IDENTITY_CONDITION)
        self.assertEqual(receipt["query_sha256"], module.tokens.QUERY_SHA256)
    ...
    def test_real_watched_toy_worker_path(self):
        ...
        def toy_launch(command, stream, cwd):
            ...
            script = (
                ...
                "import identity_span_runner_v1 as r; "
...
class ModuleHygieneTests(unittest.TestCase):
    def test_no_real_provider_imports_and_documented_binary_contract(self):
        ...
        self.assertEqual(module.JOB_ID, "identity_span_runner_root_20260914")
        self.assertEqual(module.SCHEMA, "identity_span_development_execution.v1")
        self.assertEqual(module.INDEX_SCHEMA, "identity_span_capture_index.v1")
        self.assertEqual(module.RECEIPT_SCHEMA, "identity_span_capture_receipt.v1")
        self.assertEqual(module.BINARY_SCHEMA, "identity_span_capture_windows.v1")
        self.assertIn("identity_input_adapter_v1.py", module.SOURCE_NAMES)
        self.assertIn("identity_span_runner_v1.py", module.SOURCE_NAMES)
        self.assertNotIn("prompted_input_adapter_v1.py", module.SOURCE_NAMES)
        self.assertNotIn("prompted_input_adapter_v1", roots)
        self.assertEqual(module.INPUT_ROLES[-1], "identity_sanity")
        self.assertEqual(module.tokens.IDENTITY_CONDITION, "identity_fixed_query_v1")
        self.assertNotEqual(module.SCHEMA, "prompted_span_development_execution.v1")
