...
Job ``compression_integration_20260914_takeover_v1``. This module is the smallest
...
reviewed ``compression_comparison_v1.run`` (210 CV + 14 refits + <=12 PCA fits
when every fold is valid) into an exclusive fresh directory and enforces the
256 MiB total-output bound. The root wrapper owns the release, the external
600-second watch and the process boundary.
"""
...
JOB_ID = "compression_integration_20260914_takeover_v1"
PLAN_SCHEMA = "compression_integration_plan.v1"
...
SOURCE_FILES = (
    STUDY_DIR + "/compression_integration_v1.py",
    STUDY_DIR + "/compression_comparison_v1.py",
    ...
    STUDY_DIR + "/run_compression_supervised_v1.py",
)
...
def _verify_result(result, plan, output):
    ...
    _need(counters.get("refits") == REFIT_LIMIT, "RESULT_REFIT_BUDGET", str(counters.get("refits")))
