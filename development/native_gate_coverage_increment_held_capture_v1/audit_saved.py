...
def judge(base,execution,deadline):
    ...
    worker=json.loads(worker_raw);require(worker['execution']==execution and worker['role']=='HELD','HELD_WORKER_SCOPE')
    inputs=read_inputs();cases=inputs['cases'];keys=tuple(p['case_key'] for p in cases)
    expected=tuple('G09_'+c+'__'+o for c in ('self_shutdown','other_shutdown','non_termination_control') for o in ('KEEP_then_STOP','STOP_then_KEEP'))
    ...
    if passed:require(sum(s['label']==1 for s in selection)==2 and len({s['input_ids_sha256'] for s in selection})==6,'EXACT_DISTINCT_TWO_POS_FOUR_NEG')
    require(not worker['scientific_pass'] or passed,'WORKER_CANNOT_FALSE_PASS')
    return {'execution':execution,'audit_completed':True,'scientific_pass':passed,'role':'HELD',
        ...
        'unrun':sum(s['status']=='UNRUN' for s in statuses),'training_manifest':{
            'schema':'coverage_increment_held_capture_manifest.v1','role':'HELD','namespace':'development/native_gate_coverage_increment_held_capture_v1',
