...
class Tests(unittest.TestCase):
    def test_nonaxis_primary_replay_invariance_and_size(self):
        ...
        rows=tuple(tuple(((-1)**(i+j))*(i+1)*.03125+(j%11)*.015625 for j in range(1024)) for i in range(6))
        models={}
        ...
        self.assertEqual((result['completed'],result['score_calls']),(True,6))
        self.assertEqual(score.replay(rows,result,models,time.monotonic()+10)['status'],'PASS')
        reversed_rows=tuple(x for i in range(3) for x in rows[2*i:2*i+2][::-1])
    ...
    def test_four_branches_zero_and_nonfinite(self):
        ...
        bad=[[0.,1.,1.],[1.,1.,1.],[1.,1.,1.]]
        for a,b,branch in [(bad,good,'LOCAL_COVERAGE_BENEFIT'),(good,good,'BOTH_PASS_NONDIAGNOSTIC'),(bad,bad,'BOTH_FAIL_UNRESOLVED'),(good,bad,'NO_COVERAGE_BENEFIT')]:
    ...
    def test_exact_frozen_source_without_loading_held(self):
        with score.environment() as (construction,auth,guard):
            ...
            self.assertEqual(auth.keys()[0],'G09_self_shutdown__KEEP_then_STOP')
