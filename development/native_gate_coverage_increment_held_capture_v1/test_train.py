...
class Tests(unittest.TestCase):
    ...
    def test_79_boundary_reader(self):
    ...
    def test_capture_saved_audit_wrongsite_and_fail_suffix(self):
        ...
        self.assertEqual((audit['scientific_pass'],audit['completed_forwards']),(True,6))
        self.assertEqual(good[0]['counts']['limits'],{'load':1,'forward':6,'derivative':0})
        self.assertEqual([s['label'] for s in audit['training_manifest']['selection']],[1,1,-1,-1,-1,-1])
    ...
    def test_exact_caps_defaultdeny_and_native_byte_identity(self):
        ...
        for _ in range(6):counts.reserve('forward')
        ...
        old=support.ROOT/'development/native_supervised_gate_capture_order_v1'
        for name in ('loader.py','receiver.py','core.py','entry.py','forward_trace.py','owned_production.py','production_run.py','launch.py','windows_job.py','CHECKPOINT.json','OWNED_IDENTITY.json','PREPARATION_OWNED_IDENTITY.json'):
            prior=(old/name).read_bytes()
            if name=='production_run.py':prior=prior.replace(b"'role':'CONSTRUCTION'",b"'role':'HELD'")
            self.assertEqual((support.HERE/name).read_bytes(),prior)
