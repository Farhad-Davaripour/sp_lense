"""PLAN_V3 stage-3 cached J-lens pilot supervisor V2 (job ``jlens_pilot_v1``).

TWO-PHASE, TORCH-FREE PHASE B. ``run_jlens_pilot_v1.py`` (preserved, untouched)
loaded the published lens ``.pt`` with ``torch`` inside the worker, which pinned
the pilot to the repo ``.venv`` that has no scikit-learn, while PLAN_V3 B4
requires the locked classifier runtime (NumPy 2.5.3 / SciPy 1.18.1 /
scikit-learn 1.9.1). V2 resolves the split with an explicit two-phase release:

* Phase A (``prepare_lens_export_v1.py``, repo ``.venv``) loads exactly the three
  reviewed lens layers with ``torch.load(weights_only=True)`` and writes a
  hash-pinned float32 ``.npz`` export plus a receipt; and
* Phase B (this module, locked classifier ``.runtime``) re-verifies the original
  lens pin, the export pin and the export receipt, then runs the whole cached
  pilot with NumPy + scikit-learn only.

This module imports no ``torch``/``transformers``/``tokenizers``/``safetensors``/
``transformer_lens`` at import time and in every test. It never loads a model,
tokenizer, forward, derivative, PCA, cone or estimator pickle, and it neither
creates nor claims a scientific result.
...
JOB_ID = pilot.JOB_ID
SCHEMA = "jlens_pilot_execution.v2"
...
SOURCE_PATHS = (
    ...
    (STUDY / "run_jlens_pilot_v2.py").as_posix(),
    (STUDY / "prepare_lens_export_v1.py").as_posix(),
    (STUDY / "prepare_pilot_lock_v2.py").as_posix(),
    (STUDY / "JLENS_SURFACE_TOKEN_RECEIPT_V1.json").as_posix(),
...
LENS_EXPORT_ARRAYS = ("J6", "J10", "J18")


def _lens_export(inputs, root):
    """Verify the Phase-A lens export pin, receipt binding and lens binding."""
    entry = inputs["lens_export"]
    need(type(entry) is dict and set(entry) == {"path", "bytes", "sha256", "receipt"}, "LENS_EXPORT_SCHEMA")
    pin = {"path": entry["path"], "bytes": entry["bytes"], "sha256": entry["sha256"]}
    _pin(pin, "LENS_EXPORT_PIN")
    _pin(entry["receipt"], "LENS_EXPORT_RECEIPT_PIN")
    need(pin["path"].startswith(STUDY.as_posix() + "/"), "LENS_EXPORT_SCOPE")
    need(entry["receipt"]["path"].startswith(STUDY.as_posix() + "/"), "LENS_EXPORT_SCOPE")
    pilot.reject_forbidden_path(pin["path"])
    pilot.reject_forbidden_path(entry["receipt"]["path"])
    path = (root / pin["path"]).resolve()
    need(path.is_relative_to(root), "LENS_EXPORT_SCOPE")
    io.strict_file(path, pin, full_hash=True)
    receipt_pin = {"path": entry["receipt"]["path"], "sha256": entry["receipt"]["sha256"]}
    receipt = runner.strict_json(runner.pinned(root, receipt_pin))
    need(receipt.get("schema") == "jlens_lens_export.v1", "LENS_EXPORT_RECEIPT")
    need(receipt.get("release") == io.RELEASE_V1, "LENS_EXPORT_RELEASE")
    need(receipt.get("layers") == list(BLOCKS), "LENS_EXPORT_LAYERS")
    need(set(receipt.get("arrays", {})) == set(LENS_EXPORT_ARRAYS), "LENS_EXPORT_ARRAYS")
    export = receipt.get("export", {})
    need(
        export.get("filename") == path.name
        and export.get("bytes") == pin["bytes"]
        and export.get("sha256") == pin["sha256"],
        "LENS_EXPORT_BINDING",
    )
    lens = receipt.get("lens", {})
    need(
        lens.get("sha256") == io.LENS_PIN["sha256"]
        and lens.get("revision") == io.LENS_PIN["revision"]
        and lens.get("filename") == io.LENS_PIN["filename"],
        "LENS_EXPORT_LENS_BINDING",
    )
    counters = receipt.get("counters", {})
    need(
        counters.get("model_loads") == 0
        and counters.get("tokenizer_loads") == 0
        and counters.get("forwards") == 0
        and counters.get("derivatives") == 0
        and counters.get("fits") == 0,
        "LENS_EXPORT_COUNTERS",
    )
    return pin, path, receipt
...
def _inputs(root, lock):
    ...
    export_pin, export_path, export_receipt = _lens_export(inputs, root)
...
def load_tensors(ctx, proof, token_ids):
    """Load only the pinned lens export, the norm vector and <= 6 unembedding rows.

    No ``torch`` is imported in this phase: the three Jacobian matrices come from
    the hash-pinned Phase-A ``.npz`` export produced by the sanctioned
    ``torch.load(weights_only=True)`` loader in the repo ``.venv``. The original
    lens pin and the export receipt binding were already checked in preflight. No
    model and no tokenizer is instantiated; the safetensors body is seeked per
    selected row.
    """
    lock = ctx["lock"]
    export = ctx["data"]["lens_export"]
    jacobians = lens_export_jacobians(export["path"])
...
def lens_export_jacobians(path):
    """Load and validate the three Phase-A Jacobian matrices from a pinned export."""
    with np.load(path, allow_pickle=False) as payload:
        need(set(payload.files) == set(LENS_EXPORT_ARRAYS), "EXPORT_KEYS")
        jacobians = {}
        for layer in BLOCKS:
            matrix = np.asarray(payload["J%d" % layer])
            ...
            jacobians[layer] = np.ascontiguousarray(matrix, dtype=np.float32)
    return jacobians
...
def capture(ctx, started, *, verify=verify_snapshot, tensors=None, hidden=None, scores=None, factory=None):
    ...
    receipt = {
        ...
        "lens_export_sha256": ctx["data"]["lens_export"]["pin"]["sha256"],
        "notes": {
            "cached_readout_only": True,
            "no_model_or_tokenizer_load": True,
            "lens_loaded_from_pinned_phase_a_export": True,
            "torch_imported_in_phase_b": False,
...
def verify_safe_interfaces(root=ROOT, pins=None):
    ...
    if pins.get("lens_export") is not None:
        export_pin, export_path, export_receipt = _lens_export({"lens_export": pins["lens_export"]}, root)
        summary["lens_export"] = {
            "bytes": export_pin["bytes"],
            "sha256": export_pin["sha256"],
            "lens_sha256": export_receipt["lens"]["sha256"],
        }
    return summary
